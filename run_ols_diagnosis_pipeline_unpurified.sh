...
# Step 11   : 2_G_alpha_neutralizer_unpurified (純化撤去版 = 恒等コピー、生Yを_neutralized命名で出力)
...
# (infer_period は別途手動で実行: --start/--end/--out-dir を都度指定)
#
# ログ: /workspace/logs/OLS_diagnosis/NN_<name>.log
#
# 使い方:
#   全実行 (Step 1-15):
#     bash run_ols_diagnosis_pipeline_unpurified.sh
#   範囲指定 (例: Step 4-15):
#     bash run_ols_diagnosis_pipeline_unpurified.sh 4 15
#   2_G 以降のみ (Step 11-15):
#     bash run_ols_diagnosis_pipeline_unpurified.sh 11 15
#
...
if ! [[ "$FROM_STEP" =~ ^[0-9]+$ ]] || ! [[ "$TO_STEP" =~ ^[0-9]+$ ]]; then
    echo "ERROR: FROM_STEP / TO_STEP は整数で指定してください (例: bash $0 4 15)"
    exit 1
fi
...
# ============================================================================
# [純化撤去版 §11.34.14] Step 11: 2_G を unpurified 版で実行
# ============================================================================
# 通常の run_validation_step 11 G は /workspace/validation/2_G_*.py を
# glob + head -1 で拾うため、純化版 (2_G_alpha_neutralizer.py) と
# 純化なし版 (2_G_alpha_neutralizer_unpurified.py) が両方あると ASCII 順で
# 純化版が選ばれてしまう。よってここでは unpurified を明示パスで解決して呼ぶ。
#
# unpurified 版は OLS 純化計算だけを抜いた恒等コピー (neutralized = 生 Y)。
# final_feature_set による列選別・出力命名 _neutralized・書き出しは純化版と
# 同一なので、出力 S5 は純化版と bit 同一の列名・構造 (中身は生 Y) になり、
# 下流 (Step12 create_proxy_labels = input S5 のまま無改変) はそのまま動く。
# ============================================================================
if [ 11 -lt "$FROM_STEP" ] || [ 11 -gt "$TO_STEP" ]; then
    echo "[Step 11: 2_G_alpha_neutralizer_unpurified] SKIP (out of range $FROM_STEP-$TO_STEP)"
else
    # unpurified 版を探す (純化版 2_G が置かれる /workspace/validation を含めて解決)。
    # resolve_script の既定探索パスには validation が無いため Step11 専用に明示する。
    G_UNPURIFIED=""
    for d in /workspace/validation /workspace/pipeline /workspace/models /workspace/scripts /workspace; do
        if [ -f "$d/2_G_alpha_neutralizer_unpurified.py" ]; then
            G_UNPURIFIED="$d/2_G_alpha_neutralizer_unpurified.py"
            break
        fi
    done
    if [ -z "$G_UNPURIFIED" ]; then
        echo "ERROR: 2_G_alpha_neutralizer_unpurified.py が見つかりません" >&2
        echo "       (/workspace/validation 等に配置してください)" >&2
        exit 1
    fi
    run_no_stdin 11 "2_G_alpha_neutralizer_unpurified" "$G_UNPURIFIED"
fi

# ============================================================================
# Step 12-15: create_proxy_labels → aggregate → uniqueness (infer_period は別途手動)
# ============================================================================
...
# ============================================================================
# [純化撤去版 §11.34.14] Step 16 (infer_period) は削除。
# infer_period.py は --start/--end/--out-dir が必須で、 期間を都度指定して
# 別途手動で回す運用のため、 本 pipeline には含めない。
# 例:
