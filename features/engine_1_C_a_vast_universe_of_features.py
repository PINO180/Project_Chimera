...
class MemoryMonitor:
    ...
    def check_memory_safety(self) -> Tuple[bool, str]:
        ...
        message = f"メモリ使用量: {used_gb:.2f}GB / {self.limit_gb:.2f}GB ({percent:.1f}%)"
        return True, message
...
@nb.guvectorize(['void(float64[:], int64, float64[:])'], '(n),()->(n)', nopython=True, cache=True)
...
@nb.guvectorize(['void(float64[:], float64[:], float64[:], int64, float64[:])'], '(n),(n),(n),()->(n)', nopython=True, cache=True)
...
@nb.guvectorize(['void(float64[:], float64[:], float64[:], int64, float64[:])'], 
                '(n),(n),(n),()->(n)', nopython=True, cache=True)
...
@nb.guvectorize(['void(float64[:], float64[:], float64[:], int64, int64, int64, float64[:])'], 
                '(n),(n),(n),(),(),()->(n)', nopython=True, cache=True)
...
@nb.guvectorize(['void(float64[:], float64[:], float64[:], float64[:], float64[:])'], 
                '(n),(n),(n),(n)->(n)', nopython=True, cache=True)
...
class DataEngine:
    ...
    def validate_data_source(self) -> bool:
        ...
        # timeframeディレクトリの確認
        timeframe_dirs = [d for d in self.base_path.iterdir() if d.is_dir() and d.name.startswith("timeframe=")]
    ...
    def get_parquet_paths(self, timeframe: str) -> List[Path]:
        ...
        if not timeframe_path.exists():
            logger.warning(f"タイムフレームディレクトリが存在しません: {timeframe_path}")
            return []
...
class OutputEngine:
    ...
    def save_features(self, lazy_frame: pl.LazyFrame, timeframe: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if output_path.exists():
                ...
                logger.info(f"保存完了: {output_file} ({file_size_mb:.2f}MB, {elapsed_time:.2f}秒)")
    ...
    def merge_intermediate_files(self, file_paths: List[str], timeframe: str) -> Dict[str, Any]:
        """中間ファイル統合（tickデータ用）"""
        start_time = time.time()
        ...
        try:
            ...
            if len(lazy_frames) == 1:
                ...
                for lf in lazy_frames[1:]:
                    ...
                    non_timestamp_cols = [col for col in schema.names() if col != "timestamp"]
            ...
            logger.info(f"統合完了: {output_file} ({file_size_mb:.2f}MB, {elapsed_time:.2f}秒)")
    ...
    def save_processing_metadata(self, metadata: Dict[str, Any]):
        """処理メタデータ保存"""
        metadata_file = f"metadata_{self.config.engine_id}_{time.strftime('%Y%m%d_%H%M%S')}.json"
...
class CalculationEngine:
    ...
    def get_feature_groups(self) -> Dict[str, List[str]]:
        ...
        return {
            # RSI系を4分割
            "group_1a_rsi_basic": ["rsi_14", "rsi_21", "rsi_momentum_14", "rsi_momentum_21"],
            "group_1b_rsi_extended": ["rsi_30", "rsi_50", "rsi_momentum_30", "rsi_momentum_50"],
            "group_1c_rsi_stochastic": ["stochastic_rsi_14", "stochastic_rsi_21"],
            "group_1d_rsi_divergence": ["rsi_divergence_14", "rsi_divergence_21"],
            
            # MACD系を2分割
            "group_2a_macd_standard": ["macd_12_26", "macd_signal_12_26_9", "macd_histogram_12_26_9"],
            "group_2b_macd_variants": ["macd_5_35", "macd_signal_5_35_5", "macd_histogram_5_35_5", 
                                    "macd_19_39", "macd_signal_19_39_9", "macd_histogram_19_39_9"],
            
            # ボリンジャーバンド系を3分割
            "group_3a_bb_20": ["bb_upper_20_2", "bb_lower_20_2", "bb_percent_20_2", "bb_width_20_2", 
                            "bb_width_pct_20_2", "bb_position_20_2"],
            "group_3b_bb_30_50": ["bb_upper_30_2", "bb_lower_30_2", "bb_percent_30_2", "bb_width_30_2",
                                "bb_upper_50_2", "bb_lower_50_2", "bb_percent_50_2", "bb_width_50_2"],
            "group_3c_bb_variants": ["bb_upper_20_2.5", "bb_lower_20_2.5", "bb_percent_20_2.5",
                                    "bb_upper_20_3", "bb_lower_20_3", "bb_percent_20_3"],
            
            # ATR系を2分割
            "group_4a_atr_basic": ["atr_13", "atr_21", "atr_pct_13", "atr_pct_21"],
            "group_4b_atr_extended": ["atr_34", "atr_55", "atr_trend_13", "atr_trend_21", "atr_volatility_13", "atr_volatility_21"],
            
            # オシレーター系を2分割
            "group_5a_oscillators_basic": ["adx_13", "adx_21", "di_plus_13", "di_plus_21", "di_minus_13", "di_minus_21"],
            "group_5b_oscillators_extended": ["stoch_k_14", "stoch_d_14_3", "aroon_up_14", "aroon_down_14", "williams_r_14"],
            
            # モメンタム系を2分割
            "group_6a_momentum_basic": ["dpo_20", "trix_14", "momentum_10", "momentum_20"],
            "group_6b_momentum_extended": ["ultimate_oscillator", "tsi_25", "rate_of_change_10", "rate_of_change_20"],
            
            # 移動平均線系を2分割に統合
            "group_7_moving_averages": ["sma_10", "sma_20", "sma_50", "ema_10", "ema_20", "ema_50", "hma_21", "kama_21"]
        }
        ...
        for period in self.config.window_sizes["rsi"]:  # [14, 21, 30, 50]
            ...
            exprs.append(
                pl.col("close").map_batches(
                    lambda s: calculate_rsi_numba(s.to_numpy(), period),
                    return_dtype=pl.Float64
                ).diff().alias(f"{self.prefix}rsi_momentum_{period}")
            )
        ...
        for period in [14, 21]:
            ...
            exprs.append(
                ((rsi_col - rsi_col.rolling_min(period)) / 
                 (rsi_col.rolling_max(period) - rsi_col.rolling_min(period) + 1e-10) * 100)
                .alias(f"{self.prefix}stochastic_rsi_{period}")
            )
        
        # RSIダイバージェンス（価格とRSIの乖離）
        for period in [14, 21]:
            price_change = (pl.col("close") - pl.col("close").shift(period)) / pl.col("close").shift(period)
            rsi_col = pl.col("close").map_batches(
                lambda s: calculate_rsi_numba(s.to_numpy(), period),
                return_dtype=pl.Float64
            )
            ...
            exprs.append(
                (price_change - rsi_change).alias(f"{self.prefix}rsi_divergence_{period}")
            )
    ...
    def create_macd_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for fast, slow, signal in macd_configs:
            ...
            # シグナルライン
            signal_line = macd_line.ewm_mean(span=signal, adjust=False).alias(f"{self.prefix}macd_signal_{fast}_{slow}_{signal}")
            exprs.append(signal_line)
            
            # ヒストグラム
            histogram = (macd_line - signal_line.ewm_mean(span=signal, adjust=False)).alias(f"{self.prefix}macd_histogram_{fast}_{slow}_{signal}")
    ...
    def create_bollinger_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for period in bb_periods:
            for num_std in bb_stdevs:
                ...
                # 上限・下限バンド
                upper = (sma + num_std * std).alias(f"{self.prefix}bb_upper_{period}_{num_std}")
                lower = (sma - num_std * std).alias(f"{self.prefix}bb_lower_{period}_{num_std}")
                
                exprs.extend([upper, lower])
                
                # BB内の位置（%B）
                percent_b = ((pl.col("close") - lower) / (upper - lower + 1e-10)).alias(f"{self.prefix}bb_percent_{period}_{num_std}")
                exprs.append(percent_b)
                
                # BB幅
                width = (upper - lower).alias(f"{self.prefix}bb_width_{period}_{num_std}")
                exprs.append(width)
                
                # BB幅のパーセンタイル
                width_pct = (width / sma * 100).alias(f"{self.prefix}bb_width_pct_{period}_{num_std}")
                exprs.append(width_pct)
                
                # BB内での相対位置
                position = ((pl.col("close") - sma) / (std + 1e-10)).alias(f"{self.prefix}bb_position_{period}_{num_std}")
    ...
    def create_atr_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for period in atr_periods:
            # 基本ATR（Numba版）
            atr = pl.struct(["high", "low", "close"]).map_batches(
            ...
            # ATRパーセンテージ（価格に対する比率）
            atr_pct = (atr / pl.col("close") * 100).alias(f"{self.prefix}atr_pct_{period}")
            ...
            atr_volatility = atr.rolling_std(period).alias(f"{self.prefix}atr_volatility_{period}")
            exprs.append(atr_volatility)
            
            # ATRベースのバンド
            atr_multipliers = [1.5, 2.0, 2.5]
            for mult in atr_multipliers:
                upper_band = (pl.col("close") + atr * mult).alias(f"{self.prefix}atr_upper_{period}_{mult}")
                lower_band = (pl.col("close") - atr * mult).alias(f"{self.prefix}atr_lower_{period}_{mult}")
...
# ===== ブロック5開始 =====
    ...
    def create_oscillator_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for k_period, d_period, slow_period in stoch_periods:
            stoch_k = pl.struct(["high", "low", "close"]).map_batches(
                lambda s: calculate_stochastic_numba(
                    s.struct.field("high").to_numpy(),
                    s.struct.field("low").to_numpy(),
                    s.struct.field("close").to_numpy(),
                    k_period, d_period, slow_period
                ),
                return_dtype=pl.Float64
            ...
            # %D (signal line)
            stoch_d = stoch_k.rolling_mean(d_period).alias(f"{self.prefix}stoch_d_{k_period}_{d_period}")
            exprs.append(stoch_d)
            
            # Slow %D
            slow_d = stoch_d.rolling_mean(slow_period).alias(f"{self.prefix}stoch_slow_d_{k_period}_{d_period}_{slow_period}")
            exprs.append(slow_d)
        
        # ADX, DI+, DI- (個別のNumba関数)
        for period in self.config.window_sizes["adx"]:  # [13, 21, 34]
            # ADX
            adx = pl.struct(["high", "low", "close"]).map_batches(
            ...
            # DI+
            di_plus = pl.struct(["high", "low", "close"]).map_batches(
            ...
            # DI-
            di_minus = pl.struct(["high", "low", "close"]).map_batches(
        ...
        for period in aroon_periods:
            ...
            # Aroon Oscillator
            aroon_osc = (aroon_up - aroon_down).alias(f"{self.prefix}aroon_oscillator_{period}")
            exprs.append(aroon_osc)
        
        # Williams %R
        williams_periods = [14, 28, 56]
        for period in williams_periods:
            williams_r = pl.struct(["high", "low", "close"]).map_batches(
    ...
    def create_momentum_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        uo = pl.struct(["high", "low", "close", "volume"]).map_batches(
        ...
        for period in roc_periods:
            roc = ((pl.col("close") - pl.col("close").shift(period)) / 
                   pl.col("close").shift(period) * 100).alias(f"{self.prefix}rate_of_change_{period}")
            exprs.append(roc)
        
        # Momentum
        momentum_periods = [10, 20, 30, 50]
        for period in momentum_periods:
            momentum = (pl.col("close") - pl.col("close").shift(period)).alias(f"{self.prefix}momentum_{period}")
    ...
    def create_advanced_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        kst = ((roc_10 * 1 + roc_15 * 2 + roc_20 * 3 + roc_30 * 4) / 10 * 100).alias(f"{self.prefix}kst")
        exprs.append(kst)
        ...
        for period in rvi_periods:
            ...
            rvi = (numerator / (denominator + 1e-10)).alias(f"{self.prefix}relative_vigor_index_{period}")
            exprs.append(rvi)
        ...
        coppock = (roc_11 + roc_14).rolling_mean(10).alias(f"{self.prefix}coppock_curve")
        ...
        for fast, slow in po_configs:
            ...
            po = ((fast_ma - slow_ma) / slow_ma * 100).alias(f"{self.prefix}price_oscillator_{fast}_{slow}")
            exprs.append(po)
    ...
    def create_moving_average_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for period in ma_periods:
            # SMA
            sma = pl.col("close").rolling_mean(period).alias(f"{self.prefix}sma_{period}")
            exprs.append(sma)
            
            # SMA乖離率
            sma_deviation = ((pl.col("close") - sma) / sma * 100).alias(f"{self.prefix}sma_deviation_{period}")
            exprs.append(sma_deviation)
            
            # EMA
            ema = pl.col("close").ewm_mean(span=period, adjust=False).alias(f"{self.prefix}ema_{period}")
            exprs.append(ema)
            
            # EMA乖離率
            ema_deviation = ((pl.col("close") - ema) / ema * 100).alias(f"{self.prefix}ema_deviation_{period}")
            ...
            # WMAは複雑なので簡略版
            wma = pl.col("close").rolling_mean(period).alias(f"{self.prefix}wma_{period}")
            exprs.append(wma)
        
        # HMA (Hull Moving Average) - Numba版
        for period in self.config.window_sizes["hma"]:  # [21, 34, 55]
            hma = pl.col("close").map_batches(
        ...
        for period in trend_periods:
            ...
            # 簡略版：価格変化率で代用
            trend_slope = ((pl.col("close") - pl.col("close").shift(period)) / period).alias(f"{self.prefix}trend_slope_{period}")
            exprs.append(trend_slope)
            
            # トレンド強度（R²の代用として標準偏差の逆数）
            trend_strength = (1 / (pl.col("close").rolling_std(period) + 1e-10)).alias(f"{self.prefix}trend_strength_{period}")
            exprs.append(trend_strength)
            
            # トレンド一貫性（方向変化の頻度）
            direction_changes = pl.col("close").diff().sign().diff().abs()
            trend_consistency = (1 - direction_changes.rolling_mean(period) / 2).alias(f"{self.prefix}trend_consistency_{period}")
    ...
    def create_basic_processing_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        """基本データ処理特徴量"""
    ...
    def apply_quality_assurance(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        try:
            ...
            feature_columns = [
                ...
                if col.startswith(self.prefix) and schema[col] in [pl.Float32, pl.Float64]
            ]
    ...
    def calculate_feature_group(self, lazy_frame: pl.LazyFrame, group_name: str, 
                            feature_list: List[str]) -> pl.LazyFrame:
        """特定グループの特徴量計算（細分化対応）"""
        ...
        available_features = [col for col in schema.names() if col.startswith(self.prefix)]
        ...
        result = result.select([pl.col(col) for col in final_columns if col in schema.names()])
        
        # 品質保証適用
        result = self.apply_quality_assurance(result)
        
        logger.info(f"グループ計算完了: {group_name} - {len(selected_features)}個の特徴量")
        return result
...
def run_on_partitions_mode(config: ProcessingConfig):
    ...
    logging.info("【実行モード】パーティション化されたTickデータの特徴量計算を開始します...")
    
    timeframe = "tick" # timeframeを明示的に定義
    
    PARTITION_ROOT = Path(config.partitioned_tick_path)
    # 【修正点】プロンプトの命名規則に準拠したディレクトリ名に変更
    FEATURES_ROOT = Path(config.output_base_path) / f"features_{config.engine_id}_{timeframe}/"
    ...
    for i, current_path in enumerate(sorted_partitions):
        logging.info(f"=== 水平処理 ({i+1}/{len(sorted_partitions)}): {current_path.relative_to(PARTITION_ROOT)} ===")
        
        prev_path = sorted_partitions[i-1] if i > 0 else None

        try:
            augmented_df, len_current = create_augmented_frame(current_path, prev_path, W_MAX)
            ...
            for group_name, feature_list in feature_groups.items():
                ...
                group_lf_result = calculation_engine.calculate_feature_group(augmented_df.lazy(), group_name, feature_list)
                
                # 一時ファイルとして保存
                temp_group_path = FEATURES_ROOT / f"temp_{current_path.name}_{group_name}.parquet"
            ...
            for f in intermediate_group_files[1:]:
                ...
                new_feature_columns = [col for col in feature_df.columns if col not in base_df.columns]
                
                # 新しい特徴量列のみを水平に結合
                base_df = base_df.with_columns(
                    feature_df.select(new_feature_columns)
                )
            ...
            logging.error(f"パーティション {current_path} の処理中にエラー: {e}", exc_info=True)
            continue
