...
# ────────────────────────────────────────────────────────────────
# 学習側 (engine_1_B) と本番側 (rfe_1B) で完全同一の Numba 関数を
...
class CalculationEngine:
    ...
    def _get_all_feature_expressions(self, timeframe: str = "M1") -> Dict[str, pl.Expr]:
        ...
        for window in [20, 50]:
            mean_col = _srm_expr("close", window)
            std_col = _srs_expr("close", window, ddof=1)
        ...
        expressions[f"{p}t_dist_scale_50"] = (
            ...
            # ▼▼ [Phase E] pct_change().rolling_std → stable_rolling_std (inline map_batches)
            / (
                pl.col("close").pct_change().map_batches(
                    lambda s: pl.Series(stable_rolling_std(s.to_numpy().astype(np.float64), 20, 1)),
                    return_dtype=pl.Float64,
                )
                + 1e-10
            )
        ...
        # ▼▼ [Phase E] pct_change().rolling_std(20) → stable_rolling_std (inline map_batches)
        ...
        # ▼▼ [Phase E] volume.rolling_mean(lookback_bars) → _srm_expr("volume", lookback_bars)
        rel_volume = pl.col("volume") / (_srm_expr("volume", lookback_bars) + 1e-10)

        # ▼▼ [Phase E] rel_volume.rolling_mean(20) → inline map_batches (stable_rolling_mean)
        expressions[f"{p}volume_ma20"] = (
            rel_volume.map_batches(
                lambda s: pl.Series(stable_rolling_mean(s.to_numpy().astype(np.float64), 20)),
                return_dtype=pl.Float64,
            )
            .alias(f"{p}volume_ma20")
        )
        # ▼▼ [Phase E] (pct_change*rel_volume).rolling_mean(10) → inline map_batches
    ...
    def apply_quality_assurance_to_group(
        ...
        for col_name in feature_columns:
            ...
            #   v2: aux 用に別 col_expr_qa (map_batches) を作成 → CSE 効かず 2x 遅延
            #   v3 (現): 同一 for ループ内で safe_col を再利用 → CSE で 1 回計算
            #
            # safe_col (= pl.when(is_infinite).then(None).otherwise(col).fill_nan(None))
            # を stab 側と完全に共有することで、Polars の最適化が CSE で
            # 同じサブグラフを 1 回だけ計算する。process_single_timeframe で
            # collect 後に aux 列の末尾値を読み出して artifact dict に保存し、
            # parquet 書き出し前に drop する (byte-identical を維持)。
            # ─────────────────────────────────────────────────────────────
            ...
            aux_artifact_exprs.append(
                safe_col.is_not_null().alias(f"__qa_valid__{col_name}")
            )
    ...
    def _create_basic_stats_features(self, lazy_frame: pl.LazyFrame, timeframe: str = "M1") -> pl.LazyFrame:
        ...
        for window in self.config.window_sizes["general"]:
            ...
            exprs.append(
                (_srs_expr("close", window, ddof=1) / atr_safe).alias(
                    f"{p}rolling_std_{window}"
                )
            )
            exprs.append(
                (_srv_expr("close", window, ddof=1) / (atr_safe**2)).alias(
