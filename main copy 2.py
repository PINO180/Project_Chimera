# /workspace/main.py
"""
Project Forge 統合実行スクリプト (司令塔) V5.2 (ZMQ データ取得版)

設計図5章に基づき、全てのコンポーネントを統合し、
リアルタイム取引ループを実行する中央制御システム。

[V5.2 修正内容]
1. import MetaTrader5 を削除
2. すべてのMT5データ取得をZMQ経由に変更 (initialize_data_buffer, get_latest_m1_bar)
3. bridge.request_historical_data() と bridge.request_latest_m1_bar() を使用
"""
...
def initialize_data_buffer(
    ...
    bridge: MQL5BridgePublisherV2,
    market_proxy_cache: pd.DataFrame,  # 👈 これを追加
) -> bool:
    """
    [V5.2 修正]
    ZMQ経由で ProjectForgeReceiver.mq5 から全15時間足の履歴データを取得し、
    リアルタイムエンジンの「マルチバッファ」を充填する。
    (矛盾② 解決のため)

    MT5との直接接続（mt5.initialize()）は廃止し、すべてZMQ経由に統一
    """
    logger.info("ZMQ経由で全時間足の履歴データを取得中 (矛盾②解決)...")
    ...
    # 1. M1のルックバックを特定
    tf_name = "M1"
    lookback = max_lookback_required.get(tf_name, 5000) + 5  # M1は多めに取得
    logger.info(
        f"  -> {tf_name} の履歴データを {lookback} 本取得中 (リサンプリング元)..."
    )
    ...
    if df_rates_m1 is None or len(df_rates_m1) == 0:
        logger.error(f"ZMQ {tf_name} 履歴データの取得に失敗しました。")
        return False
    ...
    # エンジンに全履歴データを一括で渡す
    engine.fill_all_buffers(history_data_map, market_proxy_cache)  # 👈 修正

    # M1ループの開始時刻をセット
    global g_last_processed_bar_time
    # [修正] 'timestamp' カラムの最後の行(.iloc[-1])をUnix時刻(.timestamp())に変換
    ...
    # (バッファ充填はループ内で行うため、起動時はチェック不要)
    return True


def get_latest_m1_bar(bridge: MQL5BridgePublisherV2) -> Optional[Dict[str, Any]]:
    """
    [V5.2 修正]
    ZMQ経由で ProjectForgeReceiver.mq5 から「M1」の新しい確定足ができたか確認する。

    MT5との直接接続（mt5.copy_rates_from_pos()）は廃止し、ZMQ経由に統一
    """
    global g_last_processed_bar_time
    ...
    if bar["time"] > g_last_processed_bar_time:
...
def main():
    logger.info("=" * 60)
    logger.info("🚀 Project Forge 統合実行システム V5.2 (ZMQ版) 起動...")
    logger.info("=" * 60)
    ...
    try:
        ...
        # ✨ bridge と g_market_proxy を渡す (ZMQ版)
