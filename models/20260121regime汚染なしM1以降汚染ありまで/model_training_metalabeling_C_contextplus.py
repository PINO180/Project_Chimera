...
class FinalTrainingConfig:
    ...
    lgbm_params_m1: Dict[str, Any] = field(
        default_factory=lambda: {
            ...
            "subsample": 0.8,
            # scale_pos_weight は M1CrossValidator (Script A) で計算済みと仮定
            # 最終モデル訓練用に S6_WEIGHTED_DATASET 全体で再計算するのが望ましいが、
            # ここでは Script A と同じ値を使うか、別途計算する。
...
class FinalAssembler:
    def __init__(self, config: FinalTrainingConfig):
        ...
        # --- [修正] M2の特徴量を「Top 50 (Ace Injection) + m1_pred_proba」に変更 ---
    ...
    def _train_m2_cv_and_write_to_disk(self):
        ...
        for i, (train_dates, val_dates) in enumerate(kfold.split(partitions_to_use)):
            ...
            if n_partitions_train == 0:
                logging.warning(
                    "    -> No train partitions found for M2 CV fold. Skipping."
                )
            else:
                ...
                logging.info(
                    f"    -> M2 CV Fold: Total {n_estimators_total} estimators distributed over {n_partitions_train} partitions."
                )
                logging.info(
            ...
            if n_partitions_train > 0:
                for p_date in tqdm(train_dates, desc=f"  Training M2 Fold {i + 1}"):
                    ...
                    if missing_features:
                        logging.warning(
                            f"Partition {p_date} missing M2 features: {missing_features}. Skipping chunk."
                        )
                        continue
                    ...
                    try:
                        ...
                        logging.error(
                            f"Error fitting M2 model for partition {p_date}: {fit_error}",
                            exc_info=False,
                        )
                        continue
            ...
            for p_date in tqdm(val_dates, desc=f"  Predicting M2 Fold {i + 1}"):
                ...
                if missing_features:
                    logging.warning(
                        f"Prediction partition {p_date} missing M2 features: {missing_features}. Skipping chunk."
                    )
                    continue
                ...
                try:
                    ...
                    predictions = model.predict(
                        X_val
                    )  # objective='binary' なので確率が返る
                except Exception as pred_error:
                    logging.error(
                        f"Error predicting M2 model for partition {p_date}: {pred_error}",
                        exc_info=False,
                    )
    ...
    def _gather_predictions_for_calibration(
        ...
        for p_date in tqdm(dates, desc=f"  Gathering {model_name} calib predictions"):
            # [修正] data.parquet を直接指定
            p_path = (
                base_path
                / f"year={p_date.year}/month={p_date.month}/day={p_date.day}/data.parquet"
            )
            try:
            ...
            # 数値型フィルタリング (前回の修正を維持)
            raw_feature_list = self.features_m2 if is_m2 else self.features_base
            ...
            if not valid_numeric_features:
                logging.warning(
                    f"Calibration partition {p_date} has NO valid numeric features for {model_name}. Skipping chunk."
                )
                continue

            features_to_use = valid_numeric_features
    ...
    def _train_single_model(
        ...
        # n_partitions_train が 0 の場合のエラーは _ensure_model_trained で処理済み
        num_boost_round_per_partition = max(
            1, int(np.ceil(n_estimators_total / n_partitions_train))
        )

        logging.info(
            f"    -> {model_name}: Total {n_estimators_total} estimators distributed over {n_partitions_train} partitions."
        )
        logging.info(
        ...
        for p_date in tqdm(partitions_to_train, desc=f"  Training {model_name}"):
            ...
            dropped = set(raw_feature_list) - set(valid_numeric_features)

            if not valid_numeric_features:
                logging.warning(
                    f"Training partition {p_date} has NO valid numeric features for {model_name}. Skipping chunk."
                )
                continue

            features_to_use = valid_numeric_features

            if dropped and len(dropped_features_log) == 0:
                sample_dropped = list(dropped)[:3]
                logging.warning(
                    f"  [Auto-Fix] Dropped {len(dropped)} non-numeric features for {model_name} to prevent errors (e.g. {sample_dropped}...)"
                )
                dropped_features_log.add("logged")
            ...
            try:
                ...
                logging.error(
                    f"Error fitting {model_name} for partition {p_date}: {fit_error}",
                    exc_info=False,
                )
                continue
