...
# エントリーフィルターより少し緩め（Whipsaw防止のため）
EMERGENCY_EXIT_RULES = {
    "skewness": {"col": "e1a_statistical_skewness_20", "thresh": -2.5, "op": "lt"},
    "kurtosis": {"col": "e1a_statistical_kurtosis_20", "thresh": 20.0, "op": "gt"},
    "phase_stability": {
        "col": "e1e_hilbert_phase_stability_50",
        "thresh": 0.15,
        "op": "lt",
...
def _numba_find_hits(
    ...
    memory_window_us: int,  # ★追加: シグナル記憶期間(マイクロ秒)
    ...
    if n_ticks == 0:
        return out_first_pt_time, out_first_sl_time, out_first_ex_time
    for i in prange(n_bets):
        ...
        for j in range(start_idx, n_ticks):
            ...
            # ★変更: 第4バリア（記憶付き 2/3 Voting）
            if first_ex_found == 0:
                # 1. 閾値を超えたら「現在のTick時刻」を記録して更新
                if ticks_skewness[j] <= exit_skew_thresh:
                    last_skew_time = tick_time
                if ticks_kurtosis[j] >= exit_kurt_thresh:
                    last_kurt_time = tick_time
                if ticks_phase[j] <= exit_phase_thresh:
                    last_phase_time = tick_time

                # 2. 記憶の有効期限チェック (現在時刻から120秒以内なら 1、過ぎていれば 0)
                skew_active = (
                    1
                    if (
                        last_skew_time > 0
                        and (tick_time - last_skew_time) <= memory_window_us
                    )
                    else 0
                )
                kurt_active = (
                    ...
                    if (
                        last_kurt_time > 0
                        and (tick_time - last_kurt_time) <= memory_window_us
                    )
                    else 0
                )
                phase_active = (
                    ...
                    if (
                        last_phase_time > 0
                        and (tick_time - last_phase_time) <= memory_window_us
                    )
                    else 0
                )

                # 3. 2/3 Voting 判定
                breach_count = skew_active + kurt_active + phase_active
                if breach_count >= 2:
...
class ProxyLabelingEngine:
    ...
    def _calculate_labels_for_batch(
        ...
        for timeframe_tuple, group_df in daily_bets_df.group_by("timeframe"):
            ...
            first_pt_time_np, first_sl_time_np, first_ex_time_np = _numba_find_hits(
                bets_t0_np,
                bets_t1_max_np,
                bets_pt_np,
                bets_sl_np,
                ...
                120_000_000,  # ★追加: 120秒(マイクロ秒)のシグナル記憶ウィンドウ
            )
...
"""
        ...
        try:
            ...
            # ★追加: exit_type_str列による内訳集計
            em = df.filter(pl.col("exit_type_str") == "emergency").height
