        ...
        for partition_date in tqdm(
            ...
            try:
                df_labels = pl.read_parquet(
                    p_path_glob, columns=[f"is_trigger_{direction}", label_col]
                )
                df_labels = df_labels.filter(pl.col(f"is_trigger_{direction}") == 1)
            ...
            if len(train_partitions) > 0:
                for partition_date in tqdm(
                    ...
                    try:
                        ...
                        if f"is_trigger_{direction}" in df_chunk.columns:
                            df_chunk = df_chunk.filter(pl.col(f"is_trigger_{direction}") == 1)
