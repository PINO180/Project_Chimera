...
class FinalAssembler:
    def __init__(self, config: FinalTrainingConfig, selected_features_dir: Path = None):
        ...
        if "m1_pred_proba" not in self.features_m2:
            self.features_m2.append("m1_pred_proba")  # ★ M2には必須のため手動追加（_load_featuresで除外されるため）
    ...
    def _load_features(self, feature_path: Path) -> List[str]:
        ...
        exclude_exact = {
            "timestamp",
            # "timeframe",  # ★削除: 特徴量として次へ渡すため除外しない (Cに統一)
            "t1",
            "label",
            "label_long",        # ★追加: V5未来ラベル
            "label_short",       # ★追加: V5未来ラベル
            "uniqueness",
            "uniqueness_long",   # ★追加: 対応uniqueness
            "uniqueness_short",  # ★追加: 対応uniqueness
            "payoff_ratio",
            "payoff_ratio_long",   # ★追加
            "payoff_ratio_short",  # ★追加
            "pt_multiplier",
            "sl_multiplier",
            "direction",
            "exit_type",
            ...
            "close",
            "meta_label",
            "m1_pred_proba",  # ★追加: 後段でjoinされるため特徴量リストから除外
            "is_trigger",
        }
                ...
                oof_df = pl.DataFrame(
                    {
                        ...
                        "timeframe": df_chunk["timeframe"],  # ★追加: C版に統一
    ...
    def _aggregate_m2_oof_predictions(self):
        ...
        try:
            ...
            ).collect(engine="streaming")  # ★修正: streaming=True → engine="streaming" (プロンプト⑬ 修正②)
            m2_oof_df.sort(
                ["timestamp", "timeframe"]  # ★修正: timeframe追加 (C版に統一)
            ).write_parquet(
                self.config.m2_oof_predictions, compression="zstd"
            )
            logging.info(
