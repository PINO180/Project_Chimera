...
# [フェーズ3: 最終ラベリングスクリプト - V5 MFE/MAE 回帰ラベリング仕様]
#
# ==========================================================================
# 【1ヶ月だけ回す (2021年7月)】 ※ /workspace から実行 (スクリプトは models/ 配下)
#   --year-month は YYYY/M 形式 (スラッシュ)
#   python models/create_proxy_labels_polars_patch_regime_Universal_Brain_V5.py --year-month 2021/7
#
#   再実行で既存出力を無視して上書きしたい場合は --no-resume を付ける:
#   python models/create_proxy_labels_polars_patch_regime_Universal_Brain_V5.py --year-month 2021/7 --no-resume
# ==========================================================================
...
# ★スプレッドコスト（XAUUSD: 1pip=0.01ドル → 50pips=0.50ドル）
#   [A-2 2026-07-14] 回帰ラベルに控除して使用: mfe_usd/mae_usd = 生の変位 − SPREAD。
#   旧トリプルバリアの「SPREADでバリア保守化」を回帰版に翻訳したもの。
#   これにより脳は「スプレッド払った後に取れる順行/逆行」を直接学習する。
#   ラベル時点でロット未定のため 1ロット相当の固定額近似 (旧設計と同じ)。
SPREAD = 0.50

# ロング用ルール
RULE_LONG = {
    "pt_mult": 1.0,  # [残骸/未使用] 旧固定バリア倍率
    "sl_mult": 0.5,  # [残骸/未使用]
    "td": "30m",  # TO窓 (MFE/MAE 観測窓)。★TO30 スプレッド入り学習版★
}
...
def _numba_mfe_mae(
    ...
    """
    [MFE/MAE ラベル] エントリー後 (t0 + offset, window_end] の窓で mid_price の
    最高値・最安値を求める。triple-barrier (_numba_find_hits_dual) とは完全独立:
      - 早期退出せず窓を最後まで走査する (最大/最小はバリア到達で止めない)。
      - 走査開始は barrier と同一の entry 時刻 (t0 + entry_offset)、window_end 含む。
        → triple-barrier の TD (= t1_max) と同一窓なので TO=TD で MFE/MAE を測る。
    tick は単一点 (high=low=close=mid_price) のため ticks_price に mid を渡す。
    返り値 (out_max, out_min): 窓内に tick が無い bet は NaN。
    MFE/MAE(ドル)・方向は呼び出し側の polars で close/atr から算出する。
    """
    n_bets = len(bets_t0)
    n_ticks = len(ticks_ts)
...
class ProxyLabelingEngine:
    ...
    def _load_all_price_data(self) -> Dict[str, Any]:
        ...
        for tf in TARGET_TIMEFRAMES:
            ...
            session_ratio_name = f"session_atr_ratio_{tf}"
            ...
            atr_lf = (
                ...
                .with_columns(
                    [
                        ...
                        # [SESSION-RATIO] UTC hour → セッション名 (計算系UTC0。BT _session と同一写像)
                        pl.col("timestamp")
                        .dt.hour()
                        .replace_strict(SESSION_BY_UTC_HOUR, return_dtype=pl.Utf8)
                        .alias("_session"),
                    ]
                )
                # [SESSION-RATIO] session_atr_ratio = 今のATR ÷ 同一セッションの過去 N 日 ATR平均。
                #   .over("_session") で「同じ時間帯」だけを分母にし、時間帯混合の歪みを排除。
                #   window は時刻ベース(N日)。closed="right" で現バーを含む(既存 atr_ratio と整合)。
                .with_columns(
                    [
                        (
                            pl.col(target_atr_name)
                            / (
                                ...
                                .rolling_mean_by(
                                    "timestamp",
                                    window_size=f"{SESSION_BASELINE_DAYS}d",
                                    min_samples=1,
                                )
                                .over("_session")
                                + 1e-10
                            )
                        ).alias(session_ratio_name)
                    ]
                )
                .select(
                    ["timestamp", target_atr_name, atr_ratio_name, session_ratio_name]
                )
            )
    ...
    def run(self):
        ...
        logging.info(
            "Gate: 撤廃 (全バー is_trigger=1)。atr_ratio / session_atr_ratio は特徴量として保持。"
        )
        logging.info(
            f"Label: MFE/MAE regression (ATR建て). TD/TO window Long={RULE_LONG['td']} / Short={RULE_SHORT['td']}"
        )
        ...
        try:
            ...
            for month_row in tqdm(
                ...
                try:
                    # [PERF-FIX 2026-07-08] hive パーティション列 (year/month) で先に枝刈り
                    # してから timestamp フィルタする。旧実装は timestamp (非パーティション列)
                    # だけで filter しており、述語プッシュダウンが効かず全 hive ファイルを
                    # 開いてから行 filter していた。month_end は lookahead_margin ぶん翌月に
                    # 食い込むため、当月と翌月の (year, month) を許可集合に含める。
                    # timestamp フィルタ (is_between) は端数落としとして後段に薄く残す
                    # (パーティション枝刈り後なので開くファイル数が激減、結果行は不変)。
                ...
                #   旧実装は日次ループ内で unified_*_lf (全期間~5年) に対し
                #   .filter(timestamp.dt.date()==current_date).collect() を hive/file の
                #   2 回、営業日ぶん繰り返していた = O(days × 全期間scan) が支配項。
                #   対処: 既に月次 materialize 済みの _monthly_hive_df/_monthly_file_df を
                #   日付キーで partition_by し、日次ループは get() で O(1) 参照する。
                #   正当性: 日次 raw_bets_df は unified_*_lf 由来、_monthly_*_df も同一
                #   unified_*_lf を is_between で切り出したもの。当月日付では行集合が一致。
                _bets_by_day: Dict[dt.date, pl.DataFrame] = {}
                _monthly_bets_for_days = [
                    _df
                    for _df in (_monthly_hive_df, _monthly_file_df)
                ...
                for row in days_in_month.iter_rows(named=True):
                    ...
                    # (旧: unified_*_lf.filter(date==current_date).collect() ×2)
    ...
    def _calculate_labels_for_batch(
        ...
        for timeframe_tuple, group_df in daily_bets_df.group_by("timeframe"):
            ...
            session_ratio_col_name = f"session_atr_ratio_{timeframe}"  # [SESSION-RATIO]
            ...
            #   - SPREAD 項は [A-2] で回帰版に復活: mfe_usd/mae_usd = 生変位 − SPREAD。
            _entry_offset = pl.duration(seconds=self.ACTION_HORIZON_SEC)

            # (1) ATR / ATR_ratio / session_atr_ratio は トリガー時刻 L で結合 (バーの値、本番整合済み)
            ...
            # atr_value・atr_ratio・session_atr_ratio ともに事前計算済みカラムをそのまま使用
            bets_df_with_atr = bets_with_price_df.with_columns(
                ...
                pl.col(session_ratio_col_name).alias("session_atr_ratio"),
            )

            # --- [GATE撤廃] ATR Ratio ゲートを廃止し、全バーを is_trigger=1 とする。 ---
            #   旧: is_trigger = (atr_ratio >= ATR_RATIO_THRESHOLD) で低ボラを学習から除外していた。
            #   撤廃理由: 「取れる局面か」の事前判断を人間が間引くのをやめ、脳に委ねる(Two-Brain思想)。
            #   atr_ratio / session_atr_ratio は特徴量として脳に渡り、脳自身が可否を学習する。
            #   ※ 全バーがトリガーになるため Numba バリア計算量が増える(低ボラ分も計算)。
            bets_df_all = bets_df_with_atr.with_columns(
                pl.lit(1, dtype=pl.Int8).alias("is_trigger")
            )
            ...
            # [TRIPLE-BARRIER撤去 2026-07-12] PT/SL バリア価格の計算を廃止。
            #   MFE/MAE 回帰では固定ゴール(PT/SL)を置かないため、必要なのは
            #   TO 窓の終端 = t1_max_long/short (= entry + TD) のみ。
            #   バリア基準価格はエントリー地点(close)に戻り、その先どこまで動くかを測る。
            ...
            # [MFE/MAE ラベル] エントリー後窓 (entry, t1_max_long] の mid 最高値/最安値を算出。
            #   窓終端 = t1_max_long (= entry + TD = entry + TO)。tick は単一点なので high=mid。
            out_mfe_max_mid, out_mae_min_mid = _numba_mfe_mae(
                bets_t0_np,
                bets_t1_max_l_np,
                ticks_ts_np,
                ticks_high_np,  # = mid_price (L912-914: tick は high=low=close=mid)
                np.int64(self.ACTION_HORIZON_SEC * 1_000_000),
            )

            # 計算済みトリガー行の DataFrame 作成
            calculated_df = (
                bets_df.with_columns(
                    pl.Series("_mfe_max_mid", out_mfe_max_mid),
                    pl.Series("_mae_min_mid", out_mae_min_mid),
                )
                .with_columns(
                    # [MFE/MAE ラベル] エントリー価格(close)基準の順行/逆行量。
                    #   mfe = 窓内mid最高値 − close (＋側最大順行), mae = close − 窓内mid最安値 (−側)
                    #   窓内tick無しは _mfe_max_mid が NaN → mfe_usd も NaN。学習側で欠損を
                    #   落とせるよう NaN は null に統一 (polars では NaN≠null、is_not_null で
                    #   NaN が捕まらないため)。ATR建ては atr_value で正規化、atr<=0 は null。
                    # [A-2] スプレッド控除: 生の変位 − SPREAD。小さい変位は負になり
                    #   「スプレッド負けする動き」を脳がそのまま学習する。NaN−0.5=NaN → null。
                    mfe_usd=(pl.col("_mfe_max_mid") - pl.col("close") - SPREAD)
                    .cast(pl.Float32)
                    .fill_nan(None),
                    mae_usd=(pl.col("close") - pl.col("_mae_min_mid") - SPREAD)
                    .cast(pl.Float32)
                    .fill_nan(None),
                )
                .with_columns(
                    mfe_atr=pl.when(pl.col("atr_value") > 0)
                    .then(pl.col("mfe_usd") / pl.col("atr_value"))
                    .otherwise(None)
                    .cast(pl.Float32),
                    mae_atr=pl.when(pl.col("atr_value") > 0)
                    .then(pl.col("mae_usd") / pl.col("atr_value"))
                    .otherwise(None)
                    .cast(pl.Float32),
                    # 方向ラベル: MFE(上) と MAE(下) の大きい方 (1=上優勢, 0=下優勢)
                    mfe_direction=pl.when(pl.col("mfe_usd") >= pl.col("mae_usd"))
                    .then(pl.lit(1, dtype=pl.Int8))
                    .otherwise(pl.lit(0, dtype=pl.Int8)),
                )
                .with_columns(
                    # 優勢度ラベル: 符号付き (mfe_atr − mae_atr)。正=上優勢の強度, 負=下優勢
                    mfe_dominance_atr=(pl.col("mfe_atr") - pl.col("mae_atr")).cast(
                        pl.Float32
                    ),
                )
                .select(
    ...
    def _update_label_counts_dual(self, df: pl.DataFrame):
        # [TRIPLE-BARRIER撤去] label_long/short は無くなったため、MFE/MAE の集計に置換。
        #   mfe_direction(1=上優勢/0=下優勢)の件数と、mfe_atr/mae_atr の和・件数を累積。
        if "mfe_direction" not in df.columns:
            return
        sub = df.filter(pl.col("is_trigger") == 1) if "is_trigger" in df.columns else df
        sub = sub.filter(pl.col("mfe_atr").is_not_null())
        if sub.is_empty():
            return
        if not hasattr(self, "_mfe_stats"):
            self._mfe_stats = {
                "n": 0, "up": 0, "down": 0,
        ...
        s = self._mfe_stats
        s["n"] += sub.height
        s["up"] += int((sub["mfe_direction"] == 1).sum())
        s["down"] += int((sub["mfe_direction"] == 0).sum())
        s["mfe_sum"] += float(sub["mfe_atr"].sum())
        s["mae_sum"] += float(sub["mae_atr"].sum())
    ...
    def _log_final_summary(self):
        # [TRIPLE-BARRIER撤去] MFE/MAE 回帰ラベルのサマリー。
        s = getattr(self, "_mfe_stats", None)
        if not s or s["n"] == 0:
            logging.warning("No samples were processed.")
            return
        n = s["n"]
        up_pct = s["up"] / n * 100
        mfe_mean = s["mfe_sum"] / n
        mae_mean = s["mae_sum"] / n
        summary = (
            "\n" + "=" * 60 + "\n"
            f"### V5 MFE/MAE Regression Labeling COMPLETED ###\n"
            f"Output Dir: {self.config.output_dir}\n"
            f"  - Total Labeled Triggers (is_trigger=1): {n}\n"
            f"  - Direction: up-dominant={s['up']} ({up_pct:.2f}%) / "
            f"down-dominant={s['down']} ({100 - up_pct:.2f}%)\n"
            f"  - Mean MFE (ATR): {mfe_mean:.4f} / Mean MAE (ATR): {mae_mean:.4f} "
            f"(diff {mfe_mean - mae_mean:+.4f})\n"
            + "=" * 60
        )
    ...
    def _generate_report(self):
        ...
        try:
            ...
            total = len(df)
            up_dom = df.filter(pl.col("mfe_direction") == 1).height
            down_dom = df.filter(pl.col("mfe_direction") == 0).height

            # --- MFE/MAE(ATR建て)の分布統計 ---
            def _q(col, q):
                return float(df[col].quantile(q)) if total > 0 else 0.0

            mfe_mean = float(df["mfe_atr"].mean() or 0.0)
            mae_mean = float(df["mae_atr"].mean() or 0.0)
            mfe_med = float(df["mfe_atr"].median() or 0.0)
            mae_med = float(df["mae_atr"].median() or 0.0)
            ...
            report_content = f"""
# Proxy Labeling Engine - Execution Report (V5 MFE/MAE Regression) 📈
...
| **Gate** | 撤廃 (全バー is_trigger=1) |
| **Label** | MFE/MAE regression (ATR建て), TD/TO window = `{RULE_LONG["td"]}` |

### 2. Direction (derived: MFE vs MAE)
| Metric | Count | Share |
|:---|---:|---:|
| **Total Setups** | `{total:,}` | - |
| **Up-dominant (MFE>=MAE)** | `{up_dom:,}` | `{up_dom / total:.2%}` |
| **Down-dominant (MFE<MAE)** | `{down_dom:,}` | `{down_dom / total:.2%}` |

### 3. MFE/MAE Distribution (ATR)
| Metric | Mean | Median | p90 | p99 |
|:---|---:|---:|---:|---:|
| **MFE (up)** | `{mfe_mean:.3f}` | `{mfe_med:.3f}` | `{_q("mfe_atr", 0.9):.3f}` | `{_q("mfe_atr", 0.99):.3f}` |
| **MAE (down)** | `{mae_mean:.3f}` | `{mae_med:.3f}` | `{_q("mae_atr", 0.9):.3f}` | `{_q("mae_atr", 0.99):.3f}` |
