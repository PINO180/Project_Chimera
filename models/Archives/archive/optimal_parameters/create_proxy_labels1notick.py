...
class ProxyLabelingEngine:
    ...
    def __init__(self, config: ProxyLabelConfig):
        ...
        # 5. 結合 (e.g., "temp_labeled_subset_partitioned_15_10_05")
    ...
    def _validate_config(self):
        ...
        duration_match = re.match(r"(\d+)m?$", cfg.target_duration)
        if not duration_match or not duration_match.group(1).isdigit():
            raise ValueError(
                f"Invalid target_duration format: {cfg.target_duration}. Expected format like '300m' or '60'."
            )
        # Ensure 'm' is present for consistency internally
        if cfg.target_duration.isdigit():
    ...
    def run(self):
        ...
        try:
            ...
            duration_match = re.match(
                r"(\d+)", cfg.target_duration
            )  # Already validated in __init__
            max_lookahead_minutes = float(duration_match.group(1))
    ...
    def _build_unified_lazyframe(self, feature_paths: List[Path]) -> pl.LazyFrame:
        ...
        for path in feature_paths:
            ...
            if path.is_dir():
                ...
                # V6のように、スキャンするベースパスをフィルター条件に応じて動的に決定する
                scan_base_path = path
                ...
                # 'all' モードの場合は、`scan_base_path` は元の `path` のまま (全期間スキャン)

                # V6のglobパターンを、動的に決定したパスに適用する
        ...
        try:
            # V7で修正された .rechunk() のない、正しい concat
    ...
    def _calculate_labels_for_batch(
        ...
        for timeframe_tuple, group_df in daily_bets_df.group_by("timeframe"):
            ...
            duration_match = re.match(r"(\d+)", cfg.target_duration)
            if not duration_match:
                continue  # Should be validated earlier
            target_duration_minutes = float(duration_match.group(1))
    ...
    def _generate_report(self):
        ...
        # 4. 結合 (e.g., "execution_report_15_10_05.md")
...
def _get_interactive_config() -> ProxyLabelConfig:
    ...
    duration = get_input(
        "\n2. Target lookahead duration (e.g., '300m', '60')",
        "300m",
        str,
        duration_valid,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="300m",
        help="Target lookahead duration (e.g., '300m' or '60').",
    )
    parser.add_argument(
        "--no-resume",
        action="store_true",
