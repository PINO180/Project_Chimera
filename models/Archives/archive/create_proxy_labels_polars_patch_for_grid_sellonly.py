...
class ProxyLabelingEngine:
    ...
    def _calculate_labels_for_batch(
        ...
        for timeframe_tuple, group_df in daily_bets_df.group_by("timeframe"):
            ...
            # 2. バリアと時間制限 (t1_max) を計算 (この時点では共通)
            ...
            # --- ▼▼▼ [フェーズ2 改造] ATRレジームフィルタリング ▼▼▼ ---
            # (Buy/Sell共通のフィルタリング)
            if cfg.atr_min is not None:
                bets_df_with_atr = bets_df_with_atr.filter(
                    pl.col("atr_value") >= cfg.atr_min
                )
            if cfg.atr_max is not None:
                # (最大値は < (exclusive) でフィルタリング)
                bets_df_with_atr = bets_df_with_atr.filter(
                    pl.col("atr_value") < cfg.atr_max
                )
            # --- ▲▲▲ 改造ここまで ▲▲▲ ---

            if bets_df_with_atr.is_empty():
                continue

            # --- ▼▼▼ [Buy/Sell 修正] Buy/Sell両方のDFを作成 ▼▼▼ ---

            # 3. Buy (Long) のバリアを計算
            bets_df_long = bets_df_with_atr.with_columns(
                (
                    pl.col("close") + pl.col("atr_value") * cfg.profit_take_multiplier
                ).alias("pt_barrier"),
                (
                    pl.col("close") - pl.col("atr_value") * cfg.stop_loss_multiplier
                ).alias("sl_barrier"),
                pl.lit(1, dtype=pl.Int8).alias("direction"),  # ★ Buy
            )

            # 4. Sell (Short) のバリアを計算
            bets_df_short = bets_df_with_atr.with_columns(
                (
                    pl.col("close") - pl.col("atr_value") * cfg.profit_take_multiplier
                ).alias("pt_barrier"),  # ★ 反転
                (
            ...
            # 6. ベット情報を Numpy 配列に変換 (Numbaに渡すため)
            try:
            ...
            # 8. Numba の結果 (Numpy配列) を Polars に戻す
            final_group_df = (
...
if __name__ == "__main__":
    ...
    # --- ▼▼▼ [フェーズ2 改造] argparse に引数を追加 ▼▼▼ ---
    parser.add_argument(
        "--atr-min",
        type=float,
        default=None,
        help="[Phase 2] Minimum atr_value to process (inclusive).",
    )
    parser.add_argument(
        "--atr-max",
        type=float,
        default=None,
        help="[Phase 2] Maximum atr_value to process (exclusive).",
    )
    # --- ▲▲▲ 改造ここまで ▲▲▲ ---
