...
class MetaLabelingConfig:
    ...
    feature_list_path: Path = S3_FEATURES_FOR_TRAINING  # ★ 変更: Full特徴量リストのパス
...
class MetaLabelGenerator:
    ...
    # ★ 変更: JSON (Top 50) ではなく TXT (Full) を読み込むメソッドにリネーム・変更
    ...
    def run(self) -> None:
        ...
        # 抽出するカラムの定義: 基本情報 + 全特徴量 (304個) + [必須] uniqueness, payoff_ratio
        ...
        columns_to_select = [
            ...
            "uniqueness",
            "payoff_ratio",
        ] + self.features  # ★ 変更: self.top_50_features から self.features (304個) へ
