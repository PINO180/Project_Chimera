...
class ProxyLabelingEngine:
    ...
    def run(self):
        ...
        try:
            ...
            for row in tqdm(
                ...
                # --- [Report Fix] Use Source Tracking and Canonicalization ---
                # Retrieve data, ensuring source_path is available if possible
                ...
                # Verify source_path existence (it might be missing if scan failed to add it)
                # We add a dummy if missing to prevent errors, though the new build_unified ensures it.
                if (
                    "source_path" not in daily_bets_hive_df.columns
                    and not daily_bets_hive_df.is_empty()
                ):
                    daily_bets_hive_df = daily_bets_hive_df.with_columns(
                        pl.lit("unknown_hive").alias("source_path")
                    )
                if (
                    "source_path" not in daily_bets_file_df.columns
                    and not daily_bets_file_df.is_empty()
                ):
                    daily_bets_file_df = daily_bets_file_df.with_columns(
                        pl.lit("unknown_file").alias("source_path")
                    )

                # --- ★★★ [修正] 「決定論的」重複排除 (Watermark Strategy) ★★★ ---
                # 1. 結合 (Diagonal)
                # 2. タイムスタンプ正規化 (Int64化 + Microsecond Truncate)
                # 3. Sort (Timestamp + SourcePath) で順序固定
                # 4. Unique (Keep First)

                raw_combined_df = pl.concat(
                    [daily_bets_hive_df, daily_bets_file_df], how="diagonal"
                )
                raw_count = raw_combined_df.height

                daily_bets_df = (
                    raw_combined_df.with_columns(
                        [
                            # [Canonicalization] Eliminate float/int96 jitter by strict truncation to microseconds
                            pl.col("timestamp")
                            .cast(pl.Datetime("us"))
                            .dt.truncate("1us")
                            .alias("ts_canonical")
                    ...
                    # [Deterministic Dedup] Sort by canonical time AND source path ensures the same row always wins
                    .sort(by=["ts_canonical", "source_path"])
                    .unique(
                        subset=["ts_canonical", "timeframe"],
                        keep="first",
                        maintain_order=True,
                    )
                    .drop(["ts_canonical", "source_path"])  # Cleanup auxiliary columns
                    .sort("timestamp")
                )
                # ---------------------------------------------------------------
                ...
                if raw_count != dedup_count:
                    logging.info(  # Changed to INFO as this is now expected behavior for cleaning
                        f"Clean Room Dedup Applied: Raw {raw_count} -> Final {dedup_count}. "
                        f"Eliminated {raw_count - dedup_count} 'undead' rows using Watermark strategy."
                    )
                else:
                    logging.debug(
    ...
    # =========================================================================
    def _generate_clean_manifest(self, source_dir: Path) -> List[str]:
        """
        [Report Fix] Generates a deterministic file list (Manifest) for ingestion.
        - Resolves symlinks to absolute paths to prevent circular loops.
        - Excludes hidden files (starting with . or _) and known backup directories.
        - Returns a sorted list to ensure reproducibility.
        """
        clean_files = set()
        try:
            # Recursively find all parquet files
            for f in source_dir.rglob("*.parquet"):
                # 1. Resolve to absolute path (fix symlinks)
                try:
                    resolved = f.resolve(strict=True)
                except FileNotFoundError:
                    continue

                # 2. Exclude hidden files and standard metadata files
                if resolved.name.startswith((".", "_")):
                    continue

                # 3. Exclude hidden directories (e.g. .ipynb_checkpoints)
                # Check if any part of the path is hidden
                path_str = str(resolved)
                if "/." in path_str or "\\." in path_str:
                    continue

                clean_files.add(path_str)
        except Exception as e:
            logging.warning(f"Error during manifest generation for {source_dir}: {e}")
            return []

        return sorted(list(clean_files))
    ...
    def _build_unified_lazyframe(
        ...
        """
        [修正版 Report Fix 8.1 & 8.2] S5の特徴量データを、Hiveパーティション(Lazy)と
        単一ファイル(Lazy)に分離する。

        ★改善点:
        - Glob文字列ではなく「マニフェスト(ファイルリスト)」を使用し、隠しファイル混入を防ぐ。
        - include_file_paths="source_path" を有効化し、データ出自を追跡可能にする。
        """
        all_lazy_frames_hive = []
        all_lazy_frames_file = []

        # [Report Fix 8.1] Regex for extracting timeframe
        timeframe_pattern = re.compile(r"features_e\d+[a-z]?_([a-zA-Z0-9]+)")
        cfg = self.config

        logging.info(
            "   -> Separating Hive partitions vs. single files for S5 (Clean Room Ingestion)..."
        )
        for path in feature_paths:
            ...
            if path.is_dir():
                scan_base_path = path
                # Apply filter to scan path if applicable
                ...
                if scan_base_path.exists():
                    # [Report Fix] Use Manifest generation instead of glob string
                    manifest = self._generate_clean_manifest(scan_base_path)
                    if manifest:
                        try:
                            # include_file_paths is crucial for deterministic dedup later
                            lf = pl.scan_parquet(
                                manifest, include_file_paths="source_path"
                            )
                        except Exception as e:
                            logging.warning(
                                f"Failed to scan manifest from {scan_base_path}: {e}"
                            )
                            continue
                ...
                try:
                    # [Report Fix] Trace source path
                    lf_full = pl.scan_parquet(
                        str(path), include_file_paths="source_path"
                    )
        ...
        if all_lazy_frames_hive:
            try:
                # Add "source_path" to schema consistency if needed, but diagonal concat handles it
                unified_hive_lf = pl.concat(all_lazy_frames_hive, how="diagonal")
                logging.info(
                    f"   -> Prepared S5 Hive LazyFrame ({len(all_lazy_frames_hive)} sources) with Source Tracking."
                )
        ...
        if all_lazy_frames_file:
            try:
                ...
                logging.info(
                    f"   -> Prepared S5 non-tick LazyFrame ({len(all_lazy_frames_file)} sources) with Source Tracking."
                )
            except pl.exceptions.ComputeError as e:
                if "cannot concat empty list" in str(e).lower():
                    logging.warning(
