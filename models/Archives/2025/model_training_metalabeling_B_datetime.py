...
class MetaLabelGenerator:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # [重要修正] Lookahead Bias (先読み) 防止と結合キーの作成
            # 1. タイムゾーンをUTCに統一して日付(Date型)を取得
            # 2. 日付を「1日進める (days=1)」。
            #    理由: Contextデータの "10月1日" の値は、10月1日のClose後に確定するため、
            #    実際にトレードで使えるのは "10月2日" 以降。
            #    したがって、10月1日のデータに "10月2日" という結合キーを持たせる。
            context_df = context_df.with_columns(
                (
                    pl.col("timestamp").dt.replace_time_zone("UTC").dt.date()
                    + pl.duration(days=1)
                ).alias("join_date_key")
            ).select(
                # 元のtimestampは分足側と重複するため除外し、join_date_keyを残す
                pl.all().exclude("timestamp")
            )

            logging.info(
                f"  -> Context features loaded and shifted by 1 day (Lag) for correct alignment."
            )
        ...
        for partition_date in tqdm(self.partitions, desc="Generating Meta-Labels"):
            ...
            try:
                ...
                # 5. [修正] join_asof を廃止し、Left Join に変更
                #    理由: join_asof は型不一致(Datetime vs Date)により全行定数化するバグがあったため。
                #    Date型同士の厳密な結合に変更する。

                context_joined_lf = (
                    merged_chunk_lf.with_columns(
                        # 分足データから「当日」の日付を取得して結合キーにする
                        pl.col("timestamp").dt.date().alias("join_date_key")
                    )
                    .join(
                        context_df.lazy(),  # 1日ずらし済みのContextデータ
                        on="join_date_key",  # 日付同士でマッチング
                        how="left",  # マッチしない日はNullになる(安全)
                    )
                    .drop("join_date_key")
                )  # 結合用キーは削除
