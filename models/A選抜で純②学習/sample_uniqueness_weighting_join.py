...
def main():
    ...
    for i, path in enumerate(partitions):
        ...
        try:
            ...
            # --- 修正後 ---
            # 入れ子メタラベリング: A幾何・B幾何それぞれの uniqueness を計算する。
            # uniqueness = 1/concurrency（同時ラベル数の逆数、AFML 平均一意性の素）。
            def _uniqueness(conc_col: str, out_col: str):
                return (
                    pl.when(
                        pl.col(conc_col).is_not_null() & (pl.col(conc_col) > 0)
                    )
                    .then(1.0 / pl.col(conc_col))
                    .otherwise(0.0)
                    .alias(out_col)
                )
            ...
            final_lf = (
                ...
                .with_columns(
                    [
                        _uniqueness("concurrency_A_long", "uniqueness_A_long"),
                        _uniqueness("concurrency_A_short", "uniqueness_A_short"),
                        _uniqueness("concurrency_B_long", "uniqueness_B_long"),
                        _uniqueness("concurrency_B_short", "uniqueness_B_short"),
                    ]
                )
                # 中間カラムである concurrency(4系統)を削除
                .select(
                    pl.all().exclude(
                        [
                            "concurrency_A_long",
