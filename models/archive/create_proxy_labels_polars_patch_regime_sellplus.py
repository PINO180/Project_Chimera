...
def _numba_find_hits(
    ...
    bets_direction: np.ndarray,  # ★★★ [修正] 方向 (1 or -1) を追加 ★★★
    ticks_ts: np.ndarray,
    ...
    for i in prange(n_bets):
        ...
        if direction == 1:  # Long (Buy)
            ...
            for j in range(start_idx, n_ticks):
                ...
                # (ロジック反転)
                tick_low = ticks_low[j]
                if first_pt_found == 0 and tick_low <= pt:  # PTはLowヒット
                    first_pt_found = tick_time

                tick_high = ticks_high[j]
                if first_sl_found == 0 and tick_high >= sl:  # SLはHighヒット
                    first_sl_found = tick_time
...
class ProxyLabelingEngine:
    ...
    def _calculate_labels_for_batch(
        ...
        for timeframe_tuple, group_df in daily_bets_df.group_by("timeframe"):
            ...
            # --- ▼▼▼ [Buy/Sell 修正] Buy/Sell両方のDFを作成 ▼▼▼ ---

            # 4. R4ルールを適用 (Buy)
            bets_df_long = bets_df_filtered.with_columns(
                ...
                pl.lit(1, dtype=pl.Int8).alias("direction"),  # ★ Buy
            )

            # 5. R4ルールを適用 (Sell)
            bets_df_short = bets_df_filtered.with_columns(
                pl.col("timestamp").alias("t0"),
                (
                    pl.col("close") - pl.col("atr_value") * rule_pt  # ★ 反転
                ).alias("pt_barrier"),
                (
                    pl.col("close") + pl.col("atr_value") * rule_sl  # ★ 反転
                ).alias("sl_barrier"),
                t1_max_expr.alias("t1_max"),
                pl.lit(-1, dtype=pl.Int8).alias("direction"),  # ★ Sell
            )

            # 6. BuyとSellを縦に結合
            bets_df = pl.concat([bets_df_long, bets_df_short]).select(
                # --- ▲▲▲ [Buy/Sell 修正] ここまで ▲▲▲ ---
                pl.col("t0"),
                pl.col("pt_barrier"),
                pl.col("sl_barrier"),
                ...
                pl.col("atr_value"),
                pl.col("close"),
                pl.col("direction"),  # ★ [修正] direction を選択
            ...
            try:
                ...
                bets_direction_np = bets_df[
                    "direction"
                ].to_numpy()  # ★ [修正] direction をNumpyに
            except Exception as e:
                logging.error(
            ...
            first_pt_time_np, first_sl_time_np = _numba_find_hits(
                ...
                bets_pt_np,
                bets_sl_np,
                bets_direction_np,  # ★ [修正] direction を渡す
                ticks_ts_np,
                ticks_high_np,
                ticks_low_np,
            )

            # 9. Numba の結果を Polars に戻す
            final_group_df = (
            ...
            labeled_group = final_group_df.with_columns(
                ...
                # --- ▲▲▲ 改造ここまで ▲▲▲ ---
                # direction は既に bets_df から引き継がれている
            ).rename({"t0": "timestamp"})
    ...
    def _collect_report_data(self, df: pl.DataFrame, current_date: dt.date):
        ...
        try:
            # ★★★ [Buy/Sell 修正] direction をレポート用に収集 ★★★
            required_cols = ["timestamp", "t1", "timeframe", "label", "direction"]
            ...
            report_df = df.with_columns(
                ...
                ["timeframe", "label", "event_duration_seconds", "date", "direction"]
            )  # ★ [修正] direction を追加
...
| **Labeling Strategy** | `ATR Regime V4 (R4 Only, Buy/Sell)` |
...
"""
            ...
            # --- ▼▼▼ [Buy/Sell 修正] Direction別の集計テーブルを追加 ▼▼▼ ---
            dir_breakdown = (
                ...
                .sort("direction", descending=True)  # Buy (1) が先, Sell (-1) が後
            )
            dir_table = "| Direction | Bet Count | PT (%) | SL (%) | Timeout (%) |\n"
            dir_table += "|:---|---:|---:|---:|---:|\n"
            for row in dir_breakdown.to_dicts():
                dir_name = "Buy (1)" if row["direction"] == 1 else "Sell (-1)"
                total_bets = row["bets"]
                pt_pct = row["pt"] / total_bets if total_bets > 0 else 0
                sl_pct = row["sl"] / total_bets if total_bets > 0 else 0
                to_pct = row["to"] / total_bets if total_bets > 0 else 0
                dir_table += f"| **{dir_name}** | `{total_bets:,}` | `{row['pt']:,}` (`{pt_pct:.2%}`) | `{row['sl']:,}` (`{sl_pct:.2%}`) | `{row['to']:,}` (`{to_pct:.2%}`) |\n"
            # --- ▲▲▲ [Buy/Sell 修正] ここまで ▲▲▲ ---
...
### 3. Direction Breakdown (Buy vs Sell) 葡
This table shows the performance of the (PT=1.0, SL=5.0) rule for Buy and Sell signals.

{dir_table.strip()}

---

### 4. Timeframe Breakdown (R4 Only) 葡
...
### 5. Event Duration Analysis (R4 Only) 竢ｳ
...
### 6. Top 10 Busiest Days (R4 Only) 欄ｸThis table lists the days with the highest number of labeled samples within the filtered period.
