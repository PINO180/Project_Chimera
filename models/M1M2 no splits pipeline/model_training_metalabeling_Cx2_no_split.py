...
class FinalAssembler:
    def __init__(self, config: FinalTrainingConfig):
        ...
        # [(い) 直交分割なし版] M1・M2 とも全特徴量を読み込む
        # (旧: S3_SELECTED_FEATURES_ORTHOGONAL_DIR / m1_{dir}, m2_{dir}_features.txt)
        # M2 は「全特徴量 + m1_pred_proba」。m1_pred_proba は _load_features で
        # 除外されるため、従来通り下で手動追加する（メタラベル層の必須入力）。
        m1_feature_path = S3_FEATURES_FOR_TRAINING_V5
        m2_feature_path = S3_FEATURES_FOR_TRAINING_V5
