...
# ============================================================================
# [新設計 2026-07-16] 残余ドリフト収穫型メタラベリング — 変更2: |d| ゲート + sign(d) 片側化
# ----------------------------------------------------------------------------
#   d = (P(L+180) − P(L)) / ATR = 直近3分の既発生運動 (符号付き)。
#   D_GATE_MIN_ATR : |d| >= この値 のバーだけをトリガーにする (①をゲートへ外部化)。
#       0.3 = |d| 中央値 ≈ 継続ドリフトが濃いレジームの下限狙い (理論§6.2)。
#       0.0 にすると |d| ゲート無効 (atr_ratio ゲートのみ)。
#   side = sign(d) を方向源とし、d>0 のバーは long ラベルのみ / d<0 は short ラベルのみ
#   を有効化 (逆側は null)。→ M1-long は d>0 母集団、M1-short は d<0 母集団を学習
#   (2モデル・side masking。金の逆レバレッジ非対称を別モデルが捉える下地)。
# ============================================================================
D_GATE_MIN_ATR = 0.3
...
class ProxyLabelingEngine:
    ...
    def __init__(self, config: ProxyLabelConfig):
        ...
        # [新設計・観測] side 別のゲート後サンプル数と PT-first 数
        self._side_obs: Dict[str, int] = {
            "long_n": 0, "long_pt": 0, "short_n": 0, "short_pt": 0
        }
        ...
        try:
            ...
            for month_row in tqdm(
                ...
                for row in days_in_month.iter_rows(named=True):
                    ...
                    # 日次メモリ解放
                    #   [修正] 元スクリプトにあった `del daily_bets_hive_df,
                    #   daily_bets_file_df` は未定義変数への del で実行時 NameError の
                    #   潜在バグ (リファクタ消し忘れ) のため除去。
    ...
    def _calculate_labels_for_batch(
        ...
        for timeframe_tuple, group_df in daily_bets_df.group_by("timeframe"):
            ...
            bets_with_price_df = (
                ...
                .sort("timestamp")
                # [新設計] P(L) も取得: トリガー時刻 L の mid を backward asof。
                #   d = (P(L+180) − P(L)) / ATR の計算に使う (方向源 + ゲート)。
                .join_asof(
                    price_window_df.select(
                        pl.col("timestamp").alias("_ancL_ts"),
            ...
            _tmp_cols = [
                c for c in ["_entry_ts", "_anchor_ts", "_ancL_ts"]
            ...
            # [新設計] close_L (P(L)) も取れた行だけ残す (d 計算に必須)
            if "close_L" in bets_with_price_df.columns:
                bets_with_price_df = bets_with_price_df.filter(
                    pl.col("close_L").is_not_null()
                )
                if bets_with_price_df.is_empty():
                    continue
            ...
            bets_df_with_atr = bets_with_price_df.with_columns(
                ...
                # [新設計] 既発生運動 d = (P(L+180) − P(L)) / ATR。符号=方向源、|大きさ|=ゲート。
                pl.when(pl.col("atr_value") > 0)
                .then((pl.col("close") - pl.col("close_L")) / pl.col("atr_value"))
                .otherwise(None)
                .alias("d_atr")
            )

            # --- 全行を保持しつつ、ATR Ratioでエントリー起点に is_trigger=1 を付与 ---
            # [新設計] ゲート = atr_ratio>=閾値 AND |d|>=D_GATE_MIN_ATR (①をゲートへ外部化)
            bets_df_all = bets_df_with_atr.with_columns(
                pl.when(
                    (pl.col("atr_ratio") >= ATR_RATIO_THRESHOLD)
                    & pl.col("d_atr").is_not_null()
                    & (pl.col("d_atr").abs() >= D_GATE_MIN_ATR)
                )
                .then(1)
                .otherwise(0)
                .cast(pl.Int8)
            ...
            # [新設計] sign(d) 片側化 + side 列。
            #   d>0 のトリガーは long ラベルのみ有効 (short は null)、d<0 は short のみ。
            #   → M1-long は d>0 母集団、M1-short は d<0 母集団を学習 (2モデル・side masking)。
            #   「上に動いたバーを short する」等の無意味ラベルを作らない。
            final_group_df = final_group_df.with_columns(
                pl.when((pl.col("is_trigger") == 1) & (pl.col("d_atr") > 0))
                .then(pl.col("label_long"))
                .otherwise(None)
                .alias("label_long"),
                pl.when((pl.col("is_trigger") == 1) & (pl.col("d_atr") < 0))
                .then(pl.col("label_short"))
                .otherwise(None)
                .alias("label_short"),
            ...
            # [新設計・観測フック] side 別サンプル数と PT-first 基底率を累積
            #   (初回ランで「分離してよかったか / 単一に戻すべきか」を判定する材料)。
            try:
                _obs = final_group_df.filter(pl.col("is_trigger") == 1)
                _lo = _obs.filter(pl.col("label_long").is_not_null())
                _sh = _obs.filter(pl.col("label_short").is_not_null())
                self._side_obs["long_n"] += _lo.height
                self._side_obs["long_pt"] += int(_lo.select((pl.col("label_long") == 1).sum()).item()) if _lo.height else 0
                self._side_obs["short_n"] += _sh.height
                self._side_obs["short_pt"] += int(_sh.select((pl.col("label_short") == 1).sum()).item()) if _sh.height else 0
            except Exception as _e:
                logging.debug(f"side-obs skip: {_e}")
    ...
    def _log_final_summary(self):
        ...
        # [新設計・観測] side 別サンプル数 + PT-first 基底率 (分離判定の材料)。
        #   a=1,b=5 なら幾何だけで基底率 ≈ b/(a+b)=5/6≈0.833。見るべきは:
        #     (1) その基底率からの lift (μ>0 が押し上げているか)
        #     (2) 基底率自体が long/short でズレていないか (実現σの方向差)
        #     (3) 片側が数千オーダーなら分離が過学習 → 単一+side特徴を再検討
        o = self._side_obs
        ln, lp = o["long_n"], o["long_pt"]
        sn, sp = o["short_n"], o["short_pt"]
        lr = (lp / ln * 100) if ln else float("nan")
        sr = (sp / sn * 100) if sn else float("nan")
        # 幾何基底率 = 無ドリフトBMで近い上バリア(+a=PT)に先着する確率 = b/(a+b)。
        #   a=1(PT), b=5(SL) → 5/6 ≈ 83.3%。PT が近いほど先着しやすい。
        base = (
            RULE_LONG["sl_mult"]
            / (RULE_LONG["pt_mult"] + RULE_LONG["sl_mult"])
            * 100.0
        )
        side_summary = (
            "\n" + "=" * 60 + "\n"
            "### [新設計] side 別観測 (2モデル分離の判定材料) ###\n"
            f"  D_GATE_MIN_ATR = {D_GATE_MIN_ATR} / a:b = "
            f"{RULE_LONG['pt_mult']}:{RULE_LONG['sl_mult']} "
            f"(幾何基底率 b/(a+b) ≈ {base:.1f}%)\n"
            f"  LONG  (d>0): n={ln:,}  PT-first={lr:.2f}%  "
            f"(lift {lr - base:+.2f}pt)\n"
            f"  SHORT (d<0): n={sn:,}  PT-first={sr:.2f}%  "
            f"(lift {sr - base:+.2f}pt)\n"
            f"  基底率の左右差 (long−short): {lr - sr:+.2f}pt\n"
            + "=" * 60
        )
        logging.info(side_summary)
