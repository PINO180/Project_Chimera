...
class M1CrossValidator:
    ...
    def _load_features(self, feature_path: Path) -> List[str]:
        ...
        exclude_exact = {
            ...
            # --- [新設計 2026-07-16] 残余ドリフト収穫メタラベリング用メタデータ ---
            "close_L",   # P(L) (d=P(L+180)-P(L) 計算用、学習特徴量ではない)
            "d_atr",     # 既発生運動 = 方向源(sign)+ゲート(|d|)。①外部化ゆえ特徴量にしない
            "side",      # sign(d) 方向源 (2モデルでは side 固定ゆえ特徴量価値なし)
        }
    ...
    def _calculate_scale_pos_weight(self, direction: str) -> float:
        ...
        for partition_date in tqdm(
            ...
            try:
                ...
                # [新設計・side masking] 逆側は null。当該 side の label が付いた行だけで
                #   scale_pos_weight を算出 (long は d>0 母集団、short は d<0 母集団)。
                df_labels = df_labels.filter(pl.col(label_col).is_not_null())
            ...
            if len(train_partitions) > 0:
                for partition_date in tqdm(
                    ...
                    try:
                        ...
                        # [新設計・side masking] 当該 side (label_col) が付いた行だけ学習/予測。
                        #   逆側 (d の符号が反対) の null 行を除外 → long は d>0、short は d<0。
    ...
    def run(self) -> None:
        ...
        for direction in directions:
            ...
            if oof_results["timestamp"].size > 0:
                ...
                # =====================================================
                # [AUC-REPORT 2026-07-08] direction 完了時点で OOF AUC を表示。
                # 材料は oof_df に既にある prediction (予測確率) と true_label (0/1)。
                # NaN 予測 (fold 失敗時のフォールバック) と、片クラスしか無い異常時は
                # 除外/ガードして計算する。uniqueness 重み付き AUC も併記。
                # =====================================================
                try:
                    ...
                    logging.info(
                        f"  - Valid samples: {_n_valid:,} "
                        f"(dropped NaN/inf: {_n_drop:,})"
                    )
                    logging.info(
                        f"  - Class balance: pos(label=1)={_n_pos:,} / "
                        f"neg(label=0)={_n_neg:,}"
                    )

                    if _n_valid == 0 or _n_pos == 0 or _n_neg == 0:
                        logging.warning(
                            f"  - AUC skipped: only one class present or no valid rows "
                            f"(pos={_n_pos}, neg={_n_neg})."
                        )
                    else:
                        ...
                        try:
                            ...
                            logging.info(
                                f"  - OOF AUC (uniqueness-weighted): {_auc_w:.4f}"
                            )
                        except Exception as _we:
                            logging.warning(
                                f"  - Weighted AUC skipped: {_we}"
                            )
                        logging.info(
                            f"  - Pred proba: min={_yp.min():.4f} / "
                            f"mean={_yp.mean():.4f} / max={_yp.max():.4f}"
                        )
                    logging.info("-" * 50)
                except Exception as _auc_err:
                    logging.warning(
                        f"AUC report failed for {direction}: {_auc_err}",
                        exc_info=True,
                    )
            else:
