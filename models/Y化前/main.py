...
def _run_gap_fill(
    ...
    if diff_df is None or len(diff_df) == 0:
        logger.warning(
            "[GAP-FILL] 差分M0.5データの取得に失敗。ギャップ未充填のまま続行します。"
        )
        return last_processed_bar_time
    ...
    for _ts, _row in gap_fill_indexed.iterrows():
        ...
        # [§11.32.10.3 修正] proxy の index 規約を学習側 (2_G_alpha_neutralizer.py L175
        # `close.shift(1)/close.shift(2)-1`) に合わせる。
        # 旧: _m5[-1]/_m5[-2] - 1 を index=_m5.index[-1] (=T) に格納
        #     → proxy[T] = bar [T, T+5min) の return (= T から始まる bar)
        # 新: _m5[-2]/_m5[-3] - 1 を index=_m5.index[-1] (=T) に格納
        #     → proxy[T] = bar [T-5min, T) の return (= T で終わる bar)
        # 学習側 proxy_M5[T] (= T で終わる bar の return) と完全一致。
        # look-ahead 入らず (両側とも過去確定済 return)。
        if len(feature_engine.m05_dataframe) >= 20:
            _recent = pd.DataFrame(list(feature_engine.m05_dataframe)[-20:]).set_index(
                "timestamp"
            )
            _m5 = (
                _recent["close"]
            ...
            if len(_m5) >= 3:  # [§11.32.10.3] iloc[-3] アクセスのため 3 本必要
                _new_proxy_val = (float(_m5.iloc[-2]) - float(_m5.iloc[-3])) / (
                    float(_m5.iloc[-3]) + 1e-10
                )
...
def initialize_data_buffer(
    ...
    try:
        ...
        # [§11.32.10.3 修正] proxy の index 規約を学習側 (2_G_alpha_neutralizer.py L175
        # `close.shift(1)/close.shift(2)-1`) に合わせる。
        # 旧: pct_change(1) を index=M5 ラベル T に格納
        #     → proxy[T] = bar [T, T+5min) の return (= T から始まる bar)
        # 新: pct_change(1).shift(1) で 1 step 過去にずらす
        #     → proxy[T] = bar [T-5min, T) の return (= T で終わる bar)
        # 学習側 proxy_M5[T] と完全一致。 look-ahead 入らず (過去確定済 return)。
        proxy_df = m5_close.pct_change(1).shift(1).to_frame(name="market_proxy").dropna()
...
def main(dry_run: bool = False, ols_dump: bool = False):
    global g_market_proxy
    ...
    try:
        ...
        while True:
            try:
                # M3確定通知をポーリング（最大1秒待機）
                # [Phase 9d 発見 #61] poll_m3_bar は Optional[List[Dict]] を返す。
                # EA が直近 6 本の M0.5 dict を JSON 配列で送ってくるため、
                # ここでは List[Dict] (時系列順) で受け取る。
                ...
                if (
                    current_time_sec - last_buffer_check_time
                    ...
                    if (
                        ...
                        and current_time_sec - last_gap_fill_time
                        >= _GAP_FILL_COOLDOWN_SEC
                    ):
                        logger.warning(
                ...
                _near_m3_boundary = (_now_in_m3_cycle > 177.0) or (
                    _now_in_m3_cycle < 3.0
                )

                _do_periodic_sync = new_m05_bars is not None or (
                    not _near_m3_boundary
                    and (current_time_sec - last_periodic_sync_time)
                    >= _PERIODIC_SYNC_INTERVAL_SEC
                )
                    ...
                    if state_manager and state_manager.current_state:
                        for trade in state_manager.current_state.trades:
                            ...
                            if trade.direction == "BUY" and duration_mins >= td_long:
                                ...
                                trade.direction == "SELL" and duration_mins >= td_short
                            ):
                                is_timeout = True
                            if is_timeout:
                                logger.info(
                ...
                # M3未確定の場合はここで次のループへ
                # [Phase 9d 発見 #61] new_m05_bars は None または非空の List[Dict]。
                # poll_m3_bar 側で全件 SPIKE-GUARD で除外された場合も None を返す仕様。
                if new_m05_bars is None or len(new_m05_bars) == 0:
                    continue

                # ==========================================================
                # [Phase 9d 発見 #61] List[Dict] 取り扱い
                # ==========================================================
                # publisher (poll_m3_bar) は M3 確定通知時に直近 6 本の M0.5 dict を
                # 時系列順で返してくる (EA 設定上限。起動直後の buf_size<6 の場合は
                # 利用可能分のみ)。
                #
                # 学習側 s1_1_B の M0.5 bucket は M3 close 時点で「直前 6 本」が
                # 確定している状態なので、本番側も同じ 6 本を順次バッファに流し
                # 込むことで M3 OHLCV の sparse 化問題 (発見 #61) を解消する。
                #
                # 処理方針 (Audit Report §B.8 main.py | #61 の指示):
                #   - 先頭〜末尾の 1 本前: warmup_only=True で process_new_m05_bar
                #     を呼び、バッファ更新・OLS 更新のみ実行 (シグナル生成しない)
                #   - 最後の 1 本 (= _active_m05_bar): 通常モードでシグナル処理含む
                #     完全実行。STALE-GUARD / 市場プロキシ更新 / シグナル発火等は
                #     この最後のバーに対してのみ動作させる (既存仕様維持)
                #
                # この設計により:
                #   - publisher 側 List[Dict] 化 (発見 #61) と整合
                #   - process_new_m05_bar の warmup_only 引数を活用 (V=0 GUARD,
                #     DEDUP, バッファ整合性は全バーで担保される)
                #   - 市場プロキシは「シグナル発火直前の最新状態」で計算 (旧仕様維持)
                # ==========================================================

                # [DEDUP-M05DF 本流根治] 既処理バーの再投入を断つ。
                # poll_m3_bar は毎回「直近 6 本」を返す (L1038)。M3 途絶後の
                # BUFFER-INTEGRITY gap-fill / EA 再起動 gap-fill / スナップショット
                # 復帰直後に、live が既に処理した範囲を再度受け取ることがある。
                # 従来この live ループは g_last_processed_bar_time を更新せず、
                # 既処理バーを無条件に process_new_m05_bar へ流していたため、
                # gap-fill (= g_last_processed_bar_time 以降を取得) と時刻範囲が
                # オーバーラップし、m05_dataframe に同一時刻バーが二重 append された
                # (実機で 527 本オーバーラップ確認 → proxy partial → OLS cov 崩壊)。
                # ここで g_last_processed_bar_time 以下のバーを除外し、処理後に
                # 時刻を前進させることで、gap-fill と live が同じ最終処理時刻を共有し
                # 互いの処理済み範囲を再取得しなくなる。
                def _bar_unixtime(_b) -> Optional[int]:
                    _ts = _b.get("timestamp")
                    if _ts is None:
                        return None
                    # tz-naive は UTC とみなす (L1086 と同じ前提)
                    if getattr(_ts, "tzinfo", None) is None:
                        _ts = _ts.replace(tzinfo=timezone.utc)
                    return int(_ts.timestamp())

                if g_last_processed_bar_time is not None:
                    _filtered_bars = [
                        _b
                        for _b in new_m05_bars
                        if (_bar_unixtime(_b) is not None)
                        and (_bar_unixtime(_b) > g_last_processed_bar_time)
                    ]
                    if len(_filtered_bars) < len(new_m05_bars):
                        logger.debug(
                            f"[DEDUP-M05DF] 既処理バーを除外: "
                            f"{len(new_m05_bars)} → {len(_filtered_bars)} 本 "
                            f"(last_processed={g_last_processed_bar_time})"
                        )
                    new_m05_bars = _filtered_bars
                ...
                for _preceding_bar in new_m05_bars[:-1]:
                    try:
                        feature_engine.process_new_m05_bar(
                            _preceding_bar, g_market_proxy, warmup_only=True
                        )
                        # [DEDUP-M05DF 本流] 処理済み時刻を前進 (gap-fill と共有)
                        _pb_ut = _bar_unixtime(_preceding_bar)
                        if _pb_ut is not None:
                            g_last_processed_bar_time = max(
                                g_last_processed_bar_time or 0, _pb_ut
                            )
                    except Exception as _e:
                        logger.warning(
                            f"⚠️ [Phase 9d #61] 先行 M0.5 バー (warmup_only) の "
                            f"処理に失敗: ts={_preceding_bar.get('timestamp')} "
                            f"err={_e}"
                        )
                        # 1 本失敗しても残りの処理は継続する (= old_old_main の
                        # _run_gap_fill と同じ堅牢性ポリシー)

                # 最後のバー = シグナル発火対象 (= 旧来 new_m05_bar が指していたもの)
                _active_m05_bar = new_m05_bars[-1]
                ...
                _bar_ts = _active_m05_bar.get("timestamp")
                ...
                # [Phase 9d 発見 #61] 先行バーは warmup_only=True で既にバッファ
                # 反映済なので、ここでの m05_dataframe は最新 (末尾バー直前) を
                # 反映した状態。「シグナル発火直前の最新プロキシ」を計算する旧仕様
                # と完全に同じ意味になる。
                # [§11.32.10.3 修正] proxy の index 規約を学習側 (2_G_alpha_neutralizer.py L175
                # `close.shift(1)/close.shift(2)-1`) に合わせる。
                # 旧: _m5[-1]/_m5[-2] - 1 を index=_m5.index[-1] (=T) に格納
                ...
                # 新: _m5[-2]/_m5[-3] - 1 を index=_m5.index[-1] (=T) に格納
                #     → proxy[T] = bar [T-5min, T) の return (= T で終わる bar)
                # 学習側 proxy_M5[T] と完全一致。 look-ahead 入らず。
                # 上のコメント「学習側2Gと完全一致」 は §11.19 後の training shift 修正で
                # 破綻していた (training は index ずれ、 production は元のまま) のを今回是正。
                if feature_engine and len(feature_engine.m05_dataframe) >= 20:
                    ...
                    if len(_m5) >= 3:  # [§11.32.10.3] iloc[-3] アクセスのため 3 本必要
                        new_proxy_val = (float(_m5.iloc[-2]) - float(_m5.iloc[-3])) / (
                            float(_m5.iloc[-3]) + 1e-10
                        )
                ...
                # [Phase 9d 発見 #61] 最後のバーのみ通常モード (warmup_only=False)
                # で実行 → シグナル生成経路が動く。先行バーは上で warmup_only=True
                # で消化済なので、この呼び出しが M3 close 確定後のシグナル発火点。
                _m3_ts_before = feature_engine.last_bar_timestamps.get("M3")
                signal_list = feature_engine.process_new_m05_bar(
                    _active_m05_bar, g_market_proxy
                )
                _m3_ts_after = feature_engine.last_bar_timestamps.get("M3")

                # [DEDUP-M05DF 本流] _active_m05_bar 本処理成功後に処理済み時刻を前進。
                # 例外時 (process_new_m05_bar が送出) はここに到達しないので前進しない。
                # COOLDOWN の continue (下) より前に置き、シグナル発火有無と独立に
                # 「処理済み時刻」を確実に進める (gap-fill との共有を担保)。
                _ab_ut = _bar_unixtime(_active_m05_bar)
                if _ab_ut is not None:
                    g_last_processed_bar_time = max(
                        g_last_processed_bar_time or 0, _ab_ut
                    )
                ...
                for signal in signal_list:
                    ...
                    # ════════════════════════════════════════════════════
                    # [HF-NB-GATE §11.32 / §B.11.9] 高 TF (M5/M8/M15) 非境界 0 化
                    # ────────────────────────────────────────────────────
                    # 学習側 (S6 build = pl.concat(diagonal) で非境界セルが NaN、
                    # Ax2/Cx2/infer の fill_null(0) で 0 化) は、M3 シグナル時刻が
                    # 高 TF グリッドと整合しないバーで該当 TF 列を 0 として学習している。
                    # 本番は latest_features_cache の前方補完で実値を流していたため
                    # train-serve OOD が発生していた (§11.31〜32 の調査結果)。
                    # 学習側レジームに本番を揃える gate。LCM(3, tf_min) 周期でのみ
                    # M3 シグナル時刻が高 TF と整合し、それ以外の時刻では学習側に
                    # 存在しない (= 0 で埋められている) 値を本番からも提供しない。
                    # M0.5/M1/M3 は M3 シグナル時刻と常に整合するため対象外。
                    # 設計根拠: Skew Detection Hardening Plan §B.11.9 (2 重直交分解)
                    # ════════════════════════════════════════════════════
                    # [LOOKAHEAD-FIX §11.34.16] 境界判定の基準を signal.timestamp (T =
                    # M3 バーのクローズ時刻) から「行のラベル L = T - 3分」に変更。
                    # 学習側 S6 の高 TF 実値行は L のグリッド (L % tf == 0) にあるため、
                    # T 基準 (旧) では本番と学習で実値/0 が互い違いになっていた
                    # (6/11 実証: ゲート不整合 48/85 行で予測 corr 0.32-0.58 に崩壊、
                    #  整合 37 行は corr 0.96-0.99)。
                    # ※ S6 再構築 (高TF閉じバー化) + 再学習モデルとセットで配置すること。
                    #    (timedelta は未 import のため epoch 秒の算術で -180s する)
                    _hfnbg_minute_idx = int(
                        (signal.timestamp.timestamp() - 180) // 60
                    )
                    for _hfnbg_name in list(feature_dict.keys()):
                        _hfnbg_m = re.search(
                            r"_M([0-9]+(?:\.[0-9]+)?)$", _hfnbg_name
                        )
                        if _hfnbg_m is None:
                            continue
                        try:
                            _hfnbg_tf_min = float(_hfnbg_m.group(1))
                        except (TypeError, ValueError):
                            continue
                        # M0.5 / M1 / M3 は M3 トリガと常に整合 → ゲート対象外
                        if _hfnbg_tf_min < 5:
                            continue
                        _hfnbg_tf_int = int(_hfnbg_tf_min)
                        if (
                            _hfnbg_tf_int > 0
                            and _hfnbg_minute_idx % _hfnbg_tf_int != 0
                        ):
                            feature_dict[_hfnbg_name] = 0.0
                    # ════════════════════════════════════════════════════
                    ...
                    X_long_m1 = np.array(
                        [
                            [
                                feature_dict.get(f, 0.0) or 0.0
                                for f in feature_lists["long_m1"]
                            ]
                        ],
                    ...
                    X_short_m1 = np.array(
                        [
                            [
                                feature_dict.get(f, 0.0) or 0.0
                                for f in feature_lists["short_m1"]
                            ]
                        ],
                        dtype=np.float32,  # [DTYPE-ALIGN 段2]
                    )
                    ...
                    if should_trade_long or should_trade_short:
                        try:
                            ...
                            # ▼▼▼ [Phase 11 修正] feature_lists union ベースの header に変更 ▼▼▼
                            # 旧実装の問題:
                            #   header = feature_dict.keys() で初回シグナル発火時に固定
                            #   → その後 rfe_1X コードが拡張されて feature_dict が増えても、
                            #     CSV header は初回時点で凍結。新 key は永遠に CSV に乗らない。
                            #   → compare_snapshots.py が CSV header と feature_lists の集合差
                            #     を取った結果「393 個 missing」と誤判定する原因になっていた
                            #     (発見 #69 FCG の真因は実際はこの CSV header 凍結問題で、
                            #      log で 1343 個全部計算されていることを確認済 2026-05-25)。
                            #
                            # 新実装:
                            #   学習側 feature_lists の union を header の真実源にする。
                            #   - 計算結果が無い (= feature_dict に key 不在) → 空白で記録
                            #   - 計算結果が 0.0 / NaN → そのまま値で記録
                            #   - これで「key 不在」と「key 有り 0 値」を CSV から識別可能
                            #   - rfe_1X コードを拡張しても CSV header は安定
                            #
                            # 注: feature_dict["m1_pred_proba"] は L1390-1392 で上書きされる
                            #     必要があるため、ヘッダー列に含める。feature_lists["long_m2"]
                            #     / ["short_m2"] の末尾には L583-587 で既に m1_pred_proba が
                            #     append 済み、union 集合に含まれる。
                            all_required_keys = sorted(
                                set(feature_lists["long_m1"])
                                | set(feature_lists["long_m2"])
                                | set(feature_lists["short_m1"])
                                | set(feature_lists["short_m2"])
                            )
                            # ▲▲▲ [Phase 11 修正] ▲▲▲
                            ...
                            with open(
                                ...
                                # ▼▼▼ [Phase 11 修正] union ベースで値を取得 ▼▼▼
                                # 旧: row_data.extend([feature_dict[k] for k in feature_keys])
                                # 新: feature_dict に key が無い場合は "" (空白) で記録。
                                #     空白 = 「計算されていない」、 0.0/数値 = 「計算済」 と
                                #     CSV を見るだけで識別可能になる。
                                row_data.extend([
                                    feature_dict[k] if k in feature_dict else ""
                            ...
                            # ▼▼▼ [Phase 11 修正] ログを 3 指標 (要求/計算済/未計算) に拡張 ▼▼▼
                            n_required = len(all_required_keys)
                            n_computed = sum(
                                1 for k in all_required_keys if k in feature_dict
                            )
                            ...
                            if n_missing > 0:
                                sample_missing = [
                                    k for k in all_required_keys if k not in feature_dict
                                ][:5]
                                logger.warning(
                                    f"⚠️  {n_missing} 個の特徴量が feature_dict に不在 "
                                    f"(CSV に空白で記録) 先頭5件={sample_missing}"
                                )
                            # ▲▲▲ [Phase 11 修正] ▲▲▲
                            ...
                            if ols_dump:
                                try:
                                    import pickle
                                    ols_dump_dir = config.LOGS_DIR / "ols_state_dumps"
                                    ols_dump_dir.mkdir(parents=True, exist_ok=True)
                                    safe_tf = signal.timeframe.replace(".", "_")
                                    ts_str = signal.timestamp.strftime(
                                        "%Y%m%d_%H%M%S"
                                    )
                                    dump_path = (
                                        ols_dump_dir
                                        / f"ols_{ts_str}_{safe_tf}.pkl"
                                    )
                                    dump_obj = {
                                        ...
                                        "ols_state": {
                                            tf: {
                                                f: dict(s)
                                                for f, s in tfs.items()
                                            }
                                            for tf, tfs in
                                            feature_engine.ols_state.items()
                                        },
                                        "proxy_feature_buffers": {
                                            tf: {
                                                f: list(d)
                                                for f, d in tfb.items()
                                            }
                                            for tf, tfb in
                                            feature_engine.proxy_feature_buffers.items()
                                        },
                                        "market_proxy_tail_200": (
                                            g_market_proxy.tail(200)
                                                .reset_index()
                                                .to_dict("records")
                                            if g_market_proxy is not None
                                            and len(g_market_proxy) > 0
                                            else None
                                        ),
                                    }
                                    ...
                                    logger.info(
                                        f"🔬 [OLS DUMP] {dump_path.name} "
                                        f"({dump_path.stat().st_size:,} bytes)"
                                    )
                                except Exception as e:
                                    logger.warning(
                                        f"OLS state dump 失敗: {e}",
                                        exc_info=False,
                                    )
                            # ▲▲▲ [OLS state dump フック] ▲▲▲
                        except Exception as e:
                    ...
                    # [Phase 9d 発見 #61] シグナル発火に使われた最後のバー
                    # (_active_m05_bar = new_m05_bars[-1]) の spread を参照する。
                    current_spread = _active_m05_bar.get("spread", 999.0)
                    ...
                    _min_barrier = risk_engine.config.get(
                        "min_barrier_dollar_width", 1.5
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--ols-dump", action="store_true",
        help="シグナル発火時に OLS state + deque を pickle dump",
    )
    cli_args = parser.parse_args()
    main(dry_run=cli_args.dry_run, ols_dump=cli_args.ols_dump)
