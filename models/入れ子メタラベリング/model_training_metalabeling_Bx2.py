...
# ============================================================================
# 【手動設定】A→B 接続の「床」= M1 ゲート閾値（logit 空間）
# ----------------------------------------------------------------------------
# 入れ子メタラベリングの接続は「床付き急峻可変重み」。床下（μ枯れバー）は切り、
# 床上は合成重み uniqueness_B × h(q_A) で濃淡をつける（h は Bx2 が自動計算）。
#
#   M1_GATE_LOGIT = 0.0  → proba 0.5（現行同値）。Ax2 は scale_pos_weight で proba を
#                          0.5 中心に均衡化するため、0.0 は実質「中央値カット」で強め。
#   combo 1（電源確認）  : 現行再現のため 0.0 のまま。
#   combo 2 以降         : μ枯れのみ切るため下げる（例 -0.85 ≈ proba 0.3）。
# ----------------------------------------------------------------------------
M1_GATE_LOGIT = 0.0
        ...
        for direction in ["long", "short"]:
            ...
            for partition_date in tqdm(
                ...
                try:
                    ...
                    # [入れ子メタラベリング] 床（M1ゲート）= μ枯れバーを切る。
                    # logit空間の閾値。既定 0.0(=proba0.5)。combo 2以降は下げる（上部 M1_GATE_LOGIT）。
                    top_n_keys = (
                        daily_m1_oof.filter(pl.col("m1_pred_proba") >= M1_GATE_LOGIT)
                    ...
                    #   h(q_A) = 2*sigmoid(logit)-1 = tanh(logit/2)（proba0.5→0, 1→1）
                    #   m1_pred_proba はこの時点で logit 値。床上のみ残るので h∈[0,1] を担保。
                    #   ※ n_eff 暴れが問題なら h を順位正規化に差し替える（設計上の代替）。
                    h_qa = (
                        (2.0 / (1.0 + (-pl.col("m1_pred_proba")).exp()) - 1.0)
                        .clip(0.0, 1.0)
                    )
