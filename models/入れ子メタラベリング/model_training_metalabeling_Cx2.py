...
class FinalAssembler:
    def __init__(self, config: FinalTrainingConfig):
        ...
        # [本番一致・絶対制約] m1_pred_proba の実行時末尾追加を維持する。
        #   本番 main.py が M2 特徴量へ同一の append を行うため、ここを外すと
        #   学習==本番の bit 一致（乖離3部作で 1e-15 まで実証）が壊れる。
        #   ※ v2 で一度リーク懸念から削除したが、本番との対称性を破壊するため復元。
        #     リーク懸念は本番一致を壊さない範囲で別途検討する（ここは触らない）。
    ...
    def _load_features(self, feature_path: Path) -> List[str]:
        ...
        exclude_exact = {
            ...
            "label_A_long", "label_A_short", "label_B_long", "label_B_short",
            "duration_A_long", "duration_A_short", "duration_B_long", "duration_B_short",
            "uniqueness_A_long", "uniqueness_A_short", "uniqueness_B_long", "uniqueness_B_short",
            "concurrency_A_long", "concurrency_A_short", "concurrency_B_long", "concurrency_B_short",
    ...
    def _calculate_scale_pos_weight_m1(self) -> float:
        ...
        for partition_date in tqdm(
            ...
            try:
                target_col = f"label_A_{self.direction}"
    ...
    def _ensure_model_trained(
        ...
        if not model_path.exists():
            ...
            model = self._train_single_model(
                f"{model_name} (Final)",
                "meta_label" if is_m2 else f"label_A_{self.direction}",
                is_m2,
                partitions_to_train,
                lgbm_params,
            )
    ...
    def _train_single_model(
        ...
        for p_date in tqdm(
            ...
            weight_col = "uniqueness" if is_m2 else f"uniqueness_A_{self.direction}"
    ...
    def _gather_predictions_for_calibration(
        ...
        for p_date in tqdm(
            ...
            actual_target_col = "meta_label" if is_m2 else f"label_A_{self.direction}"
            weight_col = "uniqueness" if is_m2 else f"uniqueness_A_{self.direction}"
