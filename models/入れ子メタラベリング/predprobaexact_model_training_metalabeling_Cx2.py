...
class FinalAssembler:
    def __init__(self, config: FinalTrainingConfig):
        ...
        # [入れ子メタラベリング v2 修正] q_A(m1_pred_proba) の M2 特徴量注入は撤廃。
        #   理由: (a) q_A=g(x) ゆえ特徴として無益＋自由度増（§9.4）、
        #        (b) q_A は Ax2 の fold でOOFだが Cx2 の fold とは境界が異なるため、
        #            特徴として使うと二次リークの主経路になる。
        #   q_A の影響はサンプル重み uniqueness(=uniqueness_B×h(q_A)) 経由でのみ効かせる。
        #   （旧: self.features_m2.append("m1_pred_proba") を削除）
