...
# ============================================================================
# 【手動設定パネル】入れ子メタラベリング用 A/B 2ラベル幾何
# ----------------------------------------------------------------------------
# combo を回すたびに、下の RULE_A_* / RULE_B_* を該当 combo の値に手で書き換える。
# この combos 早見表はコメント（人間用メモ）。プログラムは読まない。
#
#   combos 早見表（A=選抜脳ラベル / B=本番発注ラベル、td はエントリー起点の分・内部値）:
#     1: {"A":{"pt":1.0,"sl":5.0,"td":30}, "B":{"pt":1.0,"sl":5.0,"td":30}}  電源確認
#     2: {"A":{"pt":1.0,"sl":5.0,"td":30}, "B":{"pt":2.2,"sl":2.2,"td":30}}  利益の本命
#     3: {"A":{"pt":1.0,"sl":5.0,"td":30}, "B":{"pt":2.2,"sl":2.2,"td":17}}  出血止め検証
#     4: {"A":{"pt":2.2,"sl":2.2,"td":30}, "B":{"pt":2.2,"sl":2.2,"td":30}}  定規を磨く
#     5: {"A":{"pt":2.2,"sl":2.2,"td":30}, "B":{"pt":3.0,"sl":3.0,"td":42}}  長持ちμ伏兵
#     6: {"A":{"pt":1.0,"sl":5.0,"td":30}, "B":{"pt":1.0,"sl":5.0,"td":17}}  なめらか+出血止め
#
#   ※ td はエントリー起点(内部値)。BT表示(L起点)は +3分（td=30 → BT表示33 → 旧脳TO30）。
#   ※ 回す手順: ①下の RULE_A/B を上表の該当 combo に書換 → ②sh Step14-22 実行
#              → ③backtest 側の pt/sl/td も同じ B 値に手で合わせて実行。
# ----------------------------------------------------------------------------
# --- A（選抜脳 = M1）用ラベル幾何 ---
RULE_A_LONG  = {"pt_mult": 1.0, "sl_mult": 5.0, "td": "30m"}
RULE_A_SHORT = {"pt_mult": 1.0, "sl_mult": 5.0, "td": "30m"}

# --- B（本番発注 = M2）用ラベル幾何 ---
RULE_B_LONG  = {"pt_mult": 1.0, "sl_mult": 5.0, "td": "30m"}
RULE_B_SHORT = {"pt_mult": 1.0, "sl_mult": 5.0, "td": "30m"}
# --- ▲▲▲ 改造ここまで ▲▲▲ ---
...
class ProxyLabelingEngine:
    ...
    def __init__(self, config: ProxyLabelConfig):
        ...
        # A==B 配線検算を最初のトリガー群で一度だけ行うためのフラグ
        self._ab_identity_checked: bool = False
    ...
    def run(self):
        ...
        logging.info(
            f"Rule A Long : PT={RULE_A_LONG['pt_mult']}, SL={RULE_A_LONG['sl_mult']}, TD={RULE_A_LONG['td']}"
        )
        logging.info(
            f"Rule A Short: PT={RULE_A_SHORT['pt_mult']}, SL={RULE_A_SHORT['sl_mult']}, TD={RULE_A_SHORT['td']}"
        )
        logging.info(
            f"Rule B Long : PT={RULE_B_LONG['pt_mult']}, SL={RULE_B_LONG['sl_mult']}, TD={RULE_B_LONG['td']}"
        )
        logging.info(
            f"Rule B Short: PT={RULE_B_SHORT['pt_mult']}, SL={RULE_B_SHORT['sl_mult']}, TD={RULE_B_SHORT['td']}"
        )
        ...
        try:
            ...
            # A/B 両幾何の TD の最大値を採用（B が最長になり得るため両方を考慮）
            max_lookahead_minutes = max(
                self._get_duration_in_minutes(RULE_A_LONG["td"]),
                self._get_duration_in_minutes(RULE_A_SHORT["td"]),
                self._get_duration_in_minutes(RULE_B_LONG["td"]),
                self._get_duration_in_minutes(RULE_B_SHORT["td"]),
            )
            ...
            for month_row in tqdm(
                ...
                # が支配項だった (Numba 走査でも join_asof でもない)。
                #
    ...
    def _calculate_labels_for_batch(
        ...
        for timeframe_tuple, group_df in daily_bets_df.group_by("timeframe"):
            ...
            # ロング/ショートそれぞれのタイムアウト（TD）を A/B 幾何ごとに計算
            td_A_long_minutes = self._get_duration_in_minutes(RULE_A_LONG["td"])
            td_A_short_minutes = self._get_duration_in_minutes(RULE_A_SHORT["td"])
            td_B_long_minutes = self._get_duration_in_minutes(RULE_B_LONG["td"])
            td_B_short_minutes = self._get_duration_in_minutes(RULE_B_SHORT["td"])
            ...
            def _t1_expr(td_minutes: float):
                return (
                    pl.col("timestamp")
                    + pl.duration(seconds=self.ACTION_HORIZON_SEC)
                    + pl.duration(minutes=td_minutes)
                )

            t1_max_A_long_expr = _t1_expr(td_A_long_minutes)
            t1_max_A_short_expr = _t1_expr(td_A_short_minutes)
            t1_max_B_long_expr = _t1_expr(td_B_long_minutes)
            t1_max_B_short_expr = _t1_expr(td_B_short_minutes)
            ...
            # --- [入れ子メタラベリング] close_future = P(L+180) = エントリー価格 ---
            # label_A の錨は close(=P(L), 上の join_asof backward at L)＝下駄あり定規。
            # label_B の錨は close_future(=P(L+180))＝純②（本番発注幾何・下駄なし）。
            # ここで各トリガー行の「エントリー時刻 L+180」の価格を別途 backward asof で引く。
            pw_future = (
                price_window_df.select(
                    pl.col("timestamp").alias("_pw_ts"),
                    pl.col("close").alias("close_future"),
                )
                .sort("_pw_ts")
            )
            bets_with_price_df = (
                bets_with_price_df.with_columns(
                    (
                        pl.col("timestamp")
                        + pl.duration(seconds=self.ACTION_HORIZON_SEC)
                    ).alias("_entry_ts")
            ...
            # 一時列を安全に除去（バージョン差で _pw_ts が残る/残らないの両方に対応）
            _drop = [
                c for c in ("_entry_ts", "_pw_ts") if c in bets_with_price_df.columns
            ]
            if _drop:
                bets_with_price_df = bets_with_price_df.drop(_drop)
            # close_future が取れない窓端行は label_B の錨が無いので除外
            bets_with_price_df = bets_with_price_df.filter(
                pl.col("close_future").is_not_null()
            )
            if bets_with_price_df.is_empty():
                continue
            ...
            # バリアの一括計算 (ロングとショート両方)
            # --- 入れ子メタラベリング: A幾何(錨=close=P(L)) / B幾何(錨=close_future=P(L+180)) ---
            # 各方向のバリア価格式（RULE と 錨カラム を引数化）。
            #   A: 錨 close     → 下駄あり（PT=P(L)+a·ATR。L+180から見ると a·ATR−d 手前）＝定規
            #   B: 錨 close_future → 純②（PT=P(L+180)+a·ATR。下駄なし）＝本番発注幾何
            def _pt_long(rule, anchor):
                return pl.col(anchor) + pl.col("atr_value") * rule["pt_mult"] + SPREAD

            def _sl_long(rule, anchor):
                return pl.col(anchor) - pl.col("atr_value") * rule["sl_mult"] + SPREAD

            def _pt_short(rule, anchor):
                return pl.col(anchor) - pl.col("atr_value") * rule["pt_mult"] - SPREAD

            def _sl_short(rule, anchor):
                return pl.col(anchor) + pl.col("atr_value") * rule["sl_mult"] - SPREAD

            bets_df = bets_df_filtered.select(
                pl.col("timestamp").alias("t0"),
                # ★ A幾何バリア（錨=close=P(L)・下駄あり定規。PTは遠く/SLは近く）
                _pt_long(RULE_A_LONG, "close").alias("pt_A_long"),
                _sl_long(RULE_A_LONG, "close").alias("sl_A_long"),
                t1_max_A_long_expr.alias("t1_max_A_long"),
                _pt_short(RULE_A_SHORT, "close").alias("pt_A_short"),
                _sl_short(RULE_A_SHORT, "close").alias("sl_A_short"),
                t1_max_A_short_expr.alias("t1_max_A_short"),
                # ★ B幾何バリア（錨=close_future=P(L+180)・純②・本番発注幾何）
                _pt_long(RULE_B_LONG, "close_future").alias("pt_B_long"),
                _sl_long(RULE_B_LONG, "close_future").alias("sl_B_long"),
                t1_max_B_long_expr.alias("t1_max_B_long"),
                _pt_short(RULE_B_SHORT, "close_future").alias("pt_B_short"),
                _sl_short(RULE_B_SHORT, "close_future").alias("sl_B_short"),
                t1_max_B_short_expr.alias("t1_max_B_short"),
                pl.col("atr_value"),
                pl.col(
                    "atr_ratio"
            ...
            try:
                ...
                entry_offset_us = np.int64(self.ACTION_HORIZON_SEC * 1_000_000)

                # A幾何の numpy 配列
                bets_t1_max_A_l_np = bets_df["t1_max_A_long"].cast(pl.Int64).to_numpy()
                bets_t1_max_A_s_np = bets_df["t1_max_A_short"].cast(pl.Int64).to_numpy()
                bets_pt_A_l_np = bets_df["pt_A_long"].to_numpy(writable=True)
                bets_sl_A_l_np = bets_df["sl_A_long"].to_numpy(writable=True)
                bets_pt_A_s_np = bets_df["pt_A_short"].to_numpy(writable=True)
                bets_sl_A_s_np = bets_df["sl_A_short"].to_numpy(writable=True)

                # B幾何の numpy 配列
                bets_t1_max_B_l_np = bets_df["t1_max_B_long"].cast(pl.Int64).to_numpy()
                bets_t1_max_B_s_np = bets_df["t1_max_B_short"].cast(pl.Int64).to_numpy()
                bets_pt_B_l_np = bets_df["pt_B_long"].to_numpy(writable=True)
                bets_sl_B_l_np = bets_df["sl_B_long"].to_numpy(writable=True)
                bets_pt_B_s_np = bets_df["pt_B_short"].to_numpy(writable=True)
                bets_sl_B_s_np = bets_df["sl_B_short"].to_numpy(writable=True)
            except Exception as e:
                logging.error(f"Failed to convert bets data to Numpy arrays: {e}")
                continue

            # Numbaによる双方向同時判定を A幾何・B幾何で 2 回実行
            # （numba 関数シグネチャは不変。呼び出しを 2 回にする＝第2ラベルセット化）
            out_pt_l_A, out_sl_l_A, out_pt_s_A, out_sl_s_A = _numba_find_hits_dual(
                bets_t0_np,
                bets_t1_max_A_l_np,
                bets_t1_max_A_s_np,
                bets_pt_A_l_np,
            ...
            out_pt_l_B, out_sl_l_B, out_pt_s_B, out_sl_s_B = _numba_find_hits_dual(
                bets_t0_np,
                bets_t1_max_B_l_np,
                bets_t1_max_B_s_np,
                bets_pt_B_l_np,
            ...
            # ラベル/決済時刻/経過分を組み立てる汎用式（先着 = PT が SL より先）
            def _label_expr(pt_col, sl_col):
                hit = (pl.col(pt_col) > 0) & (
            ...
            def _end_expr(pt_col, sl_col, t1_col):
                hit = (pl.col(pt_col) > 0) & (
                    (pl.col(sl_col) == 0) | (pl.col(pt_col) < pl.col(sl_col))
                )
                return (
                    ...
                    .then(pl.col(sl_col))
                    .otherwise(pl.col(t1_col).cast(pl.Int64))
                )

            def _dur_expr(end_col):
                return (
                    (pl.col(end_col) - pl.col("t0").cast(pl.Int64)) / 1_000_000 / 60.0
                ).cast(pl.Float32)

            # 計算済みトリガー行の DataFrame 作成（A/B の到達時刻を全て付与）
            calculated_df = (
                bets_df.with_columns(
                    pl.Series("pt_l_time_A", out_pt_l_A),
                    pl.Series("sl_l_time_A", out_sl_l_A),
                    pl.Series("pt_s_time_A", out_pt_s_A),
                    pl.Series("sl_s_time_A", out_sl_s_A),
                    pl.Series("pt_l_time_B", out_pt_l_B),
                    pl.Series("sl_l_time_B", out_sl_l_B),
                    pl.Series("pt_s_time_B", out_pt_s_B),
                    pl.Series("sl_s_time_B", out_sl_s_B),
                )
                .with_columns(
                    # --- A幾何ラベル・決済時刻 ---
                    label_A_long=_label_expr("pt_l_time_A", "sl_l_time_A"),
                    end_A_l=_end_expr("pt_l_time_A", "sl_l_time_A", "t1_max_A_long"),
                    label_A_short=_label_expr("pt_s_time_A", "sl_s_time_A"),
                    end_A_s=_end_expr("pt_s_time_A", "sl_s_time_A", "t1_max_A_short"),
                    # --- B幾何ラベル・決済時刻 ---
                    label_B_long=_label_expr("pt_l_time_B", "sl_l_time_B"),
                    end_B_l=_end_expr("pt_l_time_B", "sl_l_time_B", "t1_max_B_long"),
                    label_B_short=_label_expr("pt_s_time_B", "sl_s_time_B"),
                    end_B_s=_end_expr("pt_s_time_B", "sl_s_time_B", "t1_max_B_short"),
                )
                .with_columns(
                    duration_A_long=_dur_expr("end_A_l"),
                    duration_A_short=_dur_expr("end_A_s"),
                    duration_B_long=_dur_expr("end_B_l"),
                    duration_B_short=_dur_expr("end_B_s"),
                )
                .select(
                    [
                        "t0",
            ...
            # --- 錨分離の配線診断（一度だけ・情報ログ）---
            # 設計: label_A 錨=close(P(L),下駄あり) / label_B 錨=close_future(P(L+180),純②)。
            # 検証の核: B は純②なのでエントリー時に価格がバリア中心にあり、最短 duration は
            #   3.0 より大きい（即決着が消える）。A は下駄で ~3.0 の即決着が残る。
            #   （ユーザーの「TD 3.00 の存在＝下駄の証拠」を配線チェックに転用）
            if not self._ab_identity_checked:
                n = calculated_df.height
                dis_l = calculated_df.filter(
                    pl.col("label_A_long") != pl.col("label_B_long")
                ).height
                dis_s = calculated_df.filter(
                    pl.col("label_A_short") != pl.col("label_B_short")
                ).height
                min_dur_A = calculated_df.select(
                    pl.col("duration_A_long").min()
                ).item()
                min_dur_B = calculated_df.select(
                    pl.col("duration_B_long").min()
                ).item()
                near3_A = calculated_df.filter(
                    pl.col("duration_A_long") < 3.05
                ).height
                near3_B = calculated_df.filter(
                    pl.col("duration_B_long") < 3.05
                ).height
                logging.info(
                    f"[ANCHOR CHECK] tf={timeframe} n={n}: "
                    f"label_A≠label_B long={dis_l}({dis_l / max(n, 1) * 100:.1f}%) short={dis_s}. "
                    f"min duration_A={min_dur_A:.3f} / duration_B={min_dur_B:.3f}. "
                    f"near-3.0(即決着) A={near3_A} B={near3_B}. "
                    "（設計期待: Aは下駄で~3.0即決着あり／Bは純②で最短>3.0・~3.0ほぼ無し）"
                )
                if dis_l == 0 and dis_s == 0:
                    logging.warning(
                        "[ANCHOR CHECK] label_A と label_B が完全一致。"
                        "close_future の錨分離が効いていない可能性（B が下駄込みのまま）。要確認。"
                    )
                self._ab_identity_checked = True
            ...
            # close_future はバリア計算(label_B錨)専用の一時列。S6 に出すと特徴量に漏れ、
            # 生の未来価格 P(L+180) の未来リークになるため、出力前に必ず落とす。
            drop_candidates = ["open", "high", "low", atr_col_name, "close_future"]
    ...
    def _update_label_counts_dual(self, df: pl.DataFrame):
        # レポートは B幾何（本番発注ラベル）を集計する
        # ロングの集計
        long_counts = df.group_by("label_B_long").len()
        for row in long_counts.iter_rows(named=True):
            if row["label_B_long"] in self.label_counts_long:
                self.label_counts_long[row["label_B_long"]] += row["len"]

        # ショートの集計
        short_counts = df.group_by("label_B_short").len()
        for row in short_counts.iter_rows(named=True):
            if row["label_B_short"] in self.label_counts_short:
                self.label_counts_short[row["label_B_short"]] += row["len"]
    ...
    def _collect_report_data_dual(self, df: pl.DataFrame, current_date: dt.date):
        try:
            ...
            # レポートは B幾何（本番発注ラベル）を集計名（label_long 等）にリネームして流用
            report_df = (
                ...
                .select(
                    [
                        "timeframe",
                        pl.col("label_B_long").alias("label_long"),
                        pl.col("label_B_short").alias("label_short"),
                        pl.col("duration_B_long").alias("duration_long"),
                        pl.col("duration_B_short").alias("duration_short"),
                        "date",
                    ]
                )
            )
...
| **Rule A Long (M1)** | `PT: {RULE_A_LONG["pt_mult"]}, SL: {RULE_A_LONG["sl_mult"]}, TD: {RULE_A_LONG["td"]}` |
| **Rule A Short (M1)** | `PT: {RULE_A_SHORT["pt_mult"]}, SL: {RULE_A_SHORT["sl_mult"]}, TD: {RULE_A_SHORT["td"]}` |
| **Rule B Long (M2/発注)** | `PT: {RULE_B_LONG["pt_mult"]}, SL: {RULE_B_LONG["sl_mult"]}, TD: {RULE_B_LONG["td"]}` |
| **Rule B Short (M2/発注)** | `PT: {RULE_B_SHORT["pt_mult"]}, SL: {RULE_B_SHORT["sl_mult"]}, TD: {RULE_B_SHORT["td"]}` |
| _(注)_ | _下表 Overall/Duration は B幾何（本番発注ラベル）の統計_ |
