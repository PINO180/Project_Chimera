        ...
        while True:
            try:
                ...
                for signal in signal_list:
                    ...
                    # ════════════════════════════════════════════════════
                    # [JOINASOF-FIX §11.34.16-O2] HF-NB-GATE 撤去 (選択肢 Y へ移行)
                    # ────────────────────────────────────────────────────
                    # 旧 HF-NB-GATE は「学習側 S6 が高TF非境界を 0 にしていた (スパース)」
                    # ことに本番を合わせるための整合装置だった。今回 cpl を join_asof
                    # (close_ts<=T の最新足を全トリガーに ffill) に統一し、学習側も
                    # 高TFを held で持つようにした (選択肢 Y)。本番 (rfe latest_features_cache)
                    # は元々 ffill (held) のため、ゲートで 0 化せずそのまま使えば学習と一致する。
                    # → ゲート (高TF非境界の 0 上書き) は不要になり撤去。
                    # ※ cpl の join_asof 化 + 再学習モデルとセットで配置すること
                    #   (旧スパースモデル + ゲート撤去は held を期待しない木に held を流し込む
                    #    train-serve 不整合を生むため厳禁)。
                    # ════════════════════════════════════════════════════
