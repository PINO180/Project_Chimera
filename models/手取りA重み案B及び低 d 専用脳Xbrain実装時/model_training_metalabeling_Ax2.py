...
#   本番と同じパス・同じ出力(S7_M1_OOF_PREDICTIONS_*)。元スクリプトはアーカイブ退避済み前提で上書き運用。
# [XBRAIN 実験] 低d(d < XBRAIN_D_MAX)母集団だけで M1 を焼く。差分は「低dフィルタ」のみ、他は元の Ax2 と同一。
#   目的: 「引くときから(pullback含む・低d=手取り厚)でも PT に届くやつを、L時点の特徴量で
#         見分けられるか」。M1 AUC は通常どおり Cx2 の performance report で出る（A も末尾に low-d AUC を log）。
#   d はエントリー時刻 L+180 で確定する既知量＝実行可能ゲートでリークではない。X は L 基準凍結のまま。
# [1周目: M1 Two-Brain Cross-Validation / 直交分割版]
...
from blueprint import (
    S6_WEIGHTED_DATASET,
    S6_LABELED_DATASET,  # [XBRAIN] np_realized を引くため（S6_WEIGHTED は drop 済み）
...
# [XBRAIN] 低d実験パラメータ
# =========================================================================
PT_MULT = 1.5          # ラベリング RULE の pt_mult（d = pt_mult − np_realized）
XBRAIN_D_MAX = 1.0     # 低d母集団の上限。d < これ を残す（= np_realized > pt_mult − これ = 0.5）。
#   まず広め 1.0（pullback=d<0 も含む「まだ動ききっていない」帯）で開始。
#   AUC が出て純度を上げたければ 0.5 等に締める。


def _apply_lowd_filter(df_chunk: "pl.DataFrame", partition_date, direction: str) -> "pl.DataFrame":
    """[XBRAIN] S6_LABELED から np_realized_{dir} を join して d を復元し、低d(d<XBRAIN_D_MAX)のみ残す。
    - df_chunk は既に is_trigger==1 & unique 済み。join は [timestamp, timeframe] の left。
    - np_realized が無い(=ラベル欠損)行は落とす。d = PT_MULT − np_realized。
    - d は L+180 で確定する既知量＝実行可能ゲート。特徴量には使わず母集団の絞りにのみ使用（リーク無し）。
    """
    np_col = f"np_realized_{direction}"
    lab_glob = str(
        S6_LABELED_DATASET
        / f"year={partition_date.year}/month={partition_date.month}/day={partition_date.day}/*.parquet"
    )
    try:
        lab = pl.read_parquet(lab_glob, columns=["timestamp", "timeframe", np_col])
    except Exception:
        return df_chunk.head(0)  # ラベルが読めなければ空に（このパーティションはスキップ）
    df_chunk = df_chunk.join(lab, on=["timestamp", "timeframe"], how="left")
    df_chunk = df_chunk.filter(
        pl.col(np_col).is_not_null() & ((PT_MULT - pl.col(np_col)) < XBRAIN_D_MAX)
    )
    return df_chunk.drop(np_col)
...
class M1CrossValidator:
    ...
    def _load_features(self, feature_path: Path) -> List[str]:
        ...
        exclude_exact = {
            ...
            "uniqueness_long",
            "uniqueness_short",
            # [TAKEHOME 案B] 最終重み・手取り生値を特徴量から除外
            "w_final_long",
            "w_final_short",
            "np_realized_long",
            "np_realized_short",
    ...
    def _train_model_partition_based(self, direction: str) -> Dict[str, np.ndarray]:
        ...
        label_col = f"label_{direction}"
        weight_col = f"w_final_{direction}"  # [TAKEHOME 案B] 手取り込みの最終重み
        ...
        for i, (train_partitions, val_partitions) in enumerate(
            ...
            if len(train_partitions) > 0:
                for partition_date in tqdm(
                    ...
                    # [XBRAIN] 低d母集団に限定（train）
                    df_chunk = _apply_lowd_filter(df_chunk, partition_date, direction)
                    if df_chunk.is_empty():
                        continue
            ...
            if model is not None:
                for partition_date in tqdm(
                    ...
                    # [XBRAIN] 低d母集団に限定（val = OOF評価も低d内で行う＝「低d内で分離できるか」を測る）
                    df_chunk = _apply_lowd_filter(df_chunk, partition_date, direction)
                    if df_chunk.is_empty():
                        continue
    ...
    def run(self) -> None:
        ...
        for direction in directions:
            ...
            if oof_results["timestamp"].size > 0:
                ...
                # [XBRAIN] go/no-go: 低d母集団での OOF AUC を直接算出
                _valid = oof_df.filter(pl.col("prediction").is_not_nan())
                _y = _valid["true_label"].to_numpy()
                _p = _valid["prediction"].to_numpy()
                if len(_y) > 0 and len(set(_y.tolist())) > 1:
                    _auc = roc_auc_score(_y, _p)
                    _base = float((_y == 1).mean())
                    logging.info("=" * 60)
                    logging.info(
                        f"[XBRAIN RESULT] {direction.upper()} 低d(d<{XBRAIN_D_MAX}) "
                        f"OOF AUC = {_auc:.4f}  (n={len(_y):,}, 正例率={_base:.3f})"
                    )
                    logging.info(
                        "  → AUC が現行M1(0.88)並みなら金脈(低d内で分離可)。"
                        "0.5〜0.6 なら低d内は分離不可＝別設計へ。"
                    )
                    logging.info("=" * 60)
                else:
                    logging.warning("[XBRAIN] AUC 算出不可（正例/負例が揃っていない or 空）")
            else:
