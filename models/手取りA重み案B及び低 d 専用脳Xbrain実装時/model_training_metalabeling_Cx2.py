...
# =========================================================================
# [XBRAIN] 低d実験パラメータ（Ax2 と同一値・同一定義）
#   M1 のみ低d(d<XBRAIN_D_MAX)母集団に限定する。M2 は B(Bx2)由来で既に低dなので触らない。
#   d = PT_MULT - np_realized_{dir}。np_realized は join で S6_WEIGHTED に残す設定に変更済み。
# =========================================================================
PT_MULT = 1.5
XBRAIN_D_MAX = 1.0
...
class FinalAssembler:
    def __init__(self, config: FinalTrainingConfig):
        ...
        if "m1_pred_proba" not in self.features_m2:
            # m1_pred_probaはBx2がjoin時に追加するため_load_featuresで除外される
            # M2学習時には必須のため手動で追加する
            self.features_m2.append("m1_pred_proba")
    ...
    def _filter_dataframe(self, df: pl.DataFrame, is_m1: bool = False) -> pl.DataFrame:
        ...
        # [XBRAIN] M1 のみ低d(d<XBRAIN_D_MAX)母集団に限定（Ax2 と同一の絞り込み）。
        #   d = PT_MULT - np_realized_{dir}。np_realized は S6_WEIGHTED 保持（join で drop 廃止）。
        #   M2(is_m1=False)は B 由来で既に低dのため何もしない。
        #   本フィルタは全 M1 読込サイト(_calculate_scale_pos_weight_m1 / M1 CV / M1 final /
        #   calibration)が本メソッドを通るため、ここ1箇所で全て低dに揃う。
        if is_m1:
            _np_col = f"np_realized_{self.direction}"
            if _np_col in df.columns:
                df = df.filter((PT_MULT - pl.col(_np_col)) < XBRAIN_D_MAX)
    ...
    def _train_single_model(
        ...
        for p_date in tqdm(
            ...
            # [TAKEHOME 案B / (b)両方] M1は w_final_{dir} を直接使用。
            # M2の"uniqueness"列は Bx2 が w_final(手取り込み)を書き込んだもの＝M2も手取りで学習される
            # (列名はuniquenessだが中身はw_final。M2を純粋uniquenessにしたい場合はBx2 L189を戻す)
            weight_col = "uniqueness" if is_m2 else f"w_final_{self.direction}"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script 3/3: M2 CV, Final Training, Calibration, and Reporting (直交分割版)"
    )
