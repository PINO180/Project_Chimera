...
class MetaLabelGenerator:
    ...
    def run(self) -> None:
        ...
        for direction in ["long", "short"]:
            ...
            # [TAKEHOME 案B / (b)両方] M2用メタラベルの重み列に w_final(手取り込み) を流す。
            # ここを w_final にすると下流Cx2のM2学習が手取り重みで動く＝手取りをM1とM2の両方に効かせる。
            # (純粋uniquenessでM2を学習したい場合のみ uniqueness_{direction} に戻す)
            uniqueness_col = f"w_final_{direction}"
            ...
            for partition_date in tqdm(
                ...
                try:
                    ...
                    final_chunk_df = merged_chunk_df.with_columns(
                        ...
                        # 注意: "uniqueness"という列名だが中身は uniqueness_col(=w_final,手取り込み)。
                        # Cx2のM2学習はこの列を重みに使う＝M2も手取りで学習される。
