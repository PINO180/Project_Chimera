...
class M1CrossValidator:
    ...
    def _train_model_partition_based(self, target: str) -> Dict[str, np.ndarray]:
        # [REGRESSION 2026-07-12] target ∈ {"mfe","mae"} → ターゲット列 = {target}_atr。
        ...
        for i, (train_partitions, val_partitions) in enumerate(
            ...
            logging.info(
                f"  [{target.upper()}] Fold {i + 1}/{self.config.n_splits}..."
            )
    ...
    def run(self) -> None:
        ...
        targets = ["mfe", "mae"]
        output_paths = {
            "mfe": S7_M1_OOF_PREDICTIONS_LONG,
            "mae": S7_M1_OOF_PREDICTIONS_SHORT,
        }
        ...
        for target in targets:
            logging.info("\n" + "=" * 50)
            logging.info(f"=== Starting Pipeline for: M1-{target.upper()} ({target}_atr) ===")
            logging.info("=" * 50)
            ...
            if oof_results["timestamp"].size > 0:
                ...
                try:
                    ...
                    logging.info("-" * 50)
                    logging.info(f"### M1-{target.upper()} OOF REGRESSION REPORT ({target}_atr) ###")
                    logging.info(
