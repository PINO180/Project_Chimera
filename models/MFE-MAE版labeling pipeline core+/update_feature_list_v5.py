...
from blueprint import S6_LABELED_DATASET, S3_FEATURES_FOR_TRAINING_V5
...
source_parquet_file = find_latest_partition(S6_LABELED_DATASET)  # 重み付けスキップ: S6_LABELED直読み
...
def create_updated_feature_list(source_file: Path, output_file: Path):
    ...
    try:
        ...
        non_feature_cols = {
            ...
            "session_atr_ratio",  # セッション別ATR Ratio（サフィックス無し=シミュレーター用。_M3 は特徴量として残す）
            # --- V5 双方向ラベリング (Long用) ---
            "label_long",
            "duration_long",
            "uniqueness_long",
            ...
            # --- [MFE/MAE ラベル] 回帰ターゲット群。特徴量に混ぜると答えのリーク。必ず除外。 ---
            "mfe_usd",
            "mae_usd",
            "mfe_atr",
            "mae_atr",
