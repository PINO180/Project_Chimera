...
class FinalAssembler:
    def __init__(self, config: FinalTrainingConfig):
        ...
        # ★★★ [実験②: All Full 304モード] ★★★
        # M1用の特徴量 (Base) は 304個 (txtから読み込み)
        self.features_base: List[str] = self._load_features()

        # M2用の特徴量も 304個 (+ m1_pred_proba) に変更
        # Top 50 ではなく features_base をそのまま流用する
        self.features_m2: List[str] = ["m1_pred_proba"] + self.features_base
        # -----------------------------------------------------------------------
