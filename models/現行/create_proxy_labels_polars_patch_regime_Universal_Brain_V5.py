# ============================================================================
# [PROTOTYPE GEOMETRY]  自動生成ファイル — 直接編集しないこと
#
#   生成元 : create_proxy_labels_polars_patch_regime_Universal_Brain_V5.py
#   生成器 : patch_labeling_to_prototype_geometry.py
#
#   プロトタイプ脳 (2025-11 / create_proxy_labels_polars_patch_regime1.py)
#   の幾何を、現行パイプラインに移植したもの。
#
#     TARGET_TIMEFRAMES  : ["M3"] -> ["M1"]
#     ACTION_HORIZON_SEC : 180 -> 60
#     ATR_PERIOD         : 13 -> 21
#     ゲート              : atr_ratio >= 0.0 -> atr_value >= 5.0 USD
#     PT / SL            : 1.4 / 1.4 -> 1.0 / 5.0
#     TD                 : "60m" -> "1200m"
#     SPREAD             : 0.0 のまま (regime1 もラベリングにスプレッドを入れていない)
#
#   変更していないもの:
#     - 特徴量の計算 (engine 再実行不要)
#     - LOOKAHEAD-FIX / ENTRY-ANCHOR FIX の構造
#       ACTION_HORIZON_SEC を 60 にしたことで、
#       高TF の閉じたバー再ラベルは tf > 60s の全TFに適用される。
#     - 出力スキーマ (label_long / label_short / duration_* / is_trigger 等)
#
#   ★ 下流で合わせる必要があるもの (このスクリプトは触らない):
#     1. blueprint.BARRIER_ATR_PERIOD  13 -> 21
#        (BT と 本番 calculate_barrier_atr が同じ ATR を使うため)
#     2. BT の USER_PARAMS
#        pt_mult=1.0 / sl_mult=5.0 / td_minutes=1200 / min_atr は絶対値運用
#     3. split_features_first_orthogonal.py の EXCLUDE_TIMEFRAME_SUFFIXES
#        M1 がトリガー足になるため "_M1" を除外していないか要確認
#     4. 出力先 S6_LABELED_DATASET
#        既存データを潰したくない場合は blueprint 側で退避すること
# ============================================================================
...
# 対象タイムフレームとATRの指定
TARGET_TIMEFRAMES = ["M1"]  # [PROTO] プロトタイプ脳の基準足 (毎分判定)
#   旧: ["M3"]  Optunaの結論: M3単体・ratio0.8・TD30min
#   刻みを細かくするほど、動き始めに近い時点で入れる。
#   プロトタイプは MT5 に30秒足が無く M1 が最短だった (歴史的制約)。
#   ※ ACTION_HORIZON_SEC も 60 に連動させること (下記)。
ATR_PERIOD = 21  # [PROTO] プロトは e1c_atr_21 を使用
#   旧: ATR_PERIOD = BARRIER_ATR_PERIOD (= 13, blueprintから取得)
#   ATR_ABS_THRESHOLD = 5.0 は ATR(21) に対して較正された閾値のため期間も揃える。
#   ★ 下流に波及する: BT の ATR 期間、本番 calculate_barrier_atr の
#      BARRIER_ATR_PERIOD も 21 に揃えないと Train-Serve が割れる。
# =============================================================================
# ATR Ratio ゲート
# -----------------------------------------------------------------------------
...
# =============================================================================
ATR_RATIO_THRESHOLD = 0.0  # [PROTO] 相対比ゲートは使わない (0.0 = 素通し)

# =============================================================================
# [PROTO] 絶対 ATR ゲート
# -----------------------------------------------------------------------------
#   プロトタイプ脳の母集団選抜。相対比ではなく USD 建ての絶対値で切る。
#     regime1 L64  : ATR_REGIME_CUTOFF = 5.0
#     regime1 L858 : filter(atr_value >= ATR_REGIME_CUTOFF)
#
#   相対比 (atr_ratio) は「その銘柄のいつもと比べて」を測る。
#   絶対値は「スプレッドに対して十分大きいか」を測る。
#   コストが USD 固定である以上、経済性に直結するのは後者。
#
#   注意: e1c_atr_21 は現行エンジンでは相対化済み(scale_by_atr)のため使えない。
#         本スクリプトが S1_PROCESSED の OHLCV から自前計算する
#         atr_value (Wilder・disc-aware・USD建て) を使う。
#
#   閾値の換算 (別TFを試す場合の目安。ランダムウォーク近似 sqrt(t)):
#       M0.5 : 5.0 / sqrt(2) = 3.54
#       M1   : 5.0            <- プロトタイプの実値
#       M3   : 5.0 * sqrt(3) = 8.66
#   ただし実際の指数は測るまで不明。原則 M1 = 5.0 のまま使うこと。
# =============================================================================
ATR_ABS_THRESHOLD = 5.0  # [PROTO] 絶対 ATR(21) の下限 (USD)
...
# ロング用ルール
# [PROTO] プロトタイプ幾何 (regime1 REGIME_RULE_R4)
#   PT = ATR x 1.0 (浅い) / SL = ATR x 5.0 (深い) / TD = 1200分 (20時間)
#
#   ★ 既知の副作用 — 対称化で塞いだ「穴1」が戻る:
#     非対称(1:5)では PT が近いため、高ボラのバーで long も short も
#     label=1 になる。無ドリフトMC実測でボラ標準18% / ボラ2倍52%。
#     プロトタイプは Long 専用 (regime1 L961 direction=1固定) だったので
#     この穴は問題にならなかった。
#     両方向で学習する場合、short 側のラベルはこの汚染を受ける。
#     → BT/評価は Long 側で見るか、DIRECTION_MODE を検討すること。
#
#   ★ 幾何だけの基準値 (無ドリフトRW): P(PT先着) = 5/(1+5) = 83.3%
#     モデルが無くてもこの勝率は出る。BT の読み方の基準線にすること。
RULE_LONG = {
    "pt_mult": 1.0,   # [PROTO] 旧 1.4
    "sl_mult": 5.0,   # [PROTO] 旧 1.4
    "td": "1200m",  # [PROTO] 旧 "60m"
}
...
class ProxyLabelingEngine:
    ...
    # =========================================================================
    ACTION_HORIZON_SEC = 60  # [PROTO] TARGET_TIMEFRAMES=["M1"] に連動 (旧: 180)
    ...
    def _calculate_labels_for_batch(
        ...
        for timeframe_tuple, group_df in daily_bets_df.group_by("timeframe"):
            ...
            bets_df_all = bets_df_with_atr.with_columns(
                # [PROTO] 相対比ではなく絶対 ATR (USD) でゲートする
                #   regime1 L858: filter(atr_value >= ATR_REGIME_CUTOFF)
                #   旧: pl.when(pl.col("atr_ratio") >= ATR_RATIO_THRESHOLD)
                pl.when(pl.col("atr_value") >= ATR_ABS_THRESHOLD)
                .then(1)
                .otherwise(0)
                .cast(pl.Int8)
                .alias("is_trigger")
