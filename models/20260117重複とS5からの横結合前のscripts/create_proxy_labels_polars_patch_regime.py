...
# [旧互換用] R4レジームの最小ATR値 (レポート表示等で参照される場合のフォールバック)
ATR_REGIME_CUTOFF = 5.0

# [新規] 変動ATRフィルターの基準比率
# 2021年基準 ($1800) での ATR 5.0 相当 = 5.0 / 1800 ≒ 0.0028 (0.28%)
# 現在価格に応じて必要なATR値が動的に変動する (例: $4600なら ATR > 12.88)
        ...
        try:
            ...
            for row in tqdm(
                ...
                if daily_labeled_df is not None and not daily_labeled_df.is_empty():
                    # ==============================================================================
                    # ★★★ [緊急パッチ] マシンガン・エントリー防止のための強制重複排除 ★★★
                    # ==============================================================================
                    # Timestamp と Timeframe の組み合わせが重複している場合、最初の1行だけを残して他を捨てる
                    daily_labeled_df = daily_labeled_df.unique(
                        subset=["timestamp", "timeframe"], keep="first"
                    )
                    # ==============================================================================
