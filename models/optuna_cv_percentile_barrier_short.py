...
ATR_THRESHOLDS = [0.8, 1.0, 1.2, 1.5, 1.8, 3.0]

# パーセンタイル・バリア設計
# q_pt: PTを過去変動幅分布の何パーセンタイルに置くか
# q_sl: SLを過去変動幅分布の何パーセンタイルに置くか
# 先入観なしにフラットに探索（損小利大・損大利小どちらも許容）
Q_PT_VALUES = [0.50, 0.60, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95]
Q_SL_VALUES = [0.50, 0.60, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 0.99]
TD_MINS = [
    # 5,
    10,
    15,
...
def create_objective(cv_folds, data_loader, target_tf, spread_cost):
    def objective(trial):
        ...
        q_pt = trial.suggest_categorical("q_pt", Q_PT_VALUES)
        q_sl = trial.suggest_categorical("q_sl", Q_SL_VALUES)
        ...
        for fold_idx, (_, test_dates) in enumerate(cv_folds):
            ...
            if len(valid_indices) < MIN_TOTAL_BETS_PER_FOLD:
                ...
                # パーセンタイル・バリア（ショート用：過去N本からの動的ローリング算出）
                window = timeframe_bars_per_day.get(tf_choice, 1440)
                rolling_pt = (
                    pl.Series(sig_atr)
                    .rolling_quantile(
                        quantile=q_pt, window_size=window, interpolation="linear"
                    )
                ...
                rolling_sl = (
                    pl.Series(sig_atr)
                    .rolling_quantile(
                        quantile=q_sl, window_size=window, interpolation="linear"
                    )
                    .fill_null(strategy="forward")
                    .to_numpy()
                )
                bets_pt_width = rolling_pt[mask][valid_indices]
                bets_sl_width = rolling_sl[mask][valid_indices]

                bets_pt = np.ascontiguousarray(bets_close - bets_pt_width - spread_cost)
                bets_sl = np.ascontiguousarray(bets_close + bets_sl_width - spread_cost)
                ...
                total_profit = float(np.nansum(net_pnl[financial_is_win]))
                total_loss_adj = float(np.nansum(np.abs(net_pnl[financial_is_loss])))
                timeout_np = float(np.nansum(net_pnl[is_timeout]))
                ...
                total_sum_atr_all += float(np.nansum(bets_atr))
                total_net_profit_all += float(np.nansum(net_pnl))
        ...
        if total_bets_all > 0:
            pt_pct = total_wins_all / total_bets_all * 100
            sl_pct = total_losses_all / total_bets_all * 100
            to_pct = total_timeouts_all / total_bets_all * 100
        else:
            pt_pct = sl_pct = to_pct = 0.0
        ...
        trial.set_user_attr("PT%", round(pt_pct, 2))
        trial.set_user_attr("SL%", round(sl_pct, 2))
        trial.set_user_attr("TO%", round(to_pct, 2))
...
def run_optimization():
    ...
    for current_spread in SPREAD_COSTS:
        ...
        for target_tf in SEARCH_TIMEFRAMES:
            ...
            # PT%・SL%・TO%を追加
            top_100["PT%"] = (top_100["Wins"] / top_100["Total_Bets"] * 100).round(2)
            top_100["SL%"] = (top_100["Losses"] / top_100["Total_Bets"] * 100).round(2)
            top_100["TO%"] = (top_100["Timeouts"] / top_100["Total_Bets"] * 100).round(
                2
            )
