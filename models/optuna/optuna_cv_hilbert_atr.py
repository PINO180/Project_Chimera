...
RESULTS_CSV_PATH = Path(
    "/workspace/data/XAUUSD/stratum_7_models/1A_2B/optuna_top100_hilbert_atr_results_.csv"
)
...
def create_objective(cv_folds, data_loader, target_tf):
    def objective(trial):
        ...
        for fold_idx, (_, test_dates) in enumerate(cv_folds):
            ...
            # フィルタリング
            mask = (
                (sig_trigger >= tr) & (sig_spread > (sig_atr * atr_f)) & (sig_atr > 0)
            )
            if not np.any(mask):
                fold_scores.append(0.0)
                continue
...
def run_optimization():
    ...
    for target_tf in SEARCH_TIMEFRAMES:
        ...
        # ★ 修正: サンプラーを定義し、序盤のランダム探索を増やして相関を考慮させる
