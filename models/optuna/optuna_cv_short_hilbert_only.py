# /workspace/models/optuna_cv_hilbert_only_short.py
...
# 【修正要件4】 ショート専用データとして保存名を変更
RESULTS_CSV_PATH = Path(
    "/workspace/data/XAUUSD/stratum_7_models/1A_2B/optuna_top100_hilbert_only_results_short.csv"
)
...
# ====================================================================
# Numba JIT 高速トリプルバリア関数 (SHORT専用に反転)
# ====================================================================
...
def _numba_find_hits_fast(
    ...
    for i in prange(n_bets):
        ...
        for j in range(start_idx, n_ticks):
            ...
            # 【修正要件2】ショート判定: 安値がPT（下）以下になれば利確、高値がSL（上）以上になれば損切
...
def create_objective(cv_folds, data_loader, target_tf):
    def objective(trial):
        ...
        total_np_all = 0.0  # 追加: 全トレードの純利益
        timeout_np_all = 0.0  # 追加: Timeoutトレードの損益
        ...
        for fold_idx, (_, test_dates) in enumerate(cv_folds):
            ...
            # --- フィルタリング条件 ---
            # 【修正要件3】ヒルベルト振幅(エネルギー)は非方向性のため条件維持
            ...
            if len(valid_indices) == 0:
                ...
                total_np_all += fold_np  # 追加: 純利益を累積
                timeout_np_all += fold_timeout_np  # 追加: Timeout損益を累積
...
def run_optimization():
    ...
    for target_tf in SEARCH_TIMEFRAMES:
        ...
        logging.info(
            f"=== Starting SHORT Optuna Optimization for {target_tf} (Trials: {N_TRIALS}) ==="
        )
