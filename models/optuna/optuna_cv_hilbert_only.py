# /workspace/models/optuna_cv_hilbert_only.py
...
RESULTS_CSV_PATH = Path(
    "/workspace/data/XAUUSD/stratum_7_models/1A_2B/optuna_top100_hilbert_only_results.csv"
)
...
TD_MINS = [3, 5, 10, 15, 30, 60, 120, 180, 360, 720, 1080, 1200]
...
# Optunaに選択させるタイムフレーム（"mixed"を動的に追加）
...
# ====================================================================
# 2段構えデータローダークラス (Tickキャッシュ / シグナルキャッシュ分離)
# ====================================================================
class FoldDataLoader:
    def __init__(self, base_tick_dir: Path):
        self.base_tick_dir = base_tick_dir
        self.tick_cache = {}  # 巨大なTickデータ用 (キー: 期間)
        self.sig_cache = {}  # 軽量なシグナルデータ用 (キー: 期間 + TF + ATR)
    ...
    def get_fold_data(self, test_dates, tf, atr_p):
        ...
        sig_cache_key = f"{tick_cache_key}_{tf}_{atr_p}"

        # --- 【超高速化】事前チェック: ファイルが存在するか最初に1回だけ探す ---
        if sig_cache_key not in self.sig_cache:
            hilbert_path, actual_hilbert = None, None
            atr_path, actual_atr = None, None

            # rglobを1回だけ回し、見つかった瞬間に探索を打ち切る (最速ロジック)
            for p in S2_FEATURES_FIXED.rglob(f"*{tf}*"):
                if "tick" in p.name:
                    continue
                ...
                scan_target = (
                    ...
                    else (str(p / "**/*.parquet") if p.is_dir() else None)
                )
                if not scan_target:
                    continue

                try:
                    cols = pl.scan_parquet(scan_target).collect_schema().names()
                    if not hilbert_path:
                        h_col = next((c for c in cols if HILBERT_COL in c), None)
                        if h_col:
                            hilbert_path, actual_hilbert = scan_target, h_col

                    if not atr_path:
                        a_col = next((c for c in cols if f"atr_{atr_p}" in c), None)
                        if a_col:
                            atr_path, actual_atr = scan_target, a_col

                    if hilbert_path and atr_path:
                        break  # 両方見つかったら即ループ終了
                except Exception:
                    pass

            if not hilbert_path or not atr_path:
                logging.warning(f"    -> [Error] Missing Hilbert or ATR for TF:{tf}.")
                self.sig_cache[sig_cache_key] = None
                return None

            # --- 1. Tickデータのキャッシュ層 ---
            if tick_cache_key not in self.tick_cache:
                logging.info(
                    f"    [Tick Cache Miss] Loading Tick data for {start_date.date()} to {end_date.date()}..."
                )
                ...
                if df_tick.is_empty():
                    self.tick_cache[tick_cache_key] = None
                else:
                    ...
                    df_tick_close = df_tick.select(["timestamp", "close"])
            ...
            if self.tick_cache[tick_cache_key] is None:
                self.sig_cache[sig_cache_key] = None
                return None

            ticks_ts, ticks_high, ticks_low, df_tick_close = self.tick_cache[
                tick_cache_key
            ]

            # --- 2. シグナルデータのロードと結合 ---
            logging.info(
                f"    [Signal Cache Miss] Loading S2/ATR for {tf}, ATR={atr_p}..."
            )

            lf_s5 = (
                pl.scan_parquet(hilbert_path)
            ...
            lf_sig = lf_s5.join_asof(
                df_tick_close.lazy().sort("timestamp"),
                on="timestamp",
                strategy="backward",
            )
            ...
            lf_sig = (
                ...
                .drop_nulls(subset=["close", "trigger_ratio_val", f"atr_{atr_p}"])
            ...
            if df_sig.is_empty():
                ...
                self.sig_cache[sig_cache_key] = (
                    np.ascontiguousarray(df_sig["ts_int"].to_numpy()),
                    np.ascontiguousarray(df_sig["close"].to_numpy()),
                    np.ascontiguousarray(df_sig["trigger_ratio_val"].to_numpy()),
                    np.ascontiguousarray(df_sig[f"atr_{atr_p}"].to_numpy()),
                )

        if self.sig_cache[sig_cache_key] is None:
            return None

        sig_ts, sig_close, sig_trigger, sig_atr = self.sig_cache[sig_cache_key]
        ticks_ts, ticks_high, ticks_low, _ = self.tick_cache[tick_cache_key]

        return sig_ts, sig_close, sig_trigger, sig_atr, ticks_ts, ticks_high, ticks_low
...
def create_objective(cv_folds, data_loader, target_tf):
    def objective(trial):
        ...
        total_net_profit_all = 0.0  # 追加: 全トレードの純利益
        timeout_net_profit_all = 0.0  # 追加: Timeoutトレードの損益

        for fold_idx, (_, test_dates) in enumerate(cv_folds):
            if tf_choice == "mixed":
                m_ts, m_close, m_trigger, m_atr = [], [], [], []
                ticks_data = None
                for t in TIMEFRAMES:
                ...
                sig_ts, sig_close, sig_trigger, sig_atr = (
                    sig_ts[sort_idx],
                    sig_close[sort_idx],
            ...
            # --- フィルタリング条件 (Hilbert単体ロジック) ---
            mask = (sig_trigger >= tr) & (sig_atr > 0)
            if not np.any(mask):
                fold_scores.append(0.0)
                continue
            ...
            if len(valid_indices) == 0:
                ...
                fold_np = total_profit - total_loss_adj  # 追加: このフォールドの純利益
                fold_timeout_np = -(
                    timeouts * SPREAD_COST
                )  # 追加: このフォールドのTimeout損益
                ...
                total_net_profit_all += fold_np  # 追加: 純利益を累積
                timeout_net_profit_all += fold_timeout_np  # 追加: Timeout損益を累積
        ...
        trial.set_user_attr(
            "Total_Net_Profit", total_net_profit_all * 100 * 150
        )  # 追加: 円換算の合計純利益
        trial.set_user_attr(
            "Timeout_Net_Profit", timeout_net_profit_all * 100 * 150
        )  # 追加: 円換算のTimeout損益
        ...
        if total_bets_all < MIN_TOTAL_BETS:
            return 0.0
