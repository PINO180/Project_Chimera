...
class BacktestConfig:
    ...
    max_risk_per_trade: float = 0.02  # ★★★ 50 (5000%) -> 0.5 (50%) に修正 ★★★
...
class BacktestSimulator:
    def __init__(self, config: BacktestConfig):
        ...
        # ★★★ [実験③: 精鋭選抜モード (33-31) 用修正] ★★★

        # 1. M1用の特徴量 (Top 33)
        m1_features_path = project_root / "models" / "TOP_M1_33_FEATURES.json"
        self.features_base = self._load_features_from_path(m1_features_path)
        logging.info(
            f"M1 Features: Loaded {len(self.features_base)} items from {m1_features_path.name}."
        )

        # 2. M2用の特徴量 (Top 31 + m1_pred_proba)
        m2_features_path = project_root / "models" / "TOP_M2_31_FEATURES.json"
        raw_m2_features = self._load_features_from_path(m2_features_path)
        self.features_m2 = ["m1_pred_proba"] + raw_m2_features
        logging.info(
            f"M2 Features: Loaded {len(self.features_m2)} items (including score) from {m2_features_path.name}."
        )

        # --- [修正 2] セントモード時の内部パラメータ変換 ---
    ...
    # ★ 汎用的な読み込みメソッド (JSON/TXT自動判別)
    def _load_features_from_path(self, path: Path) -> List[str]:
    ...
    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        ...
        if not self.config.oof_mode:  # In-Sample Mode
            ...
            required_cols_set = set(self.features_base)  # M1 features
            required_cols_set.update(base_cols)

            # --- [修正: 実験パターン③] M2用の特徴量もロード対象に追加 ---
            # features_m2 のうち "m1_pred_proba" は計算で出るので除外してロード
            m2_raw_features = [f for f in self.features_m2 if f != "m1_pred_proba"]
            required_cols_set.update(m2_raw_features)
    ...
    def _run_simulation_loop(
        ...
        # ★★★ 修正ここまで ★★★
        # --- [修正 1: セントモードなら契約サイズも100倍(セント単位)にする] ---
        if self.config.is_cent_mode:
...
if __name__ == "__main__":
    ...
    config = BacktestConfig(
        ...
        is_cent_mode=args.cent_mode,  # ★ 追加
    )
    # --- ★★★ 修正ここまで ★★★ ---
