...
class BacktestConfig:
    ...
    # ▼▼▼ 追加 ▼▼▼
    fixed_lot_size: float = 0.01  # 固定ロットモード用 (0.0より大きい場合有効)
    # ▲▲▲ 追加 ▲▲▲
...
class BacktestSimulator:
    ...
    def _run_simulation_loop(
        ...
        for i in range(len(df_chunk)):
            ...
            # ▼▼▼ 修正ブロック開始 ▼▼▼
            DECIMAL_FIXED_LOT = Decimal(str(self.config.fixed_lot_size))
            is_fixed_lot_mode = DECIMAL_FIXED_LOT > DECIMAL_ZERO

            # 取引判断とベット比率の決定
            if is_fixed_lot_mode:
                # 固定ロットモード: Kellyを無視し、閾値だけで判断
            ...
            if should_trade:
                if is_fixed_lot_mode:
                    # ★ 固定ロット適用
                    desired_lot_size_decimal = DECIMAL_FIXED_LOT
                    # リスク額の逆算（後のPnL計算のため）
                    # 注: SL幅などはATR依存で変動するが、ロットは固定される
                    if (
                        atr_value_float is not None
                        and np.isfinite(atr_value_float)
                        and sl_multiplier_float is not None
                        and np.isfinite(sl_multiplier_float)
                    ):
                ...
                elif effective_bet_fraction > DECIMAL_ZERO:
                    # ★ 既存のKellyロジック
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=0.01,
        dest="fixed_lot",
        help="Fixed lot size (e.g. 0.1). If > 0, ignores Kelly and uses this lot size.",
    )
