...
# --- 設定 ---
# 入力は「KS検定後」のディレクトリ
INPUT_DIR = Path(blueprint.S2_FEATURES_AFTER_KS)
# 出力は「AV検定後」のディレクトリ
OUTPUT_DIR = Path(blueprint.S2_FEATURES_AFTER_AV)
# 不安定リストは「AV検定」の結果を使用
UNSTABLE_LIST_PATH = Path(blueprint.S3_ARTIFACTS) / "av_unstable_features.json"
...
def load_unstable_features(path: Path) -> set:
    ...
    logger.info(f"{len(unstable_set)}個の不安定特徴量（AV）を読み込みました。")
    return unstable_set
...
def process_tick_directory(input_path: Path, output_path: Path, unstable_set: set):
    """パーティション化されたtickディレクトリを処理する（ストリーミング処理）"""
    try:
        ...
        for partition_file_path in all_input_partitions:
...
def main():
    """メイン実行関数"""
    logger.info("--- AV検定後の安定特徴量データセット作成開始 ---")
