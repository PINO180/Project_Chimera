...
class RealtimeFeatureEngine:
    ...
    def __init__(self, feature_list_path: str = str(config.S3_FEATURES_FOR_TRAINING)):
        ...
        # 1. 特徴量名簿(final_feature_set_v2.txt)をロード
        # 出口規制: このリストに含まれる特徴量のみを出力ベクトルに含める
        try:
            ...
            self.logger.info(
                f"特徴量名簿 ({len(self.feature_list)}個) をロードしました。PATH: {feature_list_path}"
            )
            # 出口規制用のターゲット特徴量リストとして保持
            self.target_features = self.feature_list
        ...
        for tf_name in self.ALL_TIMEFRAMES.keys():
            ...
            # --- [修正箇所] 必須バッファの強制確保 (M1:ベース, D1:文脈) ---
            if tf_name not in self.lookbacks_by_tf:
                if tf_name == "M1":
                    # M1はシステム稼働のベースクロックとして必須
                    lookback = 3000
                    self.lookbacks_by_tf["M1"] = lookback
                    self.logger.info(
                        f"  -> {tf_name} (SYSTEM_BASE) バッファ初期化 (Lookback: {lookback})"
                    )
                elif tf_name == "D1":
                    # D1は市場文脈(HMM/Regime)計算に必須 (KeyError: 'D1' 防止)
                    lookback = 1000
                    self.lookbacks_by_tf["D1"] = lookback
                    self.logger.info(
                        f"  -> {tf_name} (CONTEXT_BASE) バッファ初期化 (Lookback: {lookback})"
                    )
                else:
                    # その他の不要な時間足(M3, M5など)はバッファ自体を作らない (軽量化)
                    self.logger.debug(
                ...
                self.logger.info(
                    f"  -> {tf_name} バッファ初期化 (Lookback: {lookback})"
                )
        ...
        # ▼▼▼【ここに追加してください】▼▼▼
        # --- [V12.2 追加] シグナル重複排除用: {tf_name: last_signal_timestamp} ---
        self.last_emitted_signal_ts: Dict[str, pd.Timestamp] = {}
        # ▲▲▲【ここまで】▲▲▲
        ...
        # ★★★ [追加] マルチタイムフレーム(MTF)横断参照用のキャッシュストア ★★★
        # ここに全ての時間足の最新の特徴量(Top 50)が保存され、共有される
        self.latest_feature_store: Dict[str, float] = {}
    ...
    def fill_all_buffers(
        ...
        4. [V12.0 追加] 起動時ウォームアップ（初期計算）を実行
        """
        self.logger.info(
        ...
        # [V12.0] OLSコミット状態管理用フラグ (初期充填データは全てコミット済みとする)
        self.last_bar_is_committed = {}

        for tf_name, rule in self.TF_RESAMPLE_RULES.items():
            if tf_name not in self.data_buffers:
                continue  # (e.g., 'M30' が名簿になければスキップ)

            # 初期状態はTrue (履歴データは確定足のみと仮定)
            self.last_bar_is_committed[tf_name] = True
        ...
        # M1もコミット済みとする
        self.last_bar_is_committed["M1"] = True

        # 5. [V12.0 追加] ウォームアップ: 全時間足の特徴量を一度計算しておく
        # これにより、process_new_m1_bar の初回実行時から全TFのデータが揃った状態になる
        self.logger.info(
            "起動時ウォームアップ: 全時間足の特徴量を計算してキャッシュを初期化します..."
        )
        current_time = m1_history_pd.index[-1]

        # 計算可能な全ての時間足について実施
        for tf_name in self.lookbacks_by_tf.keys():
            if tf_name in self.data_buffers and self.is_buffer_filled[tf_name]:
                # その時間足の最新時刻を取得
                last_ts = self.last_bar_timestamps.get(tf_name, current_time)
                # 強制計算 (結果は latest_feature_store に保存される)
                _ = self.calculate_feature_vector(tf_name, last_ts, market_proxy_cache)

        self.logger.info("✓ 全バッファの初期充填とウォームアップが完了しました。")
    ...
    def _update_buffer_with_developing_bar(
        self,
        tf_name: str,
        ...
        """
        [V12.1 修正] Developing Bar バッファ更新 (堅牢化版)
        - タイムスタンプ比較を厳密化 (UTC統一)
        - 異常な巻き戻りを防止
        """
        # 入力バーの時刻を正規化
        new_ts = latest_bar_df.index[0]
        if new_ts.tzinfo is None:
            new_ts = new_ts.tz_localize("UTC")
        else:
            new_ts = new_ts.tz_convert("UTC")

        last_ts = self.last_bar_timestamps.get(tf_name)

        # 保存されているlast_tsも念のため正規化して比較
        ...
        # 初回登録
        if last_ts is None:
            self._append_bar_to_buffer(tf_name, latest_bar_df, market_proxy_cache)
            self.last_bar_is_committed[tf_name] = False
            self.last_bar_timestamps[tf_name] = new_ts  # タイムスタンプ保存更新
            return new_ts

        # Case 1: 時刻が同じ -> "Developing" (上書き更新)
        if new_ts == last_ts:
            # バッファが空でないことを確認してPop
            if self.data_buffers[tf_name]["close"]:
                # OHLCV削除
                for col in self.OHLCV_COLS:
                    self.data_buffers[tf_name][col].pop()

                # 純化バッファ削除 (Developing中は未コミットなのでPop可)
                for feat in self.proxy_feature_buffers[tf_name]:
                    if self.proxy_feature_buffers[tf_name][feat]:
                        self.proxy_feature_buffers[tf_name][feat].pop()

            # 新しい値をAppend
            bar_dict = latest_bar_df.iloc[0].to_dict()
            for col in self.OHLCV_COLS:
                self.data_buffers[tf_name][col].append(bar_dict[col])

            # 純化用特徴量 (OLS更新なし)
            latest_proxies = self._calculate_proxy_features_incremental(
                tf_name, latest_bar_df
            )

            # プロキシ取得
            try:
                idx = market_proxy_cache.index.get_indexer([new_ts], method="ffill")[0]
            ...
            for feat, val in latest_proxies.items():
                self.proxy_feature_buffers[tf_name][feat].append(val)
                if feat == "atr":
        ...
        # Case 2: 新しい時刻 -> "Committed" & New Bar
        elif new_ts > last_ts:
            # 直前のDevelopingバーをコミット
            if not self.last_bar_is_committed.get(tf_name, True):
                self._commit_last_bar_to_ols(tf_name)
            ...
            # タイムスタンプ更新
            self.last_bar_timestamps[tf_name] = new_ts
            ...
            # 新しいバーはまだDeveloping状態
            self.last_bar_is_committed[tf_name] = False

        else:
            # Case 3: 過去の時刻 (new_ts < last_ts) -> 異常データ
            # ログを出して無視する（データの巻き戻りはバッファ破壊の原因になるため）
            self.logger.warning(
                f"[{tf_name}] 過去のタイムスタンプを受信したため無視します: "
                f"New={new_ts} < Last={last_ts}"
            )

        return new_ts

    def _commit_last_bar_to_ols(self, tf_name: str):
        """[Helper] バッファ末尾のデータをOLS状態に正式コミットする"""
        try:
            for feat_name in self.ols_state[tf_name].keys():
                y_deque = self.proxy_feature_buffers[tf_name][feat_name]
                x_deque = self.proxy_feature_buffers[tf_name]["market_proxy"]

                if not y_deque or not x_deque:
                    continue
            ...
            self.last_bar_is_committed[tf_name] = True
            self.logger.debug(f"[{tf_name}] OLS Committed.")  # Debugログ追加

        except Exception as e:
            self.logger.error(f"OLS Commit Error ({tf_name}): {e}")
    ...
    def process_new_m1_bar(
        ...
        [V12.2 修正]
        - シグナル重複排除 (Deduping) を追加。
          同一の足(timestamp)で既にシグナルを発出済みの場合は無視する。
        """
        signal_list: List[Signal] = []

        try:
            # 1. タイムスタンプの正規化
            raw_ts = m1_bar.get("timestamp")
            if raw_ts is None:
                self.logger.warning(
                    "M1 Barにtimestampが含まれていません。スキップします。"
                )
                return []

            # pd.Timestampに変換し、tz-naiveならUTC化、tz-awareならUTC変換
            try:
                current_ts = pd.Timestamp(raw_ts)
                ...
                # m1_bar内の値も更新しておく
                m1_bar["timestamp"] = current_ts
            except Exception as e:
                self.logger.error(f"タイムスタンプ変換エラー: {raw_ts} -> {e}")
                return []

            # 2. M1バッファ更新 (M1はBaseなので常にUpdate/Append)
            self.m1_dataframe.append(m1_bar)

            # M1自身のバッファ更新
            m1_bar_df = pd.DataFrame([m1_bar]).set_index("timestamp")
            self._update_buffer_with_developing_bar("M1", m1_bar_df, market_proxy_cache)

            # 3. リサンプリング & 強制更新
            # 直近のデータを抽出 (パフォーマンス対策)
            subset_len = 2000
            if len(self.m1_dataframe) > subset_len:
                m1_subset = list(self.m1_dataframe)[-subset_len:]
            else:
                m1_subset = list(self.m1_dataframe)

            # DataFrame化と型強制
            m1_subset_df = pd.DataFrame(m1_subset)
            m1_subset_df["timestamp"] = pd.to_datetime(
                m1_subset_df["timestamp"], utc=True
            )
            m1_subset_df = m1_subset_df.set_index("timestamp").sort_index()

            # チェック対象の時間足リスト
            target_tfs = [
                tf for tf in self.TF_RESAMPLE_RULES.keys() if tf in self.data_buffers
            ]
            target_tfs.append("M1")
            ...
            for tf_name in target_tfs:
                ...
                if tf_name != "M1":
                    rule = self.TF_RESAMPLE_RULES[tf_name]
                    try:
                        # リサンプリング実行
                        resampled = (
                            m1_subset_df.resample(rule, closed="left", label="left")
                            .agg(
                                {
                                    "open": "first",
                                    "high": "max",
                        ...
                        if not resampled.empty:
                            ...
                            check_ts = latest_row.index[0]  # リサンプリング後の時刻

                            # バッファを更新
                            self._update_buffer_with_developing_bar(
                                tf_name, latest_row, market_proxy_cache
                            )
                    except Exception as e:
                        self.logger.warning(f"{tf_name} Resample Fail: {e}")
                        continue

                # 4. 特徴量計算 & Store更新
                r4_check = self._check_for_signal(tf_name, check_ts)

                if r4_check["is_r4"]:
                    # --- [V12.2] 重複排除 (DEDUP CHECK) ---
                    # この時間足で、この時刻のシグナルを既に出していればスキップ
                    last_emitted = self.last_emitted_signal_ts.get(tf_name)
                    if last_emitted == check_ts:
                        continue  # 連打防止：すでにこの足で発火済み
                    # ------------------------------------

                    feature_vector = self.calculate_feature_vector(
                        tf_name, check_ts, market_proxy_cache
                    )
                    ...
                    if feature_vector is not None:
                        signal = Signal(
                            ...
                            timestamp=check_ts,
                            timeframe=tf_name,
                            market_info=r4_check["market_info"],
                        )
                        signal_list.append(signal)

                        # --- [V12.2] 発火済みとして記録 ---
                        self.last_emitted_signal_ts[tf_name] = check_ts
    ...
    def calculate_feature_vector(
        ...
        """
        [修正] 全知全能化 (MTF Cross-Reference)
        指定された時間足(tf_name)の特徴量を計算してキャッシュを更新し、
        キャッシュにある「全時間足の最新値」を使ってベクトルを構築する。
        これにより、H1の判定時でもH4等の最新値を正しく参照できる。
        ...
        Returns:
            横断的参照済みの特徴量ベクトル (Numpy配列)、またはエラーの場合は None
        """
        if not self.is_buffer_filled[tf_name]:
        ...
        try:
            ...
            # 3. アルファの純化 (Neutralization)
            neutralized_features = self._calculate_neutralized_features(
                base_features, tf_name, timestamp, market_proxy_cache
            )

            # -----------------------------------------------------------------
            # 4. [修正] 全知全能化 (MTF Cross-Reference) & 出口規制
            # -----------------------------------------------------------------

            # ステップA: 今計算した時間足(tf_name)の特徴量を、共有キャッシュ(store)に書き込む
            # これにより、後で他の時間足が判定する際に、この最新値が参照されるようになる
            for target_feature in self.target_features:
                if target_feature.endswith(f"_{tf_name}"):
                    # 名前からベース名を抽出
                    if "_neutralized_" in target_feature:
                        base_name = target_feature.split("_neutralized_")[0]
                    else:
                        base_name = target_feature.rsplit("_", 1)[0]

                    # 計算結果があればキャッシュを更新
                    if base_name in neutralized_features:
                        val = neutralized_features[base_name]
                        if np.isnan(val) or np.isinf(val):
                            val = 0.0
                        self.latest_feature_store[target_feature] = val
                    else:
                        # 計算失敗時などは安全に0更新
                        self.latest_feature_store[target_feature] = 0.0

            # ステップB: リスト順序通りにキャッシュから値を引き出してベクトル化
            # H1の判定時であっても、キャッシュにある最新のH4やD1の値(実数値)が埋め込まれる
            feature_vector = []
            for target_feature in self.target_features:
                # キャッシュから取得 (なければ0.0)
                val = self.latest_feature_store.get(target_feature, 0.0)
                feature_vector.append(val)

            # 5. ベクトル化
            # (1, N) の形状にして返す
            final_vector = np.array([feature_vector], dtype=np.float64)
            return final_vector
