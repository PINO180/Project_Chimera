# /workspace/models/fast_grid_search_hilbert_only.py
...
RESULTS_CSV_PATH = Path(
    "/workspace/data/XAUUSD/stratum_7_models/1A_2B/fast_grid_search_hilbert_only_results.csv"
)
...
# ★ 評価対象の時間足を指定します。
# 個別評価の場合: ["M5", "M15"] のようにリストで指定
# 混合(全時間足)評価の場合: ["all"] と指定（指定された全時間足のシグナルをガッチャンコします）
TIMEFRAMES = ["all"]

ATR_PERIODS = [13, 21, 55]
TRIGGER_RATIOS = [1.03, 1.05, 1.08, 1.10, 1.12]

PT_MULTS = [0.5, 1.0, 2.5, 5.0]
SL_MULTS = [0.5, 1.0, 2.5, 5.0]
TD_MINS = [3, 5, 10, 30, 60, 120, 180, 720, 1200]
...
def evaluate_and_accumulate(
    ...
    for td in TD_MINS:
        ...
        for pt_mult, sl_mult in itertools.product(PT_MULTS, SL_MULTS):
            ...
            agg_key = (tf_label, atr_p, tr, pt_mult, sl_mult, td)
...
def run_fast_grid_search():
    ...
    logging.info(
        f"Starting Monthly Batch Grid Search. Mode: {'MIXED (all)' if is_mixed_mode else 'INDIVIDUAL'}. Total months: {len(target_months)}"
    )
    ...
    for year, month in target_months:
        ...
        for tf in target_tfs:
            ...
            s5_cols = lf_s5.collect_schema().names()
            actual_hilbert = next((c for c in s5_cols if HILBERT_COL in c), None)
            if not actual_hilbert:
                continue

            # フィルタ前に移動平均を計算し、その後当月分にカット
            lf_s5 = (
                lf_s5.select(["timestamp", actual_hilbert])
                .with_columns(pl.col("timestamp").cast(pl.Datetime("us", "UTC")))
                .sort("timestamp")
                .with_columns(
            ...
            lf_sig = (
                ...
                .drop_nulls(subset=["close", "trigger_ratio_val"] + atr_cols)
            ...
            for atr_p in ATR_PERIODS:
                ...
                for tr in TRIGGER_RATIOS:
                    # ★AND条件: Hilbert急変化 ＆ ATRが取得できていること
        ...
        if is_mixed_mode:
            for (atr_p, tr), sig_list in mixed_signals.items():
                if not sig_list:
                    continue
