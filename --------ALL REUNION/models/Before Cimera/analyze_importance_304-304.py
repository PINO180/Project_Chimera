...
if __name__ == "__main__":
    # ---------------------------------------------------------
    # 【実験パターン②】 M1もM2も Full 304 (TXT) を使用
    # ---------------------------------------------------------

    # 1. M1 分析 (通常通り TXT)
    analyze_model_full(
        model_path=M1_MODEL_PATH,
        feature_names_path=M1_FEATURE_LIST_PATH,
        model_name="M1 Model (Full 304)",
        file_type="txt",
    )

    # 2. M2 分析 (TXT + m1_pred_proba)
    # ここは少し工夫が必要です。M2用に「TXTを読み込んで先頭にスコアを足す」処理を呼び出します。

    # M2用のリストを一時的に作成して渡すか、関数内で処理させます。
    # 最も簡単なのは、M2_FEATURE_LIST_PATH を M1_FEATURE_LIST_PATH (TXT) に変えて、
    # file_type="txt_m2" という新しいモードを指定することです。

    analyze_model_full(
        model_path=M2_MODEL_PATH,
        feature_names_path=M1_FEATURE_LIST_PATH,  # ★TXTを指定
        model_name="M2 Model (Full 304 + Score)",
        file_type="txt_m2",  # ★新しいモードを指定
    )
