...
# ファイルパス
file_path = "/workspace/data/XAUUSD/stratum_7_models/1A_2B/context_features.parquet"
...
try:
    ...
    numeric_cols = [col for col, dtype in df.schema.items() if dtype in [pl.Float64, pl.Float32, pl.Int64, pl.Int32] and col != "timestamp"]
    ...
    for col_name in numeric_cols:
        ...
        if stats["中央値"][0] == 0:
            print("\n--- (参考) 0以外の有効値サンプル (最大5行) ---")
            non_zero = df.filter(pl.col(col_name) != 0.0).select(["timestamp", col_name]).head(5)
            if len(non_zero) > 0:
                print(non_zero)
            else:
