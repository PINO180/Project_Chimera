...
class FinalTrainingConfig:
    ...
    feature_list_path: Path = S3_FEATURES_FOR_TRAINING_XV
    n_splits: int = 5
    purge_days: int = 3
    embargo_days: int = 2
...
class FinalAssembler:
    ...
    def _load_features(self, feature_path: Path) -> List[str]:
        ...
        exclude_exact = {
            ...
            "is_trigger",
            "is_trigger_long",   # ★XV追加
            "is_trigger_short",  # ★XV追加
        }
    ...
    def _filter_dataframe(self, df: pl.DataFrame, is_m1: bool = False) -> pl.DataFrame:
        """XV仕様: M1学習時のみ、方向別トリガー(is_trigger_long/short=1)発火行に絞り込む"""
        if is_m1:
            trigger_col = f"is_trigger_{self.direction}"
            if trigger_col in df.columns:
                df = df.filter(pl.col(trigger_col) == 1)

        if "timeframe" in df.columns:
            # ★XV: TARGET_TIMEFRAMESはM1のみ。文字列型の場合のみInt32変換
            if df.schema["timeframe"] in [pl.Utf8, pl.String]:
                df = df.with_columns(
                    pl.col("timeframe")
                    .replace({"M1": 0})  # ★XV: M1のみ対応
                    .cast(pl.Int32)
                )
            else:
    ...
    def _train_m2_cv_and_write_to_disk(self):
        ...
        for i, (train_dates, val_dates) in enumerate(kfold.split(partitions_to_use)):
            ...
            for p_date in tqdm(
                ...
                # ★XV: timeframeをInt32→文字列に復元（M1のみ）
                reverse_map = {0: "M1"}
