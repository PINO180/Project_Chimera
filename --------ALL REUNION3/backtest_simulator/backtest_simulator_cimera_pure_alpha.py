...
from blueprint import (
    S6_LABELED_DATASET,  # Pure Alpha: OOFの代わりにラベルを直接使用
    S6_WEIGHTED_DATASET,
    S3_SELECTED_FEATURES_PURIFIED_DIR,
    S7_MODELS,
    S7_BACKTEST_CACHE_DIR,
...
class BacktestConfig:
    ...
    fixed_risk_percent: float = (
        0.0001  # 口座残高の何%を1トレードのリスクとするか (0.02 = 2%)
    )
    # ▲▲▲ ここまで追加 ▲▲▲

    # Pure Alpha: 全件エントリー（label=1.0/0.0 で判定）
    m2_proba_threshold: float = 0.0  # 実質意味なし
    ...
    prevent_simultaneous_orders: bool = False
...
class BacktestSimulator:
    ...
    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        ...
        select_cols = [
            c
            for c in base_cols + ["label_long", "label_short", "is_trigger"]
            if c in available_schema
        ]

        base_lf = (
            pl.scan_parquet(str(self.config.simulation_data_path / "**/*.parquet"))
    ...
    def _run_simulation_loop(
        ...
        for i in range(len(df_chunk)):
            ...
            # =========================================================
            # Pure Alpha: is_trigger=1の全バーで無条件エントリー
            # label=1(PT到達) / label=0(SL or タイムアウト) は
            # エントリー判定には使わずPnL計算で使用される
            # =========================================================
            p_l = p_long_chunk[i]
            p_s = p_short_chunk[i]

            # label_long/shortに関わらず常にTrue（全件エントリー）
    ...
    def _analyze_and_report(self, results_df: pl.DataFrame, trade_log: pl.DataFrame):
        ...
        strategy_str = "Pure Alpha (Label Direct) "
        if self.config.use_fixed_risk:
        ...
        strategy_str += f", L(PT{self.config.pt_multiplier_long}/SL{self.config.sl_multiplier_long}), S(PT{self.config.pt_multiplier_short}/SL{self.config.sl_multiplier_short})"
        ...
        print(
            f" Profit Factor:        {report_data.get('profit_factor', 0.0) or 0.0:.2f}"
        )
        print(
        ...
        if not trade_log.is_empty():
            ...
            try:
                ...
                utc_csv_path = (
                    ...
                    / f"detailed_trade_log_{_log_suffix}_UTC.csv"
                )
        ...
        try:
            ...
            if not trade_log.is_empty():
                ...
                count_l_win = (
                    len(l_trades.filter(pl.col("label") == 1)) if count_l > 0 else 0
                )
                count_s_win = (
                    len(s_trades.filter(pl.col("label") == 1)) if count_s > 0 else 0
                )
            ...
            with open(text_report_path, "w", encoding="utf-8") as f:
                ...
                f.write(f"Profit Factor:\t\t{profit_factor or float('inf'):.2f}\n")
                ...
                f.write(
                    f"  Long Win (label=1):  {count_l_win if 'count_l_win' in dir() else 'N/A'}\n"
                )
                f.write(
                    f"  Short Win (label=1): {count_s_win if 'count_s_win' in dir() else 'N/A'}\n"
                )
                f.write("\n")
...
if __name__ == "__main__":
    ...
    # =========================================================
    # Pure Alpha: モード選択なし・キャッシュ管理
    # =========================================================
    import datetime as _dt

    inference_mode = "PureAlpha"
    _now_str = _dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    _risk_pct = int(config.fixed_risk_percent * 100)
    _folder_name = (
        f"{inference_mode}_{_now_str}_R{_risk_pct}_Sp{int(config.spread_pips)}"
    )
    _result_dir = S7_BACKTEST_SIM_RESULTS / _folder_name
    _result_dir.mkdir(parents=True, exist_ok=True)

    FINAL_REPORT_PATH = _result_dir / "pure_alpha_report.json"
    EQUITY_CURVE_PATH = _result_dir / "pure_alpha_equity_curve.png"
    logging.info(f"出力先フォルダ: {_result_dir}")

    # キャッシュ管理（Pure Alpha専用）
    ...
    def load_or_generate_cache() -> tuple:
        if _cache_path.exists():
            print(f"\n[PureAlpha] キャッシュが存在します: {_cache_path}")
            print("  [y] このまま使用する")
            print("  [r] 削除して再生成する")
            ans = input("選択 [y/r]: ").strip().lower()
            if ans != "r":
                logging.info("[PureAlpha] キャッシュを読み込んでいます...")
                with open(_cache_path, "rb") as f:
                    data = pickle.load(f)
                logging.info("[PureAlpha] キャッシュ読み込み完了。")
                return data
            _cache_path.unlink()
            logging.info("[PureAlpha] キャッシュを削除しました。再生成します...")

        logging.info("[PureAlpha] データを生成します...")
        data = simulator.preload_data()
        _cache_path.parent.mkdir(parents=True, exist_ok=True)
        with open(_cache_path, "wb") as f:
            pickle.dump(data, f)
        logging.info(f"[PureAlpha] キャッシュ保存完了: {_cache_path}")
        return data
