...
# --- 出力ファイルパス（実行時に動的生成）---
FINAL_REPORT_PATH = S7_MODELS / "final_backtest_report_xv.json"  # 起動時に上書き
EQUITY_CURVE_PATH = S7_MODELS / "equity_curve_xv.png"  # 起動時に上書き
...
class BacktestConfig:
    ...
    min_atr_threshold: float = (
        1.5  # ★XV修正: Short側の最低閾値(1.5)。Long側は min_atr_threshold_long(3.0)で個別判定
    )
    ...
    # XV: 取引ロジックの固定パラメータ
    # ==========================================
    sl_multiplier_long: float = 10.0   # ★XV修正: 5.0→10.0
    pt_multiplier_long: float = 10.0   # ★XV修正: 1.0→10.0
    sl_multiplier_short: float = 5.0   # 変更なし
    pt_multiplier_short: float = 10.0  # ★XV修正: 1.0→10.0

    td_minutes_long: float = 120.0   # ★XV修正: 30.0→120.0
    td_minutes_short: float = 120.0  # ★XV修正: 30.0→120.0

    # ★XV追加: Long/Short個別ATR閾値
    min_atr_threshold_long: float = 3.0   # Long専用ATR閾値
    min_atr_threshold_short: float = 1.5  # Short専用ATR閾値（min_atr_thresholdと同値）
...
class BacktestSimulator:
    ...
    def run(
        ...
        logging.info("### Project Forge XV Backtest Simulator: START ###")
        logging.info(
        ...
        logging.info("### Project Forge XV Backtest Simulator: FINISHED ###")
        return report_data
    ...
    def _run_simulation_loop(
        ...
        for i in range(len(df_chunk)):
            ...
            for (
                ...
                if should_trade:
                    # ★XV: Long/Short個別のATR閾値で判定
                    atr_threshold = (
                        self.config.min_atr_threshold_long
                        if direction_int == 1
                        else self.config.min_atr_threshold_short
                    )
                    if (
                        atr_ratio_float is None
                        or not np.isfinite(atr_ratio_float)
                        or atr_ratio_float < atr_threshold
                    ):
                        continue
    ...
    def _analyze_and_report(self, results_df: pl.DataFrame, trade_log: pl.DataFrame):
        logging.info("Analyzing results and generating XV report...")
        DECIMAL_ONE = Decimal("1.0")
        DECIMAL_ZERO = Decimal("0.0")
        ...
        try:
            ...
            with open(text_report_path, "w", encoding="utf-8") as f:
                f.write("=" * 60 + "\n")
                f.write("    XV Two-Brain Strategy Performance Report (MT5 Style)\n")
                f.write("=" * 60 + "\n\n")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Project Forge XV Backtest Simulator (Two-Brain, Auto Lot + Dynamic SL + Timeouts)"
    )
    ...
    # ★XV: Long/Short個別ATR閾値引数
    parser.add_argument(
        "--min-atr-long",
        type=float,
        default=default_config.min_atr_threshold_long,
        dest="min_atr_long",
        help=f"Minimum ATR Ratio threshold for Long. Default: {default_config.min_atr_threshold_long}",
    )
    parser.add_argument(
        "--min-atr-short",
        type=float,
        default=default_config.min_atr_threshold_short,
        dest="min_atr_short",
        help=f"Minimum ATR Ratio threshold for Short. Default: {default_config.min_atr_threshold_short}",
    )
    parser.add_argument(
        "--value-per-pip",
        type=float,
    ...
    config = BacktestConfig(
        ...
        min_atr_threshold=default_config.min_atr_threshold,
        min_atr_threshold_long=args.min_atr_long,
        min_atr_threshold_short=args.min_atr_short,
    ...
    FINAL_REPORT_PATH = _result_dir / f"final_backtest_report_xv_{inference_mode}.json"
    EQUITY_CURVE_PATH = _result_dir / f"equity_curve_xv_{inference_mode}.png"
