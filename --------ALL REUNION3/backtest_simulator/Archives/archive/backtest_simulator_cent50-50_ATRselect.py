...
class BacktestConfig:
    ...
    # ★★★ 追加: 最大同時保有ポジション数 ★★★
    max_positions: int = 10  # ここを好きな数に変更（例: 3, 5, 10）
...
class BacktestSimulator:
    def __init__(self, config: BacktestConfig):
        ...
        # ★★★ [50-50専用モード] M1/M2ともに Top 50 (JSON) を使用 ★★★
        top_50_path = project_root / "models" / "TOP_50_FEATURES.json"

        # 特徴量読み込み (JSON専用)
        self.features_base = self._load_features_from_json(top_50_path)
        logging.info(
            f"M1 Features: Loaded {len(self.features_base)} items from {top_50_path.name}."
        )

        # M2特徴量 = Top 50 + Score
        self.features_m2 = ["m1_pred_proba"] + self.features_base
        logging.info(
            f"M2 Features: Loaded {len(self.features_m2)} items (including score)."
        )
    ...
    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        ...
        if not self.config.oof_mode:  # In-Sample Mode
            ...
            # M2特徴量 (Top 50) - 実質M1と同じだが、m1_pred_probaを除いて念のため追加
    ...
    def _run_simulation_loop(
        ...
        # ★★★ 【修正】 実験: 下位足を開放し、成績の悪い上位足(H4, H6, H12)を除外 ★★★
        # 元のリスト: ["M8", "M15", "H1", "H4", "H6", "H12"]

        # 今回の実験リスト: (M1, M3, M5 を追加、H4以降を削除)
        # ※ H1を残すかどうかはお任せしますが、稼ぎ頭なので入れておくのが無難です
        ...
        for i in range(len(df_chunk)):
            ...
            # 取引ログを記録 (取引した場合のみ)
            # ★★★ 【追加】 同時刻制限ロジック ★★★
            # すでにこの時刻で取引を行っていた場合、今回はスキップする
            if should_trade and (last_trade_timestamp == current_timestamp):
                should_trade = False
                pnl = DECIMAL_ZERO
                # 資産は前の行で append 済みなのでここでは何もしない（ログ記録もしない）
                ...
                trade_log_chunk.append(
                    {
                        ...
                        "timeframe": current_tf,  # ★★★ 【追加】 CSV出力用に記録 ★★★
                        "pnl": pnl,
