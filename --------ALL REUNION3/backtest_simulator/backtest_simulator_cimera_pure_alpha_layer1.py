# /workspace/models/backtest_simulator_cimera_pure_alpha_layer1.py
# [Pure Alpha + Layer 1 超軽量脳（防御フィルター）版]
# Pure Alpha（label直接）にLayer 1フィルターを追加したもの。
# Layer 1がスコア上位20%（危険なシグナル）を見送ることで
# Pure Alphaのベースラインに対してどれだけPFが改善するかを検証する。
...
class BacktestConfig:
    ...
    # Pure Alpha: S6_LABELED_DATASETを直接使用（OOF不要）
    ...
    # Pure Alpha: 全件エントリー（label=1.0/0.0 で判定）
    m2_proba_threshold: float = 0.5   # 実質意味なし
    m2_delta_threshold: float = 0.0   # Delta不要

    test_limit_partitions: int = 0
    oof_mode: bool = True  # 内部互換のため保持
    ...
    # Layer 1 超軽量脳フィルター設定
    # ==========================================
    layer1_model_long_path: Path = LAYER1_MODEL_LONG_PKL
    layer1_model_short_path: Path = LAYER1_MODEL_SHORT_PKL
    use_layer1: bool = True  # Layer 1フィルターを有効にするか
...
class BacktestSimulator:
    def __init__(self, config: BacktestConfig):
        ...
        # Pure Alpha: OOF不使用・S6ラベル直接読み込み専用
        self._current_capital = Decimal(str(self.config.initial_capital))

        # Layer 1モデルのロード
        self.layer1_bundle_long  = None
        self.layer1_bundle_short = None
        if self.config.use_layer1:
            try:
                self.layer1_bundle_long  = joblib.load(
                    self.config.layer1_model_long_path
                )
                self.layer1_bundle_short = joblib.load(
                    self.config.layer1_model_short_path
                )
                logging.info(
                    f"Layer 1モデル読み込み完了: "
                    f"{self.config.layer1_model_long_path.name}"
                )
            except Exception as e:
                logging.warning(
                    f"Layer 1モデル読み込み失敗 → フィルターなしで実行: {e}"
                )
                self.layer1_bundle_long  = None
                self.layer1_bundle_short = None
    ...
    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        ...
        # Pure Alpha: S6_LABELED_DATASETからラベルを直接読み込む
        # label_long/label_short を m2_proba_long/short として使用（1.0 or 0.0）
        logging.info("Pure Alpha: Loading labels directly from S6_LABELED_DATASET...")

        # スキーマ取得は最初の1ファイルだけから（全スキャン回避）
        first_file = next(self.config.simulation_data_path.glob("**/*.parquet"), None)
        if first_file is None:
            raise FileNotFoundError(
                f"S6_LABELED_DATASETにparquetファイルが見つかりません: "
                f"{self.config.simulation_data_path}"
            )
        available_schema = pl.read_parquet_schema(first_file).names()

        select_cols = [c for c in base_cols + [
            "label_long", "label_short", "is_trigger"
        ] if c in available_schema]
        ...
        lf = (
            base_lf
            .with_columns([
                pl.col("label_long").cast(pl.Float64).alias("m2_proba_long"),
                pl.col("label_short").cast(pl.Float64).alias("m2_proba_short"),
                pl.lit(1.0).alias("uniqueness_long"),
                pl.lit(1.0).alias("uniqueness_short"),
            ])
            .sort(["timestamp", "timeframe"])
        )

        # タイムアウト決済用の未来価格を asof join で事前結合
        price_lf_long = (
            lf.select([
    ...
    def _run_simulation_loop(
        ...
        for i in range(len(df_chunk)):
            ...
            # =========================================================
            # Pure Alpha + Layer 1: エントリー判定
            # is_trigger=1の全バーで無条件エントリー（Layer 1が後段でフィルター）
            # =========================================================
            p_l = p_long_chunk[i]
            p_s = p_short_chunk[i]
            ...
            # =========================================================
            # Layer 1 超軽量脳フィルター
            # 特徴量: atr_ratio / hour_utc / weekday（UTC基準）
            # モデルバンドルから閾値を取得して判定
            # フェイルセーフ: 推論失敗時はそのまま通す
            # =========================================================
            if self.config.use_layer1:
                try:
                    _ts  = current_timestamp_dt
                    _atr = float(atr_ratio_float) if np.isfinite(atr_ratio_float) else 1.0
                    _hour_utc = _ts.hour          # UTC時間帯（変換不要）
                    _weekday  = _ts.weekday()     # UTC基準の曜日

                    X_l1 = np.array([[_atr, _hour_utc, _weekday]])

                    if should_trade_long and self.layer1_bundle_long is not None:
                        _model_l   = self.layer1_bundle_long["model"]
                        _thresh_l  = self.layer1_bundle_long["threshold"]
                        _score_l   = float(_model_l.predict_proba(X_l1)[0][1])
                        if _score_l >= _thresh_l:
                            should_trade_long = False  # 見送り

                    if should_trade_short and self.layer1_bundle_short is not None:
                        _model_s   = self.layer1_bundle_short["model"]
                        _thresh_s  = self.layer1_bundle_short["threshold"]
                        _score_s   = float(_model_s.predict_proba(X_l1)[0][1])
                        if _score_s >= _thresh_s:
                            should_trade_short = False  # 見送り

                except Exception:
                    pass  # フェイルセーフ: 推論失敗時はそのまま通す
    ...
    def _analyze_and_report(self, results_df: pl.DataFrame, trade_log: pl.DataFrame):
        ...
        # Pure Alpha: strategy文字列
        strategy_str = "Pure Alpha + Layer1 Filter "
        if self.config.use_fixed_risk:
        ...
        l1_status = "ON" if (self.config.use_layer1 and self.layer1_bundle_long is not None) else "OFF"
        strategy_str += f", Layer1={l1_status}, L(PT{self.config.pt_multiplier_long}/SL{self.config.sl_multiplier_long}), S(PT{self.config.pt_multiplier_short}/SL{self.config.sl_multiplier_short})"
        ...
        if not trade_log.is_empty():
            _log_suffix = "PureAlpha_Layer1"
            trade_log_output_path = (
                FINAL_REPORT_PATH.parent / f"detailed_trade_log_{_log_suffix}.csv"
            )
            logging.info(
            ...
            try:
                ...
                # UTC版CSV（Layer 1学習データ生成用）を先に保存
                utc_csv_path = (
                    FINAL_REPORT_PATH.parent / f"detailed_trade_log_{_log_suffix}_UTC.csv"
                )
                trade_log_final_csv.write_csv(utc_csv_path, null_value="NaN")
                logging.info(f"UTC trade log saved to {utc_csv_path}")

                # timestampをUTC→JSTに変換（+9時間）して上書き（人間可読用）
        ...
        try:
            ...
            if not trade_log.is_empty():
                ...
                # Pure Alpha: label=1（PT到達・勝ち）のカウント
                count_l_win = len(l_trades.filter(pl.col("label") == 1)) if count_l > 0 else 0
                count_s_win = len(s_trades.filter(pl.col("label") == 1)) if count_s > 0 else 0
            ...
            with open(text_report_path, "w", encoding="utf-8") as f:
                ...
                # Pure Alpha: label=1（勝ち）/ label=0（負け）の分布を記録
                f.write("-" * 23 + " Pure Alpha Label Distribution " + "-" * 3 + "\n")
                f.write("  ※ label=1がPT到達（勝ち）/ label=0がSL or タイムアウト\n")
                f.write(f"  Long Win (label=1):  {count_l_win if 'count_l_win' in dir() else 'N/A'}\n")
                f.write(f"  Short Win (label=1): {count_s_win if 'count_s_win' in dir() else 'N/A'}\n")
                f.write("\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        action="store_true",
        dest="no_layer1",
        help="Layer 1フィルターを無効化してPure Alphaのみで実行（比較用）",
    )
    ...
    config = BacktestConfig(
        ...
        m2_proba_threshold=0.5,
        m2_delta_threshold=0.0,
    ...
    # =========================================================
    # Pure Alpha + Layer 1: モード選択・キャッシュ管理
    # =========================================================
    import datetime as _dt

    # --no-layer1 フラグで Layer 1 を無効化できる
    if getattr(args, "no_layer1", False):
        config.use_layer1 = False

    inference_mode = "PureAlpha_Layer1" if config.use_layer1 else "PureAlpha"
    _now_str  = _dt.datetime.now().strftime("%Y%m%d_%H%M%S")
    _risk_pct = int(config.fixed_risk_percent * 100)
    _folder_name = (
        f"{inference_mode}_{_now_str}"
        f"_R{_risk_pct}"
        f"_Sp{int(config.spread_pips)}"
    )
    _result_dir = S7_BACKTEST_SIM_RESULTS / _folder_name
    _result_dir.mkdir(parents=True, exist_ok=True)

    FINAL_REPORT_PATH = _result_dir / f"pure_alpha_layer1_report.json"
    EQUITY_CURVE_PATH = _result_dir / f"pure_alpha_layer1_equity_curve.png"
    logging.info(f"出力先フォルダ: {_result_dir}")
    logging.info(f"Layer 1フィルター: {'ON' if config.use_layer1 else 'OFF'}")

    # キャッシュ管理（Pure Alphaと同じキャッシュを共有）
    _cache_path = S7_BACKTEST_CACHE_DIR / "backtest_pure_alpha_preload_cache.pkl"

    def load_or_generate_cache() -> tuple:
        if _cache_path.exists():
            print(f"\n[{inference_mode}] キャッシュが存在します: {_cache_path}")
            print("  [y] このまま使用する")
            print("  [r] 削除して再生成する")
            ...
            if ans != "r":
                ...
                return data
            _cache_path.unlink()
            logging.info(f"[{inference_mode}] キャッシュを削除しました。再生成します...")

        logging.info(f"[{inference_mode}] データを生成します...")
        data = simulator.preload_data()
        _cache_path.parent.mkdir(parents=True, exist_ok=True)
        with open(_cache_path, "wb") as f:
            pickle.dump(data, f)
        logging.info(f"[{inference_mode}] キャッシュ保存完了: {_cache_path}")
        return data
