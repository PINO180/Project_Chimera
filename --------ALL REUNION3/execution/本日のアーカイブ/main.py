                    ...
                    if len(feature_engine.m1_dataframe) > 0:
                        ...
                        if diff_minutes > 0:
                            ...
                            if diff_df is not None and len(diff_df) > 0:
                                ...
                                # M0.5→M1にresampleしてエンジンに流し込む
                                if not new_m05_bars.empty:
                                    new_m05_bars = new_m05_bars.set_index("timestamp")
                                    new_m1_resampled = (
                                        new_m05_bars.resample("1min")
                                        .agg(
                                            {
                                                "open": "first",
                                                "high": "max",
                                    ...
                                    for _, row in new_m1_resampled.iterrows():
                                        bar_dict = {
                                ...
                                logger.info(
                                    f"✓ 差分 {len(new_m1_resampled)} 本(M1換算)の追いつき計算が完了しました！完全に同期しています。"
                                )
        ...
        while True:
            try:
                ...
                for signal in signal_list:
                    ...
                    current_spread = new_m1_bar.get("spread", 999.0)
