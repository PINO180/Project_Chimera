...
class RealtimeFeatureEngine:
    ...
    def _check_for_signal(self, tf_name: str, timestamp: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            if atr_ratio >= atr_threshold:
                ...
                self.logger.info(
                    f"  -> V5 Signal Check ({tf_name} @ {(timestamp + pd.Timedelta(minutes=3)).strftime('%H:%M')}): "
    ...
    def _update_incremental_ols(
        ...
        try:
            ...
            latest_x = (
                float(market_proxy_cache.iloc[idx]["market_proxy"])
                if idx != -1
                else 0.0
            )
...
RealtimeFeatureEngine.process_new_m1_bar = profiling_patch.process_new_m1_bar
