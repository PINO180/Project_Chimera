...
import sys
import time  # [LATENCY-PROBE] 各段階の所要時間計測用
...
class RealtimeFeatureEngine:
    ...
    def _log_latency_probe(
        ...
        t_step2_end: float,
        per_tf_times: dict,
        t_step3_end: Optional[float],
        t_step4_end: Optional[float],
        mode: str,
    ) -> None:
        """[LATENCY-PROBE] process_new_m05_bar の各段階所要時間を 1 行ログで出力。

        後で grep '[LATENCY-PROBE]' で削除可能な調査用コード。
        Step A の PERF-LOG と同じ要領で、ラグ問題が解決したら削除する想定。

        mode:
            "V5_PASS"     : V5 通過してシグナル生成
            "V5_FAIL"     : V5 で弾かれた
            "warmup_only" : 差分追いつき期間 (シグナル抑制)
        """
        try:
            now = time.perf_counter()
            total_ms = (now - t_start) * 1000.0
            step1_ms = (t_step1_end - t_start) * 1000.0
            step2_ms = (t_step2_end - t_step1_end) * 1000.0
            step3_ms = (
                (t_step3_end - t_step2_end) * 1000.0 if t_step3_end is not None
                else (now - t_step2_end) * 1000.0
            )
            step4_ms = (
                (t_step4_end - t_step3_end) * 1000.0 if t_step4_end is not None and t_step3_end is not None
                else 0.0
            )
            step5_ms = (
                (now - t_step4_end) * 1000.0 if t_step4_end is not None
                else 0.0
            )

            # TF 別 calc/ols/neut の所要時間
            tf_str_parts = []
            for tf, (calc_ms, ols_ms, neut_ms) in per_tf_times.items():
                tf_str_parts.append(
                    f"{tf}(calc={calc_ms:.1f}/ols={ols_ms:.1f}/neut={neut_ms:.1f})"
                )
            tf_str = " ".join(tf_str_parts) if tf_str_parts else "(no TFs)"

            self.logger.info(
                f"[LATENCY-PROBE] mode={mode} total={total_ms:.1f}ms | "
                f"step1_m05_append={step1_ms:.1f}ms | "
                f"step2_resample={step2_ms:.1f}ms | "
                f"step3_recalc_all_tf={step3_ms:.1f}ms | "
                f"step4_v5_check={step4_ms:.1f}ms | "
                f"step5_signal_gen={step5_ms:.1f}ms"
            )
            self.logger.info(f"[LATENCY-PROBE] per_tf: {tf_str}")
        except Exception as e:
            self.logger.warning(f"[LATENCY-PROBE] ログ出力失敗: {e}")
    ...
    def process_new_m05_bar(
        ...
        # [LATENCY-PROBE] 各段階の所要時間計測 (M3 確定時のみログ出力、削除可能)
        _t_start = time.perf_counter()
        _per_tf_times = {}  # {tf_name: (calc_ms, ols_ms, neut_ms)}
        ...
        try:
            ...
            # [LATENCY-PROBE]
            _t_step1_end = time.perf_counter()
            ...
            # [LATENCY-PROBE]
            _t_step2_end = time.perf_counter()
            ...
            def _recalc_one_tf(tf_name: str):
                ...
                try:
                    ...
                    # [LATENCY-PROBE] 各 TF の calc/ols/neut 時間を個別計測
                    _tf_t0 = time.perf_counter()
                    base_features = self._calculate_base_features(data, tf_name)
                    _tf_t1 = time.perf_counter()

                    self._update_incremental_ols(
                        tf_name, base_features, market_proxy_cache, m3_timestamp
                    )
                    _tf_t2 = time.perf_counter()

                    neutralized = self._calculate_neutralized_features(
                        base_features, tf_name, m3_timestamp, market_proxy_cache
                    )
                    self.latest_features_cache[tf_name] = neutralized
                    _tf_t3 = time.perf_counter()

                    return (
                        tf_name,
                        (_tf_t1 - _tf_t0) * 1000,  # calc_ms
                        (_tf_t2 - _tf_t1) * 1000,  # ols_ms
                        (_tf_t3 - _tf_t2) * 1000,  # neut_ms
                    )
                except Exception as e:
            ...
            for future in futures:
                ...
                if result is not None:
                    tf, calc_ms, ols_ms, neut_ms = result
                    _per_tf_times[tf] = (calc_ms, ols_ms, neut_ms)

            # シグナルチェックはM3のみ
            # [STALE-GUARD] warmup_only=True（差分追いつき中）はシグナル生成を根本からスキップ
            if warmup_only:
                # [LATENCY-PROBE] warmup_only でも所要時間ログは出す (識別子で区別)
                self._log_latency_probe(_t_start, _t_step1_end, _t_step2_end,
                                         _per_tf_times, None, None, mode="warmup_only")
                return signal_list

            # [LATENCY-PROBE]
            _t_step3_end = time.perf_counter()

            V5_check_result = self._check_for_signal("M3", m3_timestamp)

            # [LATENCY-PROBE]
            _t_step4_end = time.perf_counter()
            ...
            # [LATENCY-PROBE] M3 確定処理の最終ログ出力
            self._log_latency_probe(
                _t_start, _t_step1_end, _t_step2_end,
                _per_tf_times, _t_step3_end, _t_step4_end,
                mode="V5_PASS" if V5_check_result.get("is_V5") else "V5_FAIL",
            )
