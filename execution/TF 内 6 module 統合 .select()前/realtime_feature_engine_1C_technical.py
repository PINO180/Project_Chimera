...
# ==================================================================
#
# 目的: 学習側 engine_1_C_a_vast_universe_of_features.py と「同じ Polars
#       Rust エンジン」で計算することで、ビット完全一致を達成する。
#
# 【背景】
#   旧実装(numpy/手書きループ):
#     - 各 rolling 統計を毎バー O(N) で再計算
#     - STC (Schaff Trend Cycle) を手書き Python ループで実装 (重い)
#     - KST/Coppock も手書きループ (重い)
#     - atr_ok ガード (atr > 1e-10) が学習側 scale_by_atr と微小に乖離
#   新実装(Polars 直呼び):
#     - 全 rolling 計算を 1 つの .select() に集約 (約 191 特徴量)
#     - STC/KST/Coppock も Polars ewm_mean / rolling_mean で表現
#     - scale_by_atr 等価式 `target / (atr_raw + 1e-10)` で完全一致
#     - 期待速度向上: 旧実装で重かった STC を含めて一括計算
#
# 【ATR 割りの統一】
#   学習側 (engine_1_C): 全て `scale_by_atr(target, atr_raw)` 経由
#     → 内部で `target / (atr_raw + 1e-10)`
#   旧本番側: `atr_denom = atr_13 + 1e-10` で同じ式だが、
#     `atr_ok = np.isfinite(atr_13) and atr_13 > 1e-10` ガードが付き
#     → ATR が 1e-10 以下だと NaN を返す (学習側は巨大値を返す)
#   実害: XAU/USD で ATR が 1e-10 以下になることはまずないため観測不能だが、
#     ビット完全一致のため Step B では atr_ok ガードを廃止し学習側と完全に揃える。
#
# 【SSoT 階層】
#   Layer 1 (rolling 統計): 学習・本番ともに Polars Rust エンジン → ビット一致
#   Layer 2 (Numba UDF):    学習・本番ともに core_indicators から import → ビット一致
#                           (Phase 5 で確立済み、変更なし)
#   注意: Numba UDF (RSI/ADX/DI/Williams/Aroon/Stochastic/TRIX/TSI/UO/WMA/HMA/KAMA)
#         は本番側で全配列を事前計算して DataFrame の列に追加し、後続の Polars 式で
#         参照する形式とする (学習側 map_batches と等価)。
#
# 【保持される過去の修正】
...
#   ・trend_consistency の period+2 ガード (Polars が自然に NaN を返すため不要だが残置)
# ==================================================================
...
class FeatureModule1C:
    ...
    def calculate_features(
        ...
        アーキテクチャ:
            1. 各 Numba UDF (RSI/ADX/DI/Williams/Aroon/Stochastic/TRIX/TSI/UO/
               WMA/HMA/KAMA) を全配列に対して事前計算 → DataFrame の列に追加
            2. 全 rolling 系特徴量 (BB/MACD/STC/KST/Coppock/SMA/EMA/RVI 等) を
               Polars 式リストに集約
            3. 単一 .lazy().select(exprs).tail(1).collect() で一括計算
            4. QA 処理 (1A/1B と同一)
        """
        ...
        # `target / __atr_13_safe` で割れば scale_by_atr と完全一致。
        # ---------------------------------------------------------
        if len(high_arr) > 0 and len(low_arr) > 0:
            ...
            atr_13_raw = np.full(n, np.nan, dtype=np.float64)
            atr_21_raw = atr_13_raw.copy()
            atr_34_raw = atr_13_raw.copy()
            atr_55_raw = atr_13_raw.copy()
        ...
        # ---------------------------------------------------------
        # Polars DataFrame 構築 (Zero-copy + 全 UDF 結果を列化)
        # ---------------------------------------------------------
        df_dict = {
            "close": close_arr,
        ...
        # 注意: open/high/low が存在しないと計算不能
        # ---------------------------------------------------------
        if len(open_arr) > 0 and len(high_arr) > 0 and len(low_arr) > 0:
            for p in [10, 14, 20]:
        ...
        # ---------------------------------------------------------
        # RVI が無効 (open/high/low なし) 時のフォールバック
        # ---------------------------------------------------------
        if len(open_arr) == 0 or len(high_arr) == 0 or len(low_arr) == 0:
            for p in [10, 14, 20]:
                features.setdefault(f"e1c_relative_vigor_index_{p}", np.nan)
                features.setdefault(f"e1c_rvi_signal_{p}", np.nan)

        # =====================================================================
        # QA 処理 (学習側 apply_quality_assurance と等価)
        # =====================================================================
