...
class RealtimeFeatureEngine:
    ...
    def validate_feature_coverage(self):
        """
        [新規] 起動時自己診断
        特徴量名簿(final_feature_set_v2.txt)にある全ての特徴量が、
        実際の計算ロジック(_calculate_base_features)から正しく生成されるかを検証する。
        """
        self.logger.info("-" * 50)
        self.logger.info("🔍 特徴量カバレッジ検証 (Self-Diagnosis) を開始...")

        # 1. ダミーデータの生成 (全ての計算が走るのに十分な長さ)
        # H1などの長い時間足も計算できるよう、長めのランダムデータを用意
        dummy_len = 3000
        dummy_data = {
            "open": np.random.rand(dummy_len) * 100 + 1000,
            "high": np.random.rand(dummy_len) * 100 + 1100,
            "low": np.random.rand(dummy_len) * 100 + 900,
            "close": np.random.rand(dummy_len) * 100 + 1000,
            "volume": np.random.rand(dummy_len) * 1000 + 100,
        }
        # 全てfloat64に変換
        for k in dummy_data:
            dummy_data[k] = dummy_data[k].astype(np.float64)

        # 2. テスト計算を実行 (最も特徴量が多い 'H1' あたりで代表検証)
        # ※本来は全時間足やるべきですが、ロジックは時間足によらず共通のため代表値でOK
        generated_features = self._calculate_base_features(dummy_data, "H1")
        generated_keys = set(generated_features.keys())
        ...
        for feature_name_in_list in self.feature_list:
            # 時間足サフィックスを除去して「ベース名」を特定するロジック
            # (calculate_feature_vector内のロジックと完全一致させる)

            # 1. まず時間足部分を特定して除外判定
            # 名簿: "e1a_std_50_H1", "e1c_atr_H4" など
            # ここでは「ロジックが存在するか」を見たいので、H1用の特徴量を対象にする
            # (他の時間足専用の特徴量でも、ベース名が同じならH1で計算されているはず)
            ...
            # 特別扱い: m1_pred_proba はリアルタイムエンジンではなくモデル出力なので除外
            if "m1_pred_proba" in base_name:
                ignored_count += 1
                continue

            # 照合
            if base_name in generated_keys:
                covered_count += 1
            else:
        ...
        if missing_features:
            # 重複を除去して表示
            unique_missing = sorted(list(set(missing_features)))
            self.logger.error(
                f"❌ 警告: 以下の {len(unique_missing)} 個の特徴量が計算ロジックに見つかりません（0.0になります）:"
            )
            for missing in unique_missing:
                self.logger.error(f"   - {missing}")
            self.logger.error(
                "👉 _calculate_base_features 内の辞書キーと名簿のスペルを確認してください。"
            )
        else:
            self.logger.info(
                f"✅ 全特徴量のロジック実装を確認しました (検証数: {covered_count}, 除外: {ignored_count})"
            )
    ...
    def _parse_feature_list_and_get_lookbacks(
        ...
        [V6.6 修正]
        Safe Marginを適用し、最低限の長さを保証する。

        [Trigger対応修正]
        特徴量リストにM1が含まれていなくても、Trigger判定(ATR計算)のために
        M1バッファは必須であるため、強制的に管理対象に含める。
        """
        ...
        # ---------------------------------------------------------
        # [修正] Trigger判定用 M1 強制追加
        # ---------------------------------------------------------
        # M1の特徴量がなくても、Trigger (ATR > 5.0) の計算にはM1生データが必須。
        # そのため、リストにない場合でも強制的にエントリを作成する。
        if "M1" not in lookbacks:
            self.logger.info("  -> [Trigger用] M1 を強制的に管理対象に追加します。")
            lookbacks["M1"] = 0  # 後続のSafe Marginで適切な長さに設定される

        # 2. ベース特徴量計算に必要な安全マージンを適用
        # (MFDFA(1000) や WMA(200) 等を考慮し、最低1000を確保)
        SAFE_MIN_LOOKBACK = 1000

        final_lookbacks = {}
        for tf_name in lookbacks.keys():
            # 名簿由来のサイズ と 安全マージン の大きい方採用
    ...
    def process_new_m1_bar(
        ...
        """
        [I/F] main.py から M1 バーを受け取り、シグナルをチェックして返す。

        [修正] 戻り値を変更: (現在のM1_ATR, シグナルリスト)
        Trigger判定の結果にかかわらず、ATR値を常に返すことで常時監視を可能にする。
        """
        ...
        try:
            ...
            # 2. Gatekeeper (Trigger Check)
            # M1のATRを計算し、値を保持
            trigger_result = self._check_for_signal("M1", m1_timestamp)
            current_m1_atr = trigger_result["atr_value"]

            if not trigger_result["is_r4"]:
                # Trigger OFF: ATR値と空リストを返して終了
                return current_m1_atr, []

            # 3. Judgment Process (Trigger ON - ATR >= 5.0)
            self.logger.info(
                f"🚀 TRIGGER PULLED (M1 ATR {current_m1_atr:.2f} >= {self.ATR_REGIME_CUTOFF}). Starting Judgment Process..."
            )

            # M1以外の全時間足バッファをリサンプリング更新
            ...
            for tf_name, timestamps in newly_closed_timeframes.items():
                for timestamp in timestamps:
                    # 各時間足ごとのルール適合チェック
                    if tf_name == "M1":
                        current_check = trigger_result
                    else:
                        current_check = self._check_for_signal(tf_name, timestamp)

                    if current_check["is_r4"]:
                        ...
                        if feature_vector is not None:
                            signal = Signal(
                                ...
                                timestamp=timestamp,
                                timeframe=tf_name,
                                market_info=current_check["market_info"],
                            )
                            signal_list.append(signal)

            return current_m1_atr, signal_list
    ...
    def _check_for_signal(self, tf_name: str, timestamp: datetime) -> Dict[str, Any]:
        ...
        [修正] 判定結果にかかわらず、常に計算した atr_value を返すように変更。
        """
        result = {
            "is_r4": False,
            "atr_value": 0.0,  # デフォルト
        ...
        try:
            ...
            if len(data["close"]) < self.ATR_CALC_PERIOD + 1:
            ...
            if atr_value >= self.ATR_REGIME_CUTOFF:
                ...
                # 詳細ログはここではなく main.py 側で制御するため削減
                # self.logger.info(f"  -> R4 Signal Check PASSED ...")
            else:
                result["reason"] = "not_r4_regime"
    ...
    def _calculate_base_features(
        ...
        # ------------------------------------------------------------------------------
        # [追加修正] 診断で欠落が判明した特徴量の補完 (RVI & EMA Deviation)
        # ------------------------------------------------------------------------------

        # 1. EMA Deviation (50)
        # 既に計算済みの ema_50_arr を使用
        if len(ema_50_arr) > 0:
            ema_val = _last(ema_50_arr)
            if ema_val != 0 and not np.isnan(ema_val):
                features["e1c_ema_deviation_50"] = (
                    (data["close"][-1] - ema_val) / ema_val
                ) * 100
            else:
                features["e1c_ema_deviation_50"] = np.nan
        else:
            features["e1c_ema_deviation_50"] = np.nan

        # 2. Relative Vigor Index (RVI) & Signal
        # Logic: RVI = SMA(Close-Open, n) / SMA(High-Low, n)
        # Signal = SMA(RVI, 4)
        co_diff = data["close"] - data["open"]
        hl_diff = data["high"] - data["low"]

        for rvi_period in [10, 20]:
            # calc_rolling_mean_array_numba はスクリプト上部で定義済み
            num_arr = calc_rolling_mean_array_numba(co_diff, rvi_period)
            den_arr = calc_rolling_mean_array_numba(hl_diff, rvi_period)

            # ゼロ除算防止
            safe_den = den_arr.copy()
            safe_den[safe_den == 0] = 1e-10

            rvi_arr = num_arr / safe_den

            # RVI Signal (4-period SMA of RVI)
            rvi_signal_arr = calc_rolling_mean_array_numba(rvi_arr, 4)

            features[f"e1c_relative_vigor_index_{rvi_period}"] = _last(rvi_arr)
            features[f"e1c_rvi_signal_{rvi_period}"] = _last(rvi_signal_arr)
