        ...
        while True:
            try:
                ...
                for signal in signal_list:
                    ...
                    # ════════════════════════════════════════════════════
                    # [HF-NB-GATE §11.32 / §11.34.16-Q 復活] 高 TF (M5/M8/M15) 非境界 0 化
                    # ────────────────────────────────────────────────────
                    # Q 節 BT で高 TF held(Y) はスパース(X) よりリスク構造劣位・AUC 同一と
                    # 判明したため、高 TF は X (スパース) を維持する。学習側 (cpl) は高 TF を
                    # index シフト + exact join で「境界だけ実値・他 0」 のスパースにしており、
                    # 本番もゲートで非境界を 0 化して一致させる。
                    # 境界判定の基準は行のラベル L = T - ACTION_HORIZON (§11.34.16 O.2 で
                    # 実証: L 基準でゲート不整合 0 行・高 TF corr 1.0、T 基準だと互い違い)。
                    # M0.5/M1/M3 はゲート対象外 (低 TF は join_asof で held、毎回値が変わる)。
                    # ※ cpl の高 TF スパース化 + 再学習モデルとセットで配置すること。
                    #   (timedelta 未 import のため epoch 秒の算術で -180s)
                    # ════════════════════════════════════════════════════
                    ...
                    for _hfnbg_name in list(feature_dict.keys()):
                        ...
                        try:
                            ...
                            continue
                        # M0.5 / M1 / M3 は低 TF (join_asof で held) → ゲート対象外
                        if _hfnbg_tf_min < 5:
                            continue
