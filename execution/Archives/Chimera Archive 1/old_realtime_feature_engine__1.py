...
class Signal:
    ...
    atr_value: float  # [V5追加] 動的バリア計算用の定規(ATR)
    close_price: float  # [V5追加] 動的バリア計算用の起点(現在価格)
...
class RealtimeFeatureEngine:
    ...
    # R4判定用
    # [修正] 固定値(5.0)は廃止し、比率(0.0015)をメソッド内で使用
    # ATR_REGIME_CUTOFF = 5.0
    ATR_CALC_PERIOD = 21  # (e1c_atr_21_...)

    def __init__(
        self,
        feature_list_path: str = str(project_root / "models" / "TOP_50_FEATURES.json"),
    ):
        self.logger = logging.getLogger("ProjectForge.FeatureEngine")

        # 1. 特徴量名簿をロード (JSON/TXT自動判定・修正版)
        try:
    ...
    def _load_feature_list(self, path: str) -> List[str]:
        """
        特徴量リストをロード (JSON / TXT 両対応)
        """
        p = Path(path)
        if not p.exists():
            raise FileNotFoundError(f"Feature list not found: {path}")

        # JSONの場合 (TOP_50_FEATURES.json対応)
        if p.suffix.lower() == ".json":
            import json

            self.logger.info(f"Loading feature list from JSON: {p.name}")
            with open(p, "r", encoding="utf-8") as f:
                features = json.load(f)
            if not isinstance(features, list):
                raise ValueError("JSON feature list must be a list of strings.")
            return features

        # TXTの場合 (旧互換)
        else:
            self.logger.info(f"Loading feature list from TXT: {p.name}")
            with open(p, "r", encoding="utf-8") as f:
    ...
    def _check_for_signal(self, tf_name: str, timestamp: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            if atr_value >= atr_threshold:
                ...
                # self.logger.debug(f"  -> R4 Check Failed: ATR {atr_value:.2f} < {atr_threshold:.2f}")
        ...
        try:
            ...
            for feature_name_in_list in self.feature_list:
                ...
                if (
                    ...
                    # キャッシュがない場合、または該当特徴量が存在しない場合
                    value = np.nan  # [V5修正] 一旦NaNとして扱い、後段で安全に補完する

                feature_vector.append(value)

            # 3. 最終的なベクトルをNaNチェックとクリッピング (厳密なマッピング)
            ...
            # [V5追加] NaN/Infを0.0に補完し、推論器を破壊する異常値をクリップ (-1e5 ~ 1e5)
