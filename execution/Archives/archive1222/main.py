        ...
        while True:
            try:
                ...
                for signal in signal_list:
                    ...
                    # ▼▼▼ [追加] Top 50特徴量 + AIスコアをCSVに保存 (分析用) ▼▼▼
                    try:
                        import csv

                        debug_csv_path = config.LOGS_DIR / "features_debug.csv"

                        # ヘッダー作成 (初回のみ)
                        if not debug_csv_path.exists():
                            with open(
                                debug_csv_path, "w", newline="", encoding="utf-8"
                            ) as f:
                                writer = csv.writer(f)
                                # ヘッダー: Timestamp, Timeframe, Score, ...FeatureNames...
                                header = [
                                    "timestamp",
                                    "timeframe",
                                    "ai_score",
                                ] + feature_engine.target_features
                                writer.writerow(header)

                        # 値の追記
                        with open(
                            debug_csv_path, "a", newline="", encoding="utf-8"
                        ) as f:
                            writer = csv.writer(f)

                            # スコアの取得 (extreme_risk_engineの修正により command['score'] が利用可能)
                            score = command.get("score", 0.0)

                            # 時間足名を取得 (signalオブジェクトにtf_name属性があると仮定、なければ "Signal")
                            tf_name = getattr(signal, "tf_name", "Signal")

                            # 行データ作成
                            # signal.features は (1, N) なので [0] を指定してリスト化
                            row = [
                                signal.timestamp.isoformat(),
                                tf_name,
                                score,
                            ] + signal.features[0].tolist()
                            writer.writerow(row)

                    except Exception as e:
                        logger.error(f"デバッグCSV保存エラー: {e}")
                    # ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲

                    # (F) 発注
                    if command["action"] != "HOLD":
                        logger.info(
                            f"-> 発注コマンドを送信: {command['action']} {command['lots']} lots (Score: {command.get('score', 0):.3f})"
                        )
                        ...
                        # HOLD時はDEBUGログ
                        logger.debug(
                            f"-> HOLD (エントリー見送り): {command['reason']} (Score: {command.get('score', 0):.3f})"
                        )
