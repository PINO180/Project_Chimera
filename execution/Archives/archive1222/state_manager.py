...
class StateManager:
    ...
    def save_checkpoint(self, state: SystemState):
        """
        現在の状態をJSONファイルとして保存（永続化）
        [修正] 頻繁に出る保存成功ログを DEBUG レベルに変更して静音化
        """
        try:
            # ファイル名の生成 (タイムスタンプ付き)
            filename = f"checkpoint_{state.timestamp.strftime('%Y%m%d_%H%M%S')}.json"
            filepath = self.checkpoint_dir / filename

            # 状態を辞書化
            state_dict = asdict(state)

            # datetimeオブジェクトを文字列に変換 (JSONシリアライズ用)
            state_dict["timestamp"] = state.timestamp.isoformat()

            # Tradeオブジェクト内のdatetimeも変換
            if "trades" in state_dict:
                for trade in state_dict["trades"]:
            ...
            # 古いチェックポイントの削除 (最新5件を残す)
            self._cleanup_old_checkpoints()

            # [修正] ログレベルを INFO -> DEBUG に変更
            logger.debug(f"✓ チェックポイント保存成功: {filepath}")
    ...
    def reconcile_with_broker(self, broker_state: BrokerStateDict) -> bool:
        ...
        [修正内容]
        1. ログレベルを INFO -> DEBUG に下げて静音化
        2. positions キーがない場合の安全策を追加
        3. ブローカーにあってローカルにないポジションのインポート処理(救済措置)を追加
        4. Balance の同期を追加
        """
        try:
            ...
            # [修正] 毎分出るログなので DEBUG に下げる
            logger.debug("ブローカー状態とローカル状態を整合性検証中...")
            ...
            if equity_diff > 0.01 or balance_diff > 0.01:
                # 差分がある場合のみ WARNING/INFO を出す（重要なため）
                logger.info(
                    f"口座情報の差分を検出して同期します: "
                ...
                self.current_state.current_balance = broker_state.get("balance", 0.0)
            ...
            # A. ブローカーに存在しないポジションをローカルから削除 (決済検知)
            ...
            # B. ブローカーにあるがローカルにないポジションを追加 (インポート救済措置)
                ...
                for pos_data in broker_positions:
                    ...
                    if ticket in missing_in_local:
                        try:
                            ...
                            logger.info(
                                f"  -> ポジションを復元しました: Ticket={ticket}"
                            )

                        except Exception as e:
                            logger.error(
                                f"  -> ポジション復元失敗 (Ticket={ticket}): {e}"
                            )

            # 状態を保存 (ここも前回の修正でDEBUG化済みならsave_checkpoint側で静かになる)
            self.save_checkpoint(self.current_state)

            # [修正] 完了ログも DEBUG に下げる
            logger.debug("✓ 状態の整合性を確保しました。")
            return True
