...
from blueprint import (
    ...
    S3_ARTIFACTS,
    # S3_FINAL_FEATURE_TEAM, S3_SHAP_SCORES は動的に生成するため削除
)

# --- ウォークフォワード設定 ---
TRAIN_PERIOD_MONTHS = 1  # 1ヶ月
VALIDATION_PERIOD_MONTHS = 1  # 1ヶ月
...
def main(test_mode: bool):
    ...
    # --- 修正点: 実験設定に基づいたユニークな出力先を作成 ---
    run_id = f"train_{TRAIN_PERIOD_MONTHS}m_val_{VALIDATION_PERIOD_MONTHS}m"
    output_dir = S3_ARTIFACTS / run_id
    output_dir.mkdir(parents=True, exist_ok=True)
    print(f"Results for this run will be saved in: {output_dir}")

    # 動的な出力パスを定義
    final_feature_team_path = output_dir / "final_feature_team.joblib"
    shap_scores_path = output_dir / "shap_scores.csv"
    signal_path = output_dir / "primary_model_signals.csv"
    ...
    pd.Series(final_feature_team).to_csv(
        final_feature_team_path, index=False, header=False
    )
    print(f"Final feature team saved to: {final_feature_team_path}")

    avg_shap_scores.to_csv(shap_scores_path, index=False)
    print(f"Average SHAP scores saved to: {shap_scores_path}")
