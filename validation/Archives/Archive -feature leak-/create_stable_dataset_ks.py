...
OUTPUT_DIR = Path(blueprint.S2_FEATURES_AFTER_KS)
...
def process_file(input_path: Path, output_path: Path, unstable_set: set):
    ...
    try:
        ...
        cols_to_drop = {
            col for col in original_cols if f"{col}{timeframe_suffix}" in unstable_set
        }

        if cols_to_drop:
            df_cleaned = df.drop(list(cols_to_drop))
            df_cleaned.write_parquet(
                output_path, compression="snappy", use_pyarrow=True
            )
...
def process_tick_directory(input_path: Path, output_path: Path, unstable_set: set):
    ...
    try:
        logger.info(
            f"  - {input_path.name}: パーティション単位でのストリーミング処理を開始"
        )
        ...
        for i, partition_file_path in enumerate(all_input_partitions):
            ...
            df_cleaned_partition.write_parquet(
                output_file_path, compression="snappy", use_pyarrow=True
            )

        avg_cols_dropped = (
            total_cols_dropped / len(all_input_partitions)
            if all_input_partitions
            else 0
        )
        logger.info(
            f"  - {input_path.name}: {len(all_input_partitions)}個のパーティションを処理完了。平均{avg_cols_dropped:.1f}列を削除。"
        )

    except Exception as e:
        logger.error(
            f"Tickディレクトリ処理エラー {input_path.name}: {e}", exc_info=True
        )
...
def main():
    ...
    for engine_dir in tqdm(engine_dirs, desc="Engineディレクトリ処理中"):
        ...
        for item_path in tqdm(
            items_to_process, desc=f"  {engine_dir.name}", leave=False
        ):
