...
def neutralize_lazyframe(
    ...
    丸め誤差が蓄積する可能性がある。
    """
    MIN_PERIODS = 30  # Chapter4と完全一致させる固定値
    ...
    x_mean = x.rolling_mean(window_size=window, min_periods=MIN_PERIODS)
    # E[X²] - E[X]² によるローリング分散、負値はclipでゼロに（ルール4）
    var_x = (
        (x * x).rolling_mean(window_size=window, min_periods=MIN_PERIODS)
        - x_mean * x_mean
    ...
    for suffixed_feature in feature_names:
        ...
        # 特徴量(Y): null を 0 で埋める
        y = pl.col(base_name).fill_null(0.0)
        y_mean = y.rolling_mean(window_size=window, min_periods=MIN_PERIODS)

        # ローリング共分散: E[XY] - E[X]E[Y]（ルール1: 過去方向のみ）
        cov_xy = (
            (x * y).rolling_mean(window_size=window, min_periods=MIN_PERIODS)
            - x_mean * y_mean
        )
