"""
2_A_skip_ks_stability_filter.py
================================
第一防衛線 - 分布安定性フィルター（KS検定スキップ・物理保存のみ版）

派生: 2_A_ks_stability_filter.py の Step 2 (KS検定) を常時スキップして、
既存の S3_ARTIFACTS/ks_unstable_features.json を読み、Step 3 (S2_FEATURES_VALIDATED
への物理保存・Float32 cast) のみ実行する増分処理用スクリプト。

用途:
    engine_1_X が delta 範囲だけを再生成した後、本スクリプト 1 個で
    S2_FEATURES → S2_FEATURES_VALIDATED の更新を完了させる。
    KS検定そのものは過去の計算結果 (json) を流用するため数十秒〜数分で済む。

フロー：
    S2_FEATURES（全エンジン出力）を走査
        ↓
    時間足を判定
      → LF_ALL_TIMEFRAMES（H4/H6/H12/D1/W1/MN）: KS検定スキップ・既存判定で除外
      → HF_TIMEFRAMES（M0.5〜H1）             : KS検定スキップ・そのままコピー
        ↓
    KS合格特徴量 + HF全特徴量 → S2_FEATURES_VALIDATED に物理保存
        ↓
    S3_ARTIFACTS / "ks_unstable_features.json" は読み取り専用 (既存維持)

実行:
    python 2_A_skip_ks_stability_filter.py
        (引数なし、CLI フラグ不要)

警告:
    S3_ARTIFACTS/ks_unstable_features.json が存在しない状態では実行不可。
    最初の 1 回は通常の 2_A_ks_stability_filter.py を完走させて JSON を
    生成する必要がある。
"""
...
def run_ks_filter(skip_ks_test: bool = False) -> None:
    ...
    if skip_ks_test:
        logger.info("  ⚠️  --skip-ks-test 指定: KS 検定をスキップして既存 JSON を使用")
    logger.info("=" * 70)
    ...
    if skip_ks_test:
        # ─── KS 検定スキップモード: 既存 JSON から unstable_features を load ───
        existing_json = S3_ARTIFACTS / "ks_unstable_features.json"
        if not existing_json.exists():
            raise FileNotFoundError(
                f"--skip-ks-test 指定だが、既存 JSON が見つかりません: {existing_json}\n"
                "通常モード (引数なし) で 1 回完走させてから再実行してください。"
            )
        logger.info(f"Step 2 [SKIP]: 既存 JSON を読み込みます: {existing_json}")
        with open(existing_json, "r", encoding="utf-8") as f:
            existing = json.load(f)
        unstable_features = set(existing.get("unstable_features", []))
        # ks_results は空のまま (Step 4 でも skip するので使われない)
        stable_count = len(lf_tasks) - len(unstable_features)
        logger.info(
            f"  既存 JSON から復元: 不安定={len(unstable_features)} / 合格={stable_count} / 計={len(lf_tasks)}"
        )
    else:
    ...
    if skip_ks_test:
        logger.info("  [SKIP] --skip-ks-test 指定のため、既存 JSON を保護 (上書きしません)")
    else:
        results_json = {
    ...
    # サマリーログ
    # =========================================================================
    hf_feat_count = sum(len(v) for v in hf_feature_names_per_source.values())
    logger.info("=" * 70)
    logger.info("2_A_skip_ks_stability_filter.py 完了")
    logger.info(f"  検定LF特徴量数    : {len(lf_tasks)}")
    logger.info(f"  不安定LF特徴量数  : {len(unstable_features)}")
    logger.info(f"  安定LF特徴量数    : {stable_count}")
    logger.info(f"  HF特徴量数（スキップ）: {hf_feat_count}")
...
def main() -> None:
    try:
        run_ks_filter(skip_ks_test=True)
        logger.info("✅ 2_A_skip_ks_stability_filter.py が正常に完了しました。")
    except Exception as e:
        logger.critical(
