...
def _correlation_filter_chunked(
    ...
    """
    チャンク処理版の相関フィルター。
    チャンク内の相関を計算し、削除対象を累積する。
    削除済み特徴量はチャンク間をまたいでも除外する。
    NOTE: チャンク間をまたいだペアの相関は検出対象外。
          通常の使用（特徴量数 < CHUNK_LIMIT=2000）では本関数は呼ばれないため実害なし。
    """
    to_drop: set[str] = set()
    surviving_so_far: list[str] = list(cols)

    for start in range(0, len(cols), chunk_size):
        chunk_cols = [c for c in surviving_so_far[start: start + chunk_size] if c not in to_drop]
        if len(chunk_cols) < 2:
            continue
        corr_chunk = df[chunk_cols].corr().abs()
        upper = corr_chunk.where(np.triu(np.ones(corr_chunk.shape), k=1).astype(bool))
        chunk_drop = {col for col in upper.columns if any(upper[col] >= threshold)}
        to_drop.update(chunk_drop)
