...
class FinalAssembler:
    def __init__(self, config: FinalTrainingConfig):
        ...
        # --- ★★★ [修正] M2の特徴量リストに文脈特徴量と "direction" を追加 ★★★ ---
        self.features_m2: List[str] = (
            self.features_base
            + ["m1_pred_proba"]
            + [
                "direction",  # ★ Buy(-1) / Sell(1) を区別するために追加
                "hmm_prob_0",
                "hmm_prob_1",
                "atr",
    ...
    def _train_single_model(
        ...
        for p_date in tqdm(partitions_to_train, desc=f"  Training {model_name}"):
            ...
            if missing_features:
                logging.warning(
                    f"Training partition {p_date} missing features for {model_name}: {missing_features}. Skipping chunk."
                )
                continue
