...
# [1] 実行するラベリングスクリプト名
SCRIPT_TO_RUN = (
    "/workspace/models/create_proxy_labels_polars_patch_for_grid_sellonly.py"
)
...
# ★★★ [Sell-Only 修正] R4 (Sell) のみに集中するため、他をコメントアウト ★★★
REGIMES = {
...
# --- ▼▼▼ [Adjusted_PF 導入] 定数の定義 ▼▼▼ ---
# スプレッドコスト（16.0 pips = 0.16ドル価格）
SPREAD_COST = 0.16

# 各レジームの平均ATR（仮定）
# (Adjusted_PF の計算にのみ使用)
REGIME_AVG_ATR = {
    "R1_LowVol": (0.23 + 1.0) / 2,  # 0.615
    "R2_MidVol": (1.0 + 2.5) / 2,  # 1.75
    "R3_HighVol": (2.5 + 5.0) / 2,  # 3.75
    "R4_SuperVol": 5.0,  # 上限がないため最小値 5.0 を代表値として使用
}
...
RESULTS_CSV_PATH = Path(
    "/workspace/data/XAUUSD/stratum_7_models/1A_2B/grid_search_results_SELL_ONLY.csv"
)
...
def analyze_results(output_dir: Path, regime_name: str) -> dict:
    """
    ラベリング結果（S6）を読み込み、パフォーマンスを集計する
    [修正版: Adjusted_PF (タイムアウトコスト考慮) を計算]
    [★★★ Buy/Sell 修正: direction == -1 (Sell) のみフィルタリング ★★★]
    """
    ...
    try:
        # --- ▼▼▼ [Sell-Only 修正] direction == -1 のみ抽出 ▼▼▼ ---
        df = (
            pl.scan_parquet(str(partition_path))
            .filter(pl.col("direction") == -1)  # Sellシグナルのみを対象
            .collect(streaming=True)
        )
        # --- ▲▲▲ [Sell-Only 修正] ここまで ▲▲▲ ---

        if df.is_empty():
            logging.warning("Analysis skipped: No SELL signals found for this run.")
            return {
                "wins": 0,
                "losses": 0,
                "timeouts": 0,
        ...
        # --- ▼▼▼ [Adjusted_PF 導入] 新しいPF計算 (Timeouts考慮) ▼▼▼ ---

        # このレジームの平均ATRと、このルールのPT/SL乗数を取得
        avg_atr = REGIME_AVG_ATR[regime_name]
        pt_mult = df["pt_multiplier"].mean()  # pt_mult, sl_mult は全行同じはず
        sl_mult = df["sl_multiplier"].mean()

        # 総利益 = 勝ち数 * (平均ATR * PT乗数)
        total_profit = wins * (avg_atr * pt_mult)

        # 総損失 = (負け数 * (平均ATR * SL乗数)) + (タイムアウト数 * スプレッドコスト)
        total_loss = (losses * (avg_atr * sl_mult)) + (timeouts * SPREAD_COST)

        if total_loss > 0:
            adj_pf = total_profit / total_loss
        else:
            adj_pf = np.inf if total_profit > 0 else 1.0  # 損失ゼロ
        # --- ▲▲▲ Adjusted_PF 計算ここまで ▲▲▲ ---
...
if __name__ == "__main__":
    logging.info("### Phase 2: Grid Search Optimizer START (SELL-ONLY) ###")
    ...
    for regime_name, regime_values in REGIMES.items():
        ...
        for pt, sl, td in param_combinations:
            ...
            # 2. 結果の集計
            # (修正: regime_name を渡す)
            stats = analyze_results(labeled_output_dir, regime_name)

            # --- ▼▼▼ [Adjusted_PF 導入] データ行の作成 ▼▼▼ ---
