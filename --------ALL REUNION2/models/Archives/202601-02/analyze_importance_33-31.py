...
if __name__ == "__main__":
    # --- [実験③: 精鋭選抜モード (33-31)] ---
    M1_JSON_PATH = PROJECT_ROOT / "models" / "TOP_M1_33_FEATURES.json"
    M2_JSON_PATH = PROJECT_ROOT / "models" / "TOP_M2_31_FEATURES.json"

    # M1 分析 (JSONを使用するが、M2ではないのでスコア注入はスキップされる)
    analyze_model_full(
        M1_MODEL_PATH, M1_JSON_PATH, "M1 Model (Directional - Top 33)", file_type="json"
    )

    # M2 分析 (JSONを使用し、M2なのでスコアが注入される)
    analyze_model_full(
        M2_MODEL_PATH,
        M2_JSON_PATH,
        "M2 Model (Meta-Labeling - Top 31)",
        file_type="json",
    )
