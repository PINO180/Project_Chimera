"""
adversarial_validation.py - 最終安定版（完全ディスク経由処理）
第一防衛線 - 敵対的検証

アーキテクチャ：
- 非tick・tick両方をディスク経由で逐次処理し、Killedを完全回避
- S2_FEATURES_AFTER_KS（KS検定通過済みの軽量データ）を参照
- cross_val_predictによる未来情報リークの防止とスレッド数制御
"""
...
def process_single_file_to_disk(
    file_path: str, output_dir: Path, file_index: int
) -> Dict:
    ...
    try:
        ...
        # ★Killed回避：LGBMのn_jobsを1に固定してスレッド爆発を防止
        ...
        # ★リーク回避：StratifiedKFold(shuffle=False)で時系列を保持したままCV予測
        # n_jobs=2 程度に抑えてメモリ負荷をコントロール
        # AVタスクではラベル(0/1)を振った後、ランダムにシャッフルしてOOFを作るのが正しい
        cv = StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
        y_pred_proba = cross_val_predict(
            model, X, y, cv=cv, method="predict_proba", n_jobs=2
        )[:, 1]

        # 特徴量重要度を取得するために全データで1回だけ再学習
        model.fit(X, y)
...
class AdversarialValidator:
        ...
        try:
            ...
            for imp_data in all_importances:
                ...
                for feat, score in imp_data["importances"].items():
                    all_scores[f"{feat}{suffix}"] = float(score)
            ...
            if auc > self.cfg.adversarial_auc_threshold:
                ...
                unstable_features = {
                    feat
                    for feat, score in all_scores.items()
    ...
    def _process_tick_data(self, tick_path: Path) -> Tuple[float, Dict, Set]:
        ...
        try:
            logger.info(f"tickデータ処理開始: {path_name}")
            ...
            for i, part_path in enumerate(partition_paths):
                try:
                    ...
                    if not feature_columns or len(df) < 100:
                        continue
                    ...
                    if (i + 1) % 50 == 0:
                        logger.info(
                            f"  ... {i + 1}/{len(partition_paths)} パーティションの処理完了"
                        )

                except Exception as e:
                    logger.warning(
                        f"パーティション {part_path.name} の処理でエラー: {e}"
                    )
                    continue
            ...
            if not all_y_true or len(np.unique(all_y_true)) < 2:
                auc = 0.5
                logger.warning(f"{path_name}: AUC計算不可。AUC=0.5とします。")
            else:
            ...
            adversarial_scores = {
                f"{feat}_tick": float(
                    np.mean([imp.get(feat, 0) for imp in all_feature_importances])
                )
            ...
            unstable_features = set()
            if auc > self.cfg.adversarial_auc_threshold:
                threshold_importance = np.percentile(
                    list(adversarial_scores.values()), 80
                )
                unstable_features = {
                    feat
                    for feat, score in adversarial_scores.items()
                    if score > threshold_importance
                }
    ...
    def run_validation(self) -> None:
        ...
        logger.info("=" * 60)
        logger.info("敵対的検証（完全ディスク経由処理・Killed回避版）を開始")
        logger.info("=" * 60)
        ...
        if parquet_files:
            non_tick_auc, non_tick_scores, non_tick_unstable = (
                self._process_non_tick_files(parquet_files)
            )
...
def main():
    try:
        logger.info("✓ シングルプロセスモード（並列数制御）で開始します")
