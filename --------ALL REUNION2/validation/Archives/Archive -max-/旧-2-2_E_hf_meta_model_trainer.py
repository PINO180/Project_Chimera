...
def main(test_mode: bool = False) -> None:
    ...
    base_feat_cols = [c for c in base_schema if c not in EXCLUDE_COLS and c != "timestamp"]
    ...
    for path in hf_parquet_paths[1:]:
        ...
        # サフィックス付与でカラム名衝突を回避
        # 既に base_lf に存在するカラム名と重複するものはスキップ（_right 衝突防止）
        existing_cols = set(base_lf.collect_schema().names())
        rename_map = {
            c: f"{c}_{tf_tag}"
            for c in feat_cols
            if f"{c}_{tf_tag}" not in existing_cols
        }
        if not rename_map:
            continue
        feat_cols = list(rename_map.keys())
        sub_lf = (
