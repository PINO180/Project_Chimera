...
def main(test_mode: bool) -> None:
    ...
    # ── 特徴量データ読み込み（ファイル別に存在するカラムのみselect）──
    # 全ファイルをまとめてscan_parquetすると、ファイルAにしか存在しない
    # カラムをファイルBに要求してColumnNotFoundErrorになるため、
    # ファイルごとにスキーマを確認して存在するカラムのみselectする。
    # ルール8: Float32にキャスト
    file_to_cols: dict[Path, list[str]] = {}
    for p in lf_parquet_files:
        try:
            schema = pl.read_parquet_schema(p)
            cols_in_file = [c for c in valid_base_cols if c in schema.keys()]
            if cols_in_file:
                file_to_cols[p] = cols_in_file
        except Exception:
            continue
    ...
    for p, cols in file_to_cols.items():
        part = (
            pl.scan_parquet(p)
            .select(["timestamp"] + cols)
            .with_columns([pl.col(c).cast(pl.Float32) for c in cols])  # ルール8
        )
    ...
    for group, cols in groups.items():
        # サフィックス付き名のうちvalid_lf_featuresに含まれるものを抽出し、
        # LightGBMに渡すカラム名はparquet実カラム名（base名）に変換する
        valid_suffixed = [c for c in cols if c in set(valid_lf_features)]
        valid_cols = [suffixed_to_base[c] for c in valid_suffixed]  # base名リスト
        if not valid_cols:
