...
class FeatureModule1C:
    ...
    def calculate_features(data: Dict[str, np.ndarray]) -> Dict[str, float]:
        ...
        # DPO
        for p in [20, 30, 50]:
            features[f"e1c_dpo_{p}"] = np.nan  # 未来参照回避のための制約維持
        ...
        if len(close_arr) >= 31:
            ...
            features["e1c_kst_signal"] = kst_val  # リアルタイム簡易実装
        else:
        ...
        features["e1c_tsi_13"] = _last(calculate_tsi_numba(close_arr, 25, 13))
        ...
        features["e1c_wma_200"] = wma_rolling_numba(_window(close_arr, 200), 200)
