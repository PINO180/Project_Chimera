...
class BacktestSimulator:
    def __init__(self, config: BacktestConfig):
        ...
        # --- ▼▼▼ [Buy/Sell 修正] M1の特徴量に direction を追加 ▼▼▼ ---
        if "direction" not in self.features_base:
            self.features_base.append("direction")
            logging.info("   -> Added 'direction' to M1 features list for simulation.")
        # --- ▲▲▲ [Buy/Sell 修正] ここまで ▲▲▲ ---
        ...
        # --- ▼▼▼ [Buy/Sell 修正] M2の特徴量リストを再構築（features_baseがdirectionを持つため） ▼▼▼ ---
        self.features_m2 = (
            self.features_base  # 'direction' を含む
            + ["m1_pred_proba"]
            + self.context_features_m2
        )
        # --- ▲▲▲ [Buy/Sell 修正] ここまで ▲▲▲ ---
    ...
    def _run_simulation_loop(
        ...
        # --- ▼▼▼ [1ポジルール修正] アクティブなポジションの終了時刻を初期化 ▼▼▼ ---
        # (df_chunk[0]のタイムスタンプより前に設定するため 0 を使う)
        active_position_exit_time_int = 0
        # --- ▲▲▲ [1ポジルール修正] ここまで ▲▲▲ ---
        ...
        for i in range(len(df_chunk)):
            ...
            # --- ▼▼▼ [1ポジルール修正] スキップロジック ▼▼▼ ---
            # 現在のシグナルが、アクティブなポジションの終了時刻より前の場合、
            # このシグナルを無視（スキップ）する
            if current_timestamp_int < active_position_exit_time_int:
                equity_values_chunk.append(current_capital)  # 資本は変動しない
                continue  # 次のシグナルへ
            # --- ▲▲▲ [1ポジルール修正] ここまで ▲▲▲ ---
                ...
                if effective_bet_fraction > DECIMAL_ZERO:
                    ...
                    if final_lot_size_decimal > DECIMAL_ZERO:
                        ...
                        if spread_cost_decimal >= current_capital:
                            ...
                            # --- ▼▼▼ [1ポジルール修正] 決済時刻を更新 ▼▼▼ ---
                            # この取引が決済される時刻（t1）をアクティブ時刻としてセット
                            current_t1_dt = t1_chunk[i].replace(tzinfo=dt.timezone.utc)
                            active_position_exit_time_int = int(
