#!/usr/bin/env python3
"""
革新的特徴量収集スクリプト - Engine 2A: Complexity Theory Features (修正版)
【実装内容】複雑性理論特徴量群(F5, F15, F16, F21)
...
def get_default_window_sizes() -> Dict[str, List[int]]:
    return {
        "mfdfa": [100, 200, 500],  # MFDFA用ウィンドウ
        "complexity": [50, 100, 200],  # 複雑性指標用ウィンドウ
        "chaos": [100, 200, 500],  # カオス理論用ウィンドウ
        "general": [50, 100, 200, 500]
    }

@dataclass
class ProcessingConfig:
    """処理設定 - Project Forge統合版 (Engine 2A: Complexity Theory)"""
    ...
    engine_name: str = "Engine_2A_ComplexityTheory"
    ...
    w_max: int = 500  # 複雑性理論特徴量の最大ウィンドウ
...
@nb.njit(fastmath=True, cache=True)
def mfdfa_core_single_window(prices: np.ndarray, q_values: np.ndarray, scales: np.ndarray, poly_degree: int = 1) -> np.ndarray:
    """
...
def mfdfa_rolling_udf(prices: np.ndarray, window: int, component_idx: int) -> np.ndarray:
    """
    ...
    for i in nb.prange(window, n + 1):
        ...
        mfdfa_result = mfdfa_core_single_window(window_data, q_values, scales, poly_degree=1)
...
@nb.njit(fastmath=True, cache=True, parallel=True)
def kolmogorov_complexity_rolling_udf(prices: np.ndarray, window: int, component_idx: int) -> np.ndarray:
    """
...
# ===============================
# Block 4 Start: カオス理論指標UDF定義（修正版）
# ===============================
...
# ===============================
# Block 4 Complete: カオス理論指標UDF定義（修正版）
# ===============================

# ===============================
# Block 5 Start: Lempel-Ziv複雑性UDF定義（修正版）
# ===============================
...
# ===============================
# Block 5 Complete: Lempel-Ziv複雑性UDF定義（修正版）
# ===============================

# ===============================
# Block 6 Start: DataEngine/QualityAssurance クラス
# ===============================
...
# ===============================
# Block 6 Complete: DataEngine/QualityAssurance
# ===============================

# ===============================
# Block 7 Start: CalculationEngine/OutputEngine/メイン実行部（最終）
# ===============================

class CalculationEngine:
    """
    計算核心クラス(60%) - Project Forge統合版(複雑性理論特徴量専用)
    ...
    def _get_all_feature_expressions(self) -> Dict[str, pl.Expr]:
        ...
        for window in self.config.window_sizes["mfdfa"]:
            ...
            expressions[f"{p}mfdfa_hurst_mean_{window}"] = pl.col("close").map_batches(
                lambda s, w=window: mfdfa_rolling_udf(s.to_numpy(), w, 0),
                return_dtype=pl.Float64
            ).alias(f"{p}mfdfa_hurst_mean_{window}")
            
            expressions[f"{p}mfdfa_width_{window}"] = pl.col("close").map_batches(
                lambda s, w=window: mfdfa_rolling_udf(s.to_numpy(), w, 1),
                return_dtype=pl.Float64
            ).alias(f"{p}mfdfa_width_{window}")
            
            expressions[f"{p}mfdfa_holder_max_{window}"] = pl.col("close").map_batches(
        ...
        for window in self.config.window_sizes["complexity"]:
            ...
            expressions[f"{p}kolmogorov_complexity_{window}"] = pl.col("close").map_batches(
                lambda s, w=window: kolmogorov_complexity_rolling_udf(s.to_numpy(), w, 0),
                return_dtype=pl.Float64
            ).alias(f"{p}kolmogorov_complexity_{window}")
            
            expressions[f"{p}compression_ratio_{window}"] = pl.col("close").map_batches(
                lambda s, w=window: kolmogorov_complexity_rolling_udf(s.to_numpy(), w, 1),
                return_dtype=pl.Float64
            ).alias(f"{p}compression_ratio_{window}")
            
            expressions[f"{p}pattern_diversity_{window}"] = pl.col("close").map_batches(
                lambda s, w=window: kolmogorov_complexity_rolling_udf(s.to_numpy(), w, 2),
                return_dtype=pl.Float64
    ...
    def _create_vertical_slices(self) -> Dict[str, Dict[str, pl.Expr]]:
        ...
        slices["kolmogorov"] = {
            name: expr for name, expr in all_expressions.items() 
            if "kolmogorov" in name or "compression_ratio" in name or "pattern_diversity" in name
        }
...
class OutputEngine:
    ...
    def create_summary_report(self, processing_metadata: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        summary = {
            "engine_id": self.engine_id,
            "engine_name": "Engine_2A_ComplexityTheory",
...
def run_on_partitions_mode(config: ProcessingConfig, resume_date: Optional[datetime.date] = None):
    ...
    # 既存のファイル/ディレクトリの競合を解決
    if FEATURES_ROOT.exists():
        if FEATURES_ROOT.is_file():
            logging.warning(f"出力パスがファイルとして存在します。削除します: {FEATURES_ROOT}")
            FEATURES_ROOT.unlink()
            FEATURES_ROOT.mkdir(parents=True, exist_ok=True)
        else:
            logging.info(f"出力ディレクトリは既に存在します: {FEATURES_ROOT}")
    else:
        FEATURES_ROOT.mkdir(parents=True, exist_ok=True)
        logging.info(f"出力ディレクトリを作成: {FEATURES_ROOT}")
...
def main():
    ...
    print("\n" + "="*70)
    print(f"  Engine 2A - Complexity Theory Features (修正版)")
    print("  Project Forge - 軍資金増大プロジェクト  ")
    print("="*70)
...
# ===============================
# Block 7 Complete: CalculationEngine/OutputEngine/メイン実行部
# ===============================
# 
# 全7ブロック完成: engine_2_complexity_theory.py (修正版)
#
# 【重大修正完了】
# 1. 重量級UDF呼び出しパターン: rolling_map → map_batches に完全移行
# 2. 遅延束縛バグ: デフォルト引数による即時束縛で完全修正
# 3. UDF並列化: 全てのローリングUDFでprange並列化を実装
# 4. 設計規律遵守: Project Forgeプロンプトの必須パターンに完全準拠
#
# 実装完了した特徴量:
# - F5: MFDFA (マルチフラクタル解析)
# - F15: コルモゴロフ複雑性
...
# - CalculationEngine: 複雑性理論特徴量専用実装（修正版）
