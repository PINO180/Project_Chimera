...
class ProcessingConfig:
    ...
    # エンジン識別
    engine_id: str = "e1a"  # Engine 1A用に修正
    engine_name: str = "Engine_1A_Basic_Statistical_Refactored"  # 刷新版の名称
            ...
            if len(finite_data) < 10:
                ...
                if std_val < 1e-10:
                    ...
                    for j in range(n_data):
                        ...
                        F_nj = standard_normal_cdf_fast(standardized_data[n_data-1-j])
...
def basic_stabilization_numba(arr, out):
    ...
    for i in range(n):
        if np.isfinite(out[i]): 
            finite_count += 1
    if finite_count > 10:
        min_val = np.nanmin(out); max_val = np.nanmax(out); range_val = max_val - min_val
        if range_val > 1e-10:
...
def robust_stabilization_numba(arr, out):
    ...
    if len(finite_vals) < 3:
        for i in range(n): 
            out[i] = 0.0
        return
    sorted_vals = sorted(finite_vals); n_finite = len(sorted_vals)
    if n_finite % 2 == 0: 
    ...
    sorted_abs_devs = sorted(abs_devs); n_abs = len(sorted_abs_devs)
    if n_abs % 2 == 0: 
    ...
    lower_bound = median_val - 3 * mad_val; upper_bound = median_val + 3 * mad_val
    for i in range(n):
        if np.isnan(arr[i]): 
            out[i] = median_val
        elif np.isinf(arr[i]):
...
class CalculationEngine:
    ...
    def _get_all_feature_expressions(self) -> Dict[str, pl.Expr]:
        ...
        for window in [10, 20, 50]:
            expressions[f"{p}statistical_mean_{window}"] = pl.col("close").rolling_mean(window).alias(f"{p}statistical_mean_{window}")
            expressions[f"{p}statistical_variance_{window}"] = pl.col("close").rolling_var(window).alias(f"{p}statistical_variance_{window}")
            expressions[f"{p}statistical_std_{window}"] = pl.col("close").rolling_std(window).alias(f"{p}statistical_std_{window}")
            expressions[f"{p}statistical_cv_{window}"] = (pl.col("close").rolling_std(window) / pl.col("close").rolling_mean(window)).alias(f"{p}statistical_cv_{window}")
        
        for window in [20, 50]:
            expressions[f"{p}statistical_skewness_{window}"] = pl.col("close").rolling_skew(window_size=window).alias(f"{p}statistical_skewness_{window}")
            expressions[f"{p}statistical_kurtosis_{window}"] = ((pl.col("close") - pl.col("close").rolling_mean(window)).pow(4).rolling_mean(window) / pl.col("close").rolling_std(window).pow(4) - 3).alias(f"{p}statistical_kurtosis_{window}")
            for moment in [5, 6, 7, 8]:
                mean_col = pl.col("close").rolling_mean(window)
                std_col = pl.col("close").rolling_std(window)
                expr = ((pl.col("close") - mean_col) / std_col).pow(moment).rolling_mean(window)
                expressions[f"{p}statistical_moment_{moment}_{window}"] = expr.alias(f"{p}statistical_moment_{moment}_{window}")

        # --- Robust Statistics Features ---
        for window in [10, 20, 50]:
            expressions[f"{p}robust_median_{window}"] = pl.col("close").rolling_median(window).alias(f"{p}robust_median_{window}")
            q25 = pl.col("close").rolling_quantile(0.25, window_size=window)
            q75 = pl.col("close").rolling_quantile(0.75, window_size=window)
            expressions[f"{p}robust_q25_{window}"] = q25.alias(f"{p}robust_q25_{window}")
            expressions[f"{p}robust_q75_{window}"] = q75.alias(f"{p}robust_q75_{window}")
            expressions[f"{p}robust_iqr_{window}"] = (q75 - q25).alias(f"{p}robust_iqr_{window}")
            expressions[f"{p}robust_trimmed_mean_{window}"] = pl.col("close").rolling_map(lambda s: trim_mean(s.to_numpy(), proportiontocut=0.1), window_size=window).alias(f"{p}robust_trimmed_mean_{window}")

        # --- Advanced Robust Features ---
        expressions[f"{p}robust_mad_20"] = pl.col("close").map_batches(lambda s: mad_rolling_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}robust_mad_20")
        expressions[f"{p}robust_biweight_location_20"] = pl.col("close").map_batches(lambda s: biweight_location_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}robust_biweight_location_20")
        expressions[f"{p}robust_winsorized_mean_20"] = pl.col("close").map_batches(lambda s: winsorized_mean_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}robust_winsorized_mean_20")
        
        # --- Statistical Tests Features ---
        expressions[f"{p}jarque_bera_statistic_50"] = pl.col("close").map_batches(lambda s: jarque_bera_statistic_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}jarque_bera_statistic_50")
        expressions[f"{p}anderson_darling_statistic_30"] = pl.col("close").map_batches(lambda s: anderson_darling_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}anderson_darling_statistic_30")
        expressions[f"{p}runs_test_statistic_30"] = pl.col("close").map_batches(lambda s: runs_test_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}runs_test_statistic_30")
        expressions[f"{p}von_neumann_ratio_30"] = pl.col("close").map_batches(lambda s: von_neumann_ratio_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}von_neumann_ratio_30")
        
        # --- Basic Processing / Fast Features ---
        for window in [5, 10, 20, 50, 100]:
            expressions[f"{p}fast_rolling_mean_{window}"] = pl.col("close").rolling_mean(window).alias(f"{p}fast_rolling_mean_{window}")
            expressions[f"{p}fast_rolling_std_{window}"] = pl.col("close").rolling_std(window).alias(f"{p}fast_rolling_std_{window}")
            expressions[f"{p}fast_volume_mean_{window}"] = pl.col("volume").rolling_mean(window).alias(f"{p}fast_volume_mean_{window}")
            
        # --- Numba Optimized Basic Features ---
        expressions[f"{p}fast_rolling_mean_numba_20"] = pl.col("close").map_batches(lambda s: fast_rolling_mean_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}fast_rolling_mean_numba_20")
        expressions[f"{p}fast_rolling_std_numba_20"] = pl.col("close").map_batches(lambda s: fast_rolling_std_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}fast_rolling_std_numba_20")
        expressions[f"{p}fast_quality_score_50"] = pl.col("close").map_batches(lambda s: fast_quality_score_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}fast_quality_score_50")

        # --- Quality Assurance Features ---
        expressions[f"{p}fast_basic_stabilization"] = pl.col("close").map_batches(lambda s: basic_stabilization_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}fast_basic_stabilization")
        expressions[f"{p}fast_robust_stabilization"] = pl.col("close").map_batches(lambda s: robust_stabilization_numba(s.to_numpy()), return_dtype=pl.Float64).alias(f"{p}fast_robust_stabilization")
    ...
    def _create_vertical_slices(self) -> Dict[str, Dict[str, pl.Expr]]:
        ...
        slices["group_1_moments"] = {
            ...
            if any(pattern in name for pattern in ["statistical_mean", "statistical_variance", "statistical_std", "statistical_cv"])
        }
        
        # グループ2: 歪度・尖度（中程度）
        slices["group_2_skew_kurt"] = {
            name: expr for name, expr in all_expressions.items() 
            if any(pattern in name for pattern in ["statistical_skewness", "statistical_kurtosis", "statistical_moment"])
        }
        
        # グループ3: ロバスト統計（中程度）
        slices["group_3_robust"] = {
            name: expr for name, expr in all_expressions.items() 
            if any(pattern in name for pattern in ["robust_median", "robust_q25", "robust_q75", "robust_iqr", "robust_trimmed_mean"])
        }
        
        # グループ4: 高度ロバスト統計（重い）
        slices["group_4_advanced"] = {
            name: expr for name, expr in all_expressions.items() 
            if any(pattern in name for pattern in ["robust_mad", "robust_biweight_location", "robust_winsorized_mean"])
        }
        
        # グループ5: 統計検定（重い）
        slices["group_5_tests"] = {
            name: expr for name, expr in all_expressions.items() 
            if any(pattern in name for pattern in ["jarque_bera", "anderson_darling", "runs_test", "von_neumann_ratio"])
        }
        
        # グループ6: 高速処理特徴量（軽量）
        slices["group_6_fast"] = {
            name: expr for name, expr in all_expressions.items() 
            if any(pattern in name for pattern in ["fast_rolling_mean", "fast_rolling_std", "fast_volume_mean"]) and "numba" not in name
        }
        
        # グループ7: Numba最適化（中程度）
        slices["group_7_numba"] = {
            name: expr for name, expr in all_expressions.items() 
            if any(pattern in name for pattern in ["fast_rolling_mean_numba", "fast_rolling_std_numba", "fast_quality_score"])
        }
        
        # グループ8: 品質保証（軽量）
        slices["group_8_qa"] = {
            name: expr for name, expr in all_expressions.items() 
            if any(pattern in name for pattern in ["fast_basic_stabilization", "fast_robust_stabilization"])
        }
    ...
    def _create_basic_stats_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for window in [10, 20, 50]:
            exprs.append(pl.col("close").rolling_mean(window).alias(f"{p}statistical_mean_{window}"))
            exprs.append(pl.col("close").rolling_var(window).alias(f"{p}statistical_variance_{window}"))
            exprs.append(pl.col("close").rolling_std(window).alias(f"{p}statistical_std_{window}"))
            exprs.append((pl.col("close").rolling_std(window) / pl.col("close").rolling_mean(window)).alias(f"{p}statistical_cv_{window}"))
    ...
    def _create_skew_kurt_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for window in [20, 50]:
            exprs.append(pl.col("close").rolling_skew(window_size=window).alias(f"{p}statistical_skewness_{window}"))
            exprs.append(((pl.col("close") - pl.col("close").rolling_mean(window)).pow(4).rolling_mean(window) / pl.col("close").rolling_std(window).pow(4) - 3).alias(f"{p}statistical_kurtosis_{window}"))
    ...
    def _create_robust_stats_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        for window in [10, 20, 50]:
            exprs.append(pl.col("close").rolling_median(window).alias(f"{p}robust_median_{window}"))
            ...
            exprs.extend([
                ...
                pl.col("close").rolling_map(lambda s: trim_mean(s.to_numpy(), proportiontocut=0.1), window_size=window).alias(f"{p}robust_trimmed_mean_{window}")
            ])
        
        return lazy_frame.with_columns(exprs)

    def _create_advanced_robust_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        exprs.append(pl.col("close").map_batches(
            lambda s: mad_rolling_numba(s.to_numpy()),
    ...
    def _create_statistical_tests_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        ...
        exprs.extend([
            ...
            pl.col("close").map_batches(
                lambda s: runs_test_numba(s.to_numpy()),
                return_dtype=pl.Float64
    ...
    def _create_fast_processing_features(self, lazy_frame: pl.LazyFrame) -> pl.LazyFrame:
        """基本データ処理特徴量（高速実装）"""
        exprs = []
        p = self.prefix
        ...
        for window in [5, 10, 20, 50, 100]:
            # 高速ローリング平均
            exprs.append(pl.col("close").rolling_mean(window).alias(f"{p}fast_rolling_mean_{window}"))
            
            # 高速ローリング標準偏差
            exprs.append(pl.col("close").rolling_std(window).alias(f"{p}fast_rolling_std_{window}"))
            
            # 出来高の高速ローリング平均
            exprs.append(pl.col("volume").rolling_mean(window).alias(f"{p}fast_volume_mean_{window}"))
