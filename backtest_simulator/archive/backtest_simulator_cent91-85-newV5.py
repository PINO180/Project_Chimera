...
class BacktestConfig:
    ...
    m1_feature_list_path: Path = (
        project_root / "models" / "selected_features91-85_m1.txt"
    )
    m2_feature_list_path: Path = (
        project_root / "models" / "selected_features91-85_m2.txt"
    )
...
class BacktestSimulator:
    def __init__(self, config: BacktestConfig):
        ...
        # ★★★ [修正] M1/M2 それぞれの厳選リスト(txt)を読み込む ★★★
        self.features_m1 = self._load_features_from_txt(config.m1_feature_list_path)
        logging.info(
            f"M1 Features: Loaded {len(self.features_m1)} items from {config.m1_feature_list_path.name}."
        )

        self.features_m2 = self._load_features_from_txt(config.m2_feature_list_path)
        logging.info(
            f"M2 Features: Loaded {len(self.features_m2)} items from {config.m2_feature_list_path.name}."
        )
    ...
    def _run_ai_predictions(self, df_chunk: pl.DataFrame) -> pl.DataFrame:
        ...
        calc_df = calc_df.with_columns(
            [
                (pl.col("e1e_hilbert_amplitude_50") > pl.col("v5_hilbert_mean") * 1.00)
                .fill_null(False)
                .alias("v5_trigger"),
                (pl.col("e1a_statistical_skewness_20") >= -1.52)
                .fill_null(False)
                .alias("v5_pass_skew"),
                (pl.col("e1a_statistical_kurtosis_20") <= 12.0)
                .fill_null(False)
                .alias("v5_pass_kurt"),
                (pl.col("e1e_hilbert_phase_stability_50") >= 0.25)
                .fill_null(False)
                .alias("v5_pass_phase"),
                (pl.col("v5_body_ratio") <= 0.85)
                .fill_null(False)
                .alias("v5_pass_body"),
            ]
        )
