# /workspace/backtest_simulator/backtest_simulator_cimera_stage3.py
# ============================================================================
# 【回帰版BT stage3 — 前処理(動的決済 tick走査)を BT に統合した完全版】
# ----------------------------------------------------------------------------
# stage2 との違い:
#   - 旧 compute_dynamic_exits_regression.py を廃し、動的TP/SLの tick 走査を
#     BT 内 (_compute_exits_in_process) で実行。前処理スクリプト不要。
#   - M1/M2 切替が実際に効く (stage2 は M2 でも M1 予測を使うバグがあった)。
#     → 起動時プロンプトで M1/M2 を選ぶと、その OOF 予測が実際に使われる。
#   - --year-month / --year で tick走査+BT を一部期間だけに限定可 (1ヶ月BTが軽い)。
#   - 動的決済は月ごとにキャッシュ (mode/TO/K_TP/K_SL でキー化) → 再実行・M1M2切替が速い。
#
# 調整パラメータ (このファイル上部):
#   TO_MINUTES / K_TP / K_SL … tick走査の窓・TP/SL水準。ここを編集して調整。
#   BacktestConfig 上部 … 確信度ゲート/ATR帯/セッション (stage2 と同じ)。
#
# 【実行コマンド例】
#   1ヶ月だけ (2021/7) BT:
#     python backtest_simulator/backtest_simulator_cimera_stage3.py --year-month 2021/7
#   1年 (2021) BT:
#     python backtest_simulator/backtest_simulator_cimera_stage3.py --year 2021
#   全期間:
#     python backtest_simulator/backtest_simulator_cimera_stage3.py
#   起動時に [1]M2 / [2]M1 を選択。決済キャッシュを無視して走査し直すなら --no-exits-cache。
# ============================================================================
...
# [回帰版 2026-07-12] 前処理(compute_dynamic_exits_regression.py)の出力ディレクトリ。
# 各バーの動的TP/SL tick走査済み決済結果(direction/outcome/exit_time/exit_price)を持つ。
S7_DYNAMIC_EXITS = S7_MODELS / "backtest_simulator_regression" / "dynamic_exits"
_REG_BT_RESULTS = S7_MODELS / "backtest_simulator_regression" / "results"

# --- 出力ファイルパス（実行時に動的生成）---
FINAL_REPORT_PATH = _REG_BT_RESULTS / "final_backtest_report_v5.json"  # 起動時に上書き
EQUITY_CURVE_PATH = _REG_BT_RESULTS / "equity_curve_v5.png"  # 起動時に上書き
...
# ================================================================
# セッション判定 (JST時間帯基準)。レポート集計とエントリーフィルタで共通利用。
# ================================================================
def hour_to_session_jst(h: int) -> str:
    """JST時間 h (0-23) をセッション名に変換 (レポートの _session と同一定義)。"""
    if 9 <= h < 16:
        return "Tokyo"
    elif 16 <= h < 21:
        return "London"
...
def utc_ts_to_session_jst(ts) -> str:
    """UTC timestamp を JST 時間に直してセッション名を返す。"""
    try:
        h_jst = (ts.hour + 9) % 24
    except Exception:
        return "NA"
    return hour_to_session_jst(h_jst)


def _regression_pred_bins(vals) -> dict:
    """回帰予測(mfe_atr/mae_atr、ATR値)のヒストグラム。確率でなく回帰値用。"""
    return {
        "<= 0.0": sum(1 for x in vals if x is not None and x <= 0.0),
        "0.0-0.5": sum(1 for x in vals if x is not None and 0.0 < x <= 0.5),
        "0.5-1.0": sum(1 for x in vals if x is not None and 0.5 < x <= 1.0),
        "1.0-2.0": sum(1 for x in vals if x is not None and 1.0 < x <= 2.0),
        "2.0-3.0": sum(1 for x in vals if x is not None and 2.0 < x <= 3.0),
        "3.0-5.0": sum(1 for x in vals if x is not None and 3.0 < x <= 5.0),
        "5.0-8.0": sum(1 for x in vals if x is not None and 5.0 < x <= 8.0),
        "8.0-12.0": sum(1 for x in vals if x is not None and 8.0 < x <= 12.0),
        ">= 12.0": sum(1 for x in vals if x is not None and x > 12.0),
    }


# ================================================================
# [stage3] 動的TP/SL tick走査を BT 内に統合 (旧 compute_dynamic_exits を移植)
# ----------------------------------------------------------------
#   TO_MINUTES : TO窓(分)。ラベリングの TD と揃える。K_TP/K_SL: TP/SL を予測の何倍に置くか。
#   これらは tick 走査に効く水準/窓パラメータ。ここを直接編集して調整する。
# ================================================================
TO_MINUTES = 15
K_TP = 1.0
K_SL = 1.0

# [B1] 方向脳 (net_dominance) の M1 OOF。Ax2 が出力する。
S7_M1_OOF_PREDICTIONS_DOMINANCE = (
    S7_M1_OOF_PREDICTIONS_LONG.parent / "m1_oof_predictions_dominance.parquet"
)
DYN_ACTION_HORIZON_SEC = (
    180  # エントリーオフセット(秒)。ラベリング ACTION_HORIZON_SEC と一致。
)
...
def _numba_find_hits_dual(
    ...
    """ラベリングと同一の双方向 tick 走査 (検証済み)。各 bet の (t0+offset, t1] を走査し
    PT/SL の最初の到達時刻(us)を返す。0=未到達。tick は単一点 (high=low=mid)。"""
    n_bets = len(bets_t0)
...
def _dyn_compute_dual_levels(df: pl.DataFrame) -> pl.DataFrame:
    """全バーについて Long/Short 両方向の動的TP/SL価格を計算 (ゲート無し)。"""
...
def _dyn_resolve_outcomes(df: pl.DataFrame, pt_l, sl_l, pt_s, sl_s) -> pl.DataFrame:
    """カーネルの到達時刻から Long/Short 両方向の outcome/exit_time/exit_price を出す。
    outcome: 1=TP先着 / 0=SL先着 / -1=TO。exit_price: TO は null(BT の close_future)。"""
    df = df.with_columns(
    ...
    df = df.with_columns(
        (
            ...
            + int(DYN_ACTION_HORIZON_SEC * 1_000_000)
...
class BacktestConfig:
    ...
    # [回帰版] S6_WEIGHTED は使わず S6_LABELED を直読み(重み付けスキップ)
    simulation_data_path: Path = S6_LABELED_DATASET

    # [回帰版] 前処理(動的TP/SL tick走査)の出力パス。BT はこれを再生する。
    dynamic_exits_path: Path = S7_DYNAMIC_EXITS

    # 期間フィルタ (YYYY-MM-DD, UTC, inclusive)。None なら全期間
    start_date: Optional[str] = None
    end_date: Optional[str] = None

    # V5: Long/Short独立のOOF予測パス (回帰版では前処理が消費。BT は前処理出力を読む)
    oof_long_path: Path = S7_M1_OOF_PREDICTIONS_LONG
    oof_short_path: Path = S7_M1_OOF_PREDICTIONS_SHORT
    ...
    # [残骸/未使用] 旧確率メタラベリングの閾値。回帰版では未使用(比ゲートに置換)。
    m2_proba_threshold: float = 0.70
    m2_delta_threshold: float = 0.30

    # ========================================================================
    # [回帰版 段階2] エントリー用パラメータ ★ここを直接編集して調整する★
    #   (前処理を回し直さずここで調整可。引数でも上書きできるが、通常はここを編集)
    # ------------------------------------------------------------------------
    #   entry_ratio_threshold: 確信度ゲート(比)。片方の予測がもう片方の何倍超で発注。
    #       1.2=1.2倍超で発注 / 1.0=常に発注(比ゲート無効相当)
    #
    #   atr_ratio_min / atr_ratio_max: 発注を許可する atr_ratio の"帯"(min<=x<max)。
    #       全バー          → min=0.0,  max=float("inf")
    #       中ボラだけ狙う  → min=1.0,  max=1.5
    #       低ボラ除外      → min=1.0,  max=float("inf")
    #
    #   session_filter: 発注を許可するセッション(JST時間帯基準)のリスト。
    #       全セッション    → None
    #       London/NYのみ   → ["London", "NY"]
    #       選択肢: "Tokyo"(JST9-16) / "London"(16-21) / "Overlap"(21-1) /
    #               "NY"(1-6) / "Oceania"(6-9)
    #
    #   min_pred_atr: 勝つ側の予測がこの ATR 未満なら発注しない。0.0=無効。
    #   ※ K_TP/K_SL(TP/SL水準)は前処理側パラメータ。変えたら前処理を回し直す。
    # ========================================================================
    entry_ratio_threshold: float = 1.2
    atr_ratio_min: float = 0.0
    atr_ratio_max: float = float("inf")
    session_filter: Optional[List[str]] = None
    min_pred_atr: float = 1.5
    # [B1] エントリー方式:
    #   "dominance" … 方向脳 (M1-dominance の OOF 予測 dom_pred) の符号で方向を決める。
    #                  dom_pred >= +dominance_threshold → Long / <= −同 → Short。
    #                  |dom_pred| が閾値未満なら見送り。要 m1_oof_predictions_dominance.parquet。
    #   "ratio"     … 従来の確信度比 (mfe_pred/mae_pred >= entry_ratio_threshold)。
    entry_mode: str = "dominance"
    dominance_threshold: float = 0.3
    # ------------------------------------------------------------------------
    # ★選択肢の参照★ (テキストレポートの分布・Band Analysis と同じ区分)
    #
    #   atr_ratio の区分 (ATR Ratio Band Analysis の帯):
    #       < 0.5 / 0.5-0.8 / 0.8-1.0 / 1.0-1.2 / 1.2-1.5 / 1.5-2.0 / 2.0-3.0 / >= 3.0
    #     → atr_ratio_min / atr_ratio_max はこの境界値から選ぶと分布と対応する。
    #       例) 1.0-1.5 の帯だけ  → min=1.0, max=1.5
    #           1.5 以上を除外    → min=0.0, max=1.5
    #
    #   session の選択肢 (Session Summary と同じ / JST時間帯):
    #       "Tokyo"   … JST 09-16
    #       "London"  … JST 16-21
    #       "Overlap" … JST 21-01
    #       "NY"      … JST 01-06
    #       "Oceania" … JST 06-09
    #     → session_filter = ["London", "NY"] のように複数指定可。None=全セッション。
    # ------------------------------------------------------------------------
    # 前処理のエントリーオフセット(秒)。保有時間表示の補正に使う(前処理と一致させる)。
    action_horizon_sec: int = 180

    # [stage3] tick走査する対象期間 (None=全期間)。1ヶ月BTなら (2021, 7) 等。
    target_year: Optional[int] = None
    target_month: Optional[int] = None
    # [stage3] 算出した動的決済のキャッシュ利用 (True=キャッシュがあれば再利用、無ければ算出保存)
    use_exits_cache: bool = True
    ...
    # ==========================================
    # [残骸/未使用] 固定TP/SL倍率。回帰版では決済は前処理の動的水準で確定するため未使用。
    # ==========================================
    sl_multiplier_long: float = 0.5
    pt_multiplier_long: float = 1.0
    sl_multiplier_short: float = 0.5
    pt_multiplier_short: float = 1.0

    # [stage3] TO決済価格(close_future)参照窓。ファイル上部の TO_MINUTES に自動追従する
    #   ので、ここは編集しない。TO窓を変えるときは上部の TO_MINUTES を書き換えるだけ。
    td_minutes_long: float = float(TO_MINUTES)
    td_minutes_short: float = float(TO_MINUTES)
...
class BacktestSimulator:
    ...
    def _compute_exits_in_process(self) -> pl.DataFrame:
        """[stage3] 旧 compute_dynamic_exits を BT 内に統合。
        config.oof_long_path/short_path (M1 or M2) から予測を読み、S6 と結合、
        両方向の動的TP/SL水準を作り、対象月の tick を走査して決済結果を出す。
        月ごとにキャッシュ (mode/K_TP/K_SL/TO/年月 でキー化) → M1/M2 切替や再実行が速い。
        返り値スキーマは旧前処理の出力と同一 (timestamp..mae_pred)。"""
        mode = "M1" if "m1_oof" in str(self.config.oof_long_path) else "M2"
        cache_root = (
            S7_MODELS
            / "backtest_simulator_regression"
            / "exits_cache_stage3"
            / f"{mode}_TO{int(TO_MINUTES)}_KTP{K_TP}_KSL{K_SL}"
        )

        # --- 予測ロード (ここで M1/M2 が実際に切り替わる) ---
        logging.info(
            f"[stage3] Loading {mode} OOF predictions (mfe=long / mae=short)..."
        )
        mfe = (
            pl.read_parquet(self.config.oof_long_path)
            .rename({"prediction": "mfe_pred"})
            .select(["timestamp", "timeframe", "mfe_pred"])
        )
        mae = (
            pl.read_parquet(self.config.oof_short_path)
        ...
        # [B1] 方向脳 dominance OOF (M1)。ファイルが無ければ dom_pred=null で続行
        #   (entry_mode="ratio" 用)。dominance モードで無ければ起動時に停止する。
        if S7_M1_OOF_PREDICTIONS_DOMINANCE.exists():
            dom = (
                pl.read_parquet(S7_M1_OOF_PREDICTIONS_DOMINANCE)
                .rename({"prediction": "dom_pred"})
                .select(["timestamp", "timeframe", "dom_pred"])
                .with_columns(pl.col("timestamp").cast(pl.Datetime("us", "UTC")))
            )
            preds = preds.join(dom, on=["timestamp", "timeframe"], how="left")
            logging.info(
                f"[B1] dominance OOF loaded: {S7_M1_OOF_PREDICTIONS_DOMINANCE}"
            )
        else:
            preds = preds.with_columns(pl.lit(None, dtype=pl.Float64).alias("dom_pred"))
            if self.config.entry_mode == "dominance":
                raise SystemExit(
                    "[B1] entry_mode='dominance' ですが方向脳 OOF がありません: "
                    f"{S7_M1_OOF_PREDICTIONS_DOMINANCE}\n"
                    "  → Ax2 (dominance 追加版) で再学習するか、entry_mode='ratio' にしてください。"
                )

        # --- S6 から atr_value/close/atr_ratio(+session) を結合 ---
        ...
        df = _dyn_compute_dual_levels(df)

        # --- 対象月の決定 (target_year/month。未指定なら全期間) ---
        df = df.with_columns(
            pl.col("timestamp").dt.year().alias("_y"),
            pl.col("timestamp").dt.month().alias("_m"),
        )
        ym_pairs = df.select(["_y", "_m"]).unique().sort(["_y", "_m"]).rows()
        if self.config.target_year is not None:
            ym_pairs = [
                (y, m)
                for (y, m) in ym_pairs
                if y == self.config.target_year
                and (self.config.target_month is None or m == self.config.target_month)
            ]
            if not ym_pairs:
                raise SystemExit(
                    f"[stage3] target_year/month に合致する月がありません: "
                    f"{self.config.target_year}/{self.config.target_month}"
                )
        logging.info(
            f"[stage3] 対象月: {ym_pairs} (mode={mode}, TO={TO_MINUTES}m, "
            f"K_TP={K_TP}, K_SL={K_SL})"
        )

        entry_offset_us = np.int64(DYN_ACTION_HORIZON_SEC * 1_000_000)
        parts = []
        for y, m in ym_pairs:
            cache_file = cache_root / f"year={y}/month={m}/exits.parquet"
            if self.config.use_exits_cache and cache_file.exists():
                # [鮮度チェック] OOF(予測)がキャッシュより新しい場合は再計算する。
                #   再学習後に古い予測ベースの決済を黙って再利用する事故を防ぐ。
                _oof_mtime = max(
                    Path(self.config.oof_long_path).stat().st_mtime,
                    Path(self.config.oof_short_path).stat().st_mtime,
                )
                _cached = None
                if cache_file.stat().st_mtime >= _oof_mtime:
                    _cached = pl.read_parquet(cache_file)
                    if "dom_pred" not in _cached.columns:
                        logging.info(
                            f"[stage3] {y}-{m:02d}: 旧スキーマ(dom_pred無し)のため再計算"
                        )
                        _cached = None
                if _cached is not None:
                    logging.info(
                        f"[stage3] {y}-{m:02d}: 決済キャッシュ再利用 {cache_file}"
                    )
                    parts.append(_cached)
                    continue
                logging.info(
                    f"[stage3] {y}-{m:02d}: 決済キャッシュはOOFより古いため再計算します"
                )
            ...
            margin = dt.timedelta(
                minutes=TO_MINUTES + DYN_ACTION_HORIZON_SEC / 60.0 + 5
            )
            ...
            try:
                ...
                logging.error(f"[stage3] {y}-{m:02d}: tick load failed: {e}")
                continue
            if ticks.is_empty():
                logging.warning(f"[stage3] {y}-{m:02d}: no ticks, skip.")
                continue
            ...
            opt_l, osl_l, opt_s, osl_s = _numba_find_hits_dual(
                ...
                t1,
                t1,
                month_df["pt_long"].to_numpy(),
                month_df["sl_long"].to_numpy(),
                month_df["pt_short"].to_numpy(),
                month_df["sl_short"].to_numpy(),
                ticks_ts,
                ticks_mid,
                ticks_mid,
                entry_offset_us,
            )
            resolved = _dyn_resolve_outcomes(month_df, opt_l, osl_l, opt_s, osl_s)
            resolved = self._shape_exits_out(resolved)
            if self.config.use_exits_cache:
                cache_file.parent.mkdir(parents=True, exist_ok=True)
                resolved.write_parquet(cache_file, compression="zstd")
                if cache_file.exists():
                    _sz = cache_file.stat().st_size
                    logging.info(
                        f"[stage3] {y}-{m:02d}: 決済キャッシュ保存OK "
                        f"({_sz:,} bytes) → {cache_file}"
                    )
                else:
                    logging.error(
                        f"[stage3] {y}-{m:02d}: 決済キャッシュ保存失敗(ファイル無し): "
                        f"{cache_file}"
                    )
            parts.append(resolved)
            logging.info(f"[stage3] {y}-{m:02d}: bars={resolved.height} tick走査完了")

        if not parts:
            raise SystemExit("[stage3] 決済算出結果が空です。")
    ...
    def _shape_exits_out(self, df: pl.DataFrame) -> pl.DataFrame:
        df = df.with_columns(pl.col("close").alias("entry_price"))
        for c in self._EXITS_OUT_COLS:
            if c not in df.columns:
                df = df.with_columns(pl.lit(None).alias(c))
        return df.select(self._EXITS_OUT_COLS)

    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        # [回帰版 2026-07-12] 前処理(compute_dynamic_exits_regression.py)の出力を読む。
        #   出力は全バー(NoTrade含む)を持ち、各バーに動的TP/SL tick走査済みの
        #   direction / outcome(1=TP先着/0=SL先着/-1=TO) / exit_time / exit_price を格納。
        #   旧: S6 の duration + OOF の true_label/proba を結合していた箇所を丸ごと置換。
        ...
        logging.info(
            "[stage3] 動的決済を BT 内で算出 (旧 compute_dynamic_exits を統合)"
        )
        exits_df = self._compute_exits_in_process()
        lf = (
            exits_df.lazy()
            .select(
                [
                    "timestamp",
                    "timeframe",
                    # [道B] 両方向の決済結果。BT 側で確信度ゲートにより方向を選ぶ。
                    "pt_long",
                    "sl_long",
                    "pt_short",
                    "sl_short",
                    "long_outcome",  # 1=TP先着 / 0=SL先着 / -1=TO
                    "long_exit_time",
                    "long_exit_price",  # TP/SL先着時のみ。TO は null → close_future
                    "short_outcome",
                    "short_exit_time",
                    "short_exit_price",
                    "entry_price",  # = エントリー時 close
                    "atr_value",
                    "atr_ratio",
                    "mfe_pred",
                    "mae_pred",
            ...
            .with_columns(
                pl.col("timestamp").cast(pl.Datetime("us", "UTC")),
                pl.col("entry_price").alias("close"),  # 既存ループ互換で close 名も保持
            )
        )

        # ─── TO決済用の未来価格 (close_future) を asof join(forward)で事前結合 ───
        #   全バーの close を timestamp+TD にひもづける(旧実装と同一手法、参照列のみ変更)。
        price_lf = (
            lf.select(
                [
                    pl.col("timestamp").alias("ts_future"),
                    pl.col("close").alias("_cf"),
                ]
            )
        ...
        lf = (
            ...
            .join_asof(
                price_lf.rename({"_cf": "close_future_long"}),
        ...
        lf = (
            ...
            .join_asof(
                price_lf.rename({"_cf": "close_future_short"}),
    ...
    def _run_simulation_loop(
        ...
        # [道B 段階2] 前処理の両方向決済結果 + 予測。BT 側で確信度ゲートにより方向を選ぶ。
        mfe_pred_chunk = df_chunk["mfe_pred"].to_numpy()
        mae_pred_chunk = df_chunk["mae_pred"].to_numpy()
        dom_pred_chunk = (
            df_chunk["dom_pred"].to_numpy()
            if "dom_pred" in df_chunk.columns
            else np.full(len(df_chunk), np.nan)
        )
        long_outcome_chunk = df_chunk["long_outcome"].to_numpy()
        long_exit_time_chunk = df_chunk["long_exit_time"].to_numpy()
        long_exit_price_chunk = df_chunk["long_exit_price"].to_numpy()
        short_outcome_chunk = df_chunk["short_outcome"].to_numpy()
        short_exit_time_chunk = df_chunk["short_exit_time"].to_numpy()
        short_exit_price_chunk = df_chunk["short_exit_price"].to_numpy()
        # 動的SL距離(ロット計算用) + tp/sl価格(ログ用)に使う両方向の水準
        pt_long_price_chunk = df_chunk["pt_long"].to_numpy()
        sl_long_price_chunk = df_chunk["sl_long"].to_numpy()
        pt_short_price_chunk = df_chunk["pt_short"].to_numpy()
        sl_short_price_chunk = df_chunk["sl_short"].to_numpy()
        ...
        for i in range(len(df_chunk)):
            ...
            # =========================================================
            # [道B 段階2] 確信度ゲート(比) & 方向決定
            #   mfe_pred / mae_pred の比が閾値を超えた方向にのみ発注。
            # =========================================================
            mfe_p = mfe_pred_chunk[i]
            mae_p = mae_pred_chunk[i]
            dom_p = dom_pred_chunk[i]

            should_trade_long = False
            should_trade_short = False

            _eps = 1e-12
            ratio_up = mfe_p / (mae_p + _eps)  # Long 方向への偏り
            ratio_dn = mae_p / (mfe_p + _eps)  # Short 方向への偏り

            # [段階2改修] atr_ratio は"帯"で許可 (min <= x < max)
            atr_ratio_ok = (
                self.config.atr_ratio_min <= atr_ratio_float < self.config.atr_ratio_max
            )
            # [段階2改修] セッションフィルタ (JST時間帯基準)。None/空 = 全セッション。
            if self.config.session_filter:
                _sess = utc_ts_to_session_jst(current_timestamp)
                session_ok = _sess in self.config.session_filter
            else:
                session_ok = True
            ...
            if (
                atr_ratio_ok
                and session_ok
                and np.isfinite(mfe_p)
                and np.isfinite(mae_p)
            ):
                if self.config.entry_mode == "dominance":
                    # [B1] 方向脳: dom_pred の符号で方向、|dom_pred| >= 閾値で発注。
                    #   水準(TP/SL)は従来通り mfe_pred/mae_pred から (役割分担)。
                    if np.isfinite(dom_p):
                        if (
                            dom_p >= self.config.dominance_threshold
                            and mfe_p >= self.config.min_pred_atr
                        ):
                            should_trade_long = True
                        elif (
                            dom_p <= -self.config.dominance_threshold
                            and mae_p >= self.config.min_pred_atr
                        ):
                            should_trade_short = True
                else:
                    # 従来: 確信度比 (mfe_pred/mae_pred)
                    if (
                        ratio_up >= self.config.entry_ratio_threshold
                        and mfe_p >= self.config.min_pred_atr
                    ):
                        should_trade_long = True
                    elif (
                        ratio_dn >= self.config.entry_ratio_threshold
                        and mae_p >= self.config.min_pred_atr
                    ):
                        should_trade_short = True

            # 発注なし(比/atr_ratio帯/セッション不適合)はブロックカウント
            if not should_trade_long and not should_trade_short:
                self.cb_simultaneous_prevented += 1

            # =========================================================
            # 両建て評価 (Long -> Short)。各方向は前処理の該当方向決済結果を持つ。
            #   tuple: (方向, 予測(ログ用), outcome, exit_time(us), exit_price,
            #           close_future, should_trade, 動的SL価格(ロット計算用))
            # =========================================================
            directions_to_evaluate = [
                (
                    1,
                    mfe_pred_chunk[i],
                    long_outcome_chunk[i],
                    long_exit_time_chunk[i],
                    long_exit_price_chunk[i],
                    close_future_long_chunk[i],
                    should_trade_long,
                    sl_long_price_chunk[i],
                    pt_long_price_chunk[i],
                ),  # Long評価
                (
                    -1,
                    mae_pred_chunk[i],
                    short_outcome_chunk[i],
                    short_exit_time_chunk[i],
                    short_exit_price_chunk[i],
                    close_future_short_chunk[i],
                    should_trade_short,
                    sl_short_price_chunk[i],
                    pt_short_price_chunk[i],
                ),  # Short評価
            ]
            ...
            for (
                ...
                if should_trade:
                    ...
                    if self.config.use_fixed_risk:
                        ...
                        # [道B] SL距離は動的: |entry - 前処理の動的SL価格|。
                        #   予測(mae/mfe)×atr×K_SL ぶんの逆行幅。ロットはこの距離で
                        #   最大損失=risk_pct×資金 になるよう決める。
                        if pp_sl_price is not None and np.isfinite(pp_sl_price):
                            sl_price_distance = abs(
                                current_price_decimal - Decimal(str(pp_sl_price))
                            )
                        else:
                            sl_price_distance = DECIMAL_ZERO
                    ...
                    if final_lot_size_decimal >= DECIMAL_MIN_LOT_SIZE:
                        ...
                        pnl = DECIMAL_ZERO
                        is_sl_hit = False  # SL判定フラグ

                        # [道B] 決済は前処理の tick 走査結果を再生する。
                        #   pp_outcome: 1=TP先着(勝) / 0=SL先着(負) / -1=TO
                        #   pp_exit_price: TP/SL先着時の動的水準。TO は null → close_future で決済。
                        _oc = (
                            int(pp_outcome)
                            if (pp_outcome is not None and np.isfinite(pp_outcome))
                            else -1
                        )
                        if _oc == 0:
                            is_sl_hit = True
                        ...
                        if (
                            _oc in (1, 0)
                            and pp_exit_price is not None
                            and np.isfinite(pp_exit_price)
                        ):
                            exit_price_decimal = Decimal(str(pp_exit_price))
                        else:  # TO (または exit_price 欠損) は close_future(TD価格)で決済
                            future_p = (
                                close_future_float
                                if (
                        ...
                        # PnL: 方向で符号が決まる(Long=上昇益 / Short=下落益)。式は現BAT踏襲。
                        ...
                        # ログ/決済スケジュール用: label は outcome、TD は決済までの経過分。
                        valid_label = _oc
                        if pp_exit_time is not None and np.isfinite(pp_exit_time):
                            duration_val = (
                                float(pp_exit_time) - float(current_timestamp_int)
                            ) / 60_000_000.0  # us → 分
                            if duration_val < 0.0:
                                duration_val = 0.0
                        else:
                            duration_val = float(self.config.td_minutes_long)
                        ...
                        # [道B 段階2] 保有時間 = エントリー(t0+offset)から決済までの分。
                        #   duration_val は t0(ラベル時刻)からの経過なので offset を引く。
                        _hold_min = float(duration_val) - (
                            self.config.action_horizon_sec / 60.0
                            if hasattr(self.config, "action_horizon_sec")
                            else 3.0
                        )
                        if _hold_min < 0.0:
                            _hold_min = 0.0
                        # 確信度比 (大きい方/小さい方)
                        _ratio = (
                            (mfe_p / mae_p if mae_p > 0 else float("inf"))
                            if direction_int == 1
                            else (mae_p / mfe_p if mfe_p > 0 else float("inf"))
                        )
                        # outcome ラベル: 1=TP先着(win)/0=SL先着(loss)/-1=TO
                        _oc_str = {1: "TP", 0: "SL", -1: "TO"}.get(
                            int(valid_label), "?"
                        )
                        _log_entry = {
                            "timestamp": current_timestamp,
                            "direction": int(direction_int),
                            "outcome": int(valid_label),  # 1=TP/0=SL/-1=TO
                            "result": _oc_str,
                            "mfe_pred": float(mfe_p),
                            "mae_pred": float(mae_p),
                            "dom_pred": float(dom_p) if np.isfinite(dom_p) else None,
                            "ratio": float(_ratio),
                            "entry_price": current_price_decimal,
                            "tp_price": (
                                float(pp_tp_price)
                                if (
                                    pp_tp_price is not None and np.isfinite(pp_tp_price)
                                )
                                else None
                            ),
                            "sl_price": (
                                float(pp_sl_price)
                                if (
                                    pp_sl_price is not None and np.isfinite(pp_sl_price)
                                )
                                else None
                            ),
                        ...
                        # [道B] 決済時刻は前処理の絶対時刻(exit_time, us)をそのまま使う。
                        #   TP/SL先着 or TO の実時刻。null の場合のみ TD で代替。
                        if pp_exit_time is not None and np.isfinite(pp_exit_time):
                            new_exit_time = int(pp_exit_time)
                            if new_exit_time <= current_timestamp_int:
                                new_exit_time = current_timestamp_int + int(
                                    float(duration_val) * 60 * 1_000_000
                                )
                            active_exit_times.append(new_exit_time)
        ...
        # [道B 段階2] 回帰版トレードログスキーマ
        trade_log_schema = {
            "timestamp": pl.Datetime,
            "direction": pl.Int8,  # 1=Long / -1=Short
            "outcome": pl.Int64,  # 1=TP先着 / 0=SL先着 / -1=TO
            "result": pl.Utf8,  # "TP"/"SL"/"TO"
            "mfe_pred": pl.Float64,  # 上の伸び予測(ATR)
            "mae_pred": pl.Float64,  # 下の伸び予測(ATR)
            "ratio": pl.Float64,  # 確信度比(大/小)
            "entry_price": pl.Object,
            "tp_price": pl.Float64,  # 動的TP水準
            "sl_price": pl.Float64,  # 動的SL水準
            "exit_price": pl.Float64,  # 決済価格
            "pnl": pl.Object,
            "balance": pl.Object,
            "lot_size": pl.Float64,
            "atr_value": pl.Float64,
            "atr_ratio": pl.Float64,
            "TD": pl.Float64,  # エントリーから決済までの保有分(≤TO窓)
            "leverage": pl.Float32,
            "margin": pl.Object,
            "spread": pl.Object,
            "aL": pl.Int32,
    ...
    def _analyze_and_report(self, results_df: pl.DataFrame, trade_log: pl.DataFrame):
        ...
        self._n_sl = 0
        self._n_to = 0
        self._ratio_mean = 0.0
        self._ratio_med = 0.0

        # [道B] スプレッド控除後の純PnL列(_net_pnl)を作る。残高は既に spread 控除済みなので、
        #   表示・集計をこの純PnLで行うと Session/Weekday/勝敗が残高と一致する。
        if not trade_log.is_empty():
            _pnl_f0 = [
                float(d) if (d is not None and d.is_finite()) else float("nan")
                for d in trade_log["pnl"].to_list()
            ]
            _spr_f0 = (
                [
                    float(d) if (d is not None and d.is_finite()) else 0.0
                    for d in trade_log["spread"].to_list()
                ]
                if "spread" in trade_log.columns
                else [0.0] * len(_pnl_f0)
            )
            _net0 = [p - s for p, s in zip(_pnl_f0, _spr_f0)]
            trade_log = trade_log.with_columns(pl.Series("_net_pnl", _net0))
        ...
        if total_trades > 0:
            ...
            # [道B 段階2] 勝敗は PnL 符号で判定(TP/SL/TO いずれも実損益で評価)。
            #   さらに outcome (TP先着/SL先着/TO) の内訳も別途集計する。
            tl = trade_log.with_columns(pl.Series("_pnl_f", pnl_list_float))
            winning_trades = tl.filter(pl.col("_pnl_f") > 0)
            losing_trades = tl.filter(pl.col("_pnl_f") < 0)

            num_winning_trades = len(winning_trades)
            num_losing_trades = len(losing_trades)
            win_rate = num_winning_trades / total_trades if total_trades > 0 else 0.0

            # outcome 内訳 (1=TP先着 / 0=SL先着 / -1=TO)
            self._n_tp = tl.filter(pl.col("outcome") == 1).height
            self._n_sl = tl.filter(pl.col("outcome") == 0).height
            self._n_to = tl.filter(pl.col("outcome") == -1).height
            # 確信度比の分布
            _ratio_s = (
                tl["ratio"].replace([float("inf")], None).drop_nulls().drop_nans()
            )
            self._ratio_mean = (
                float(_ratio_s.mean()) if not _ratio_s.is_empty() else 0.0
            )
            self._ratio_med = (
                float(_ratio_s.median()) if not _ratio_s.is_empty() else 0.0
            )

            winning_pnl_series = winning_trades["_pnl_f"].drop_nans()
            losing_pnl_series = losing_trades["_pnl_f"].drop_nans()
        ...
        # [道B 段階2] Strategy 名を回帰版パラメータで表示
        strategy_str = "Regression MFE/MAE "
        if self.config.use_fixed_risk:
        ...
        strategy_str += (
            f", entry_mode={self.config.entry_mode}"
            f", dom_th={self.config.dominance_threshold}"
            f", entry_ratio={self.config.entry_ratio_threshold}"
            f", atr_ratio=[{self.config.atr_ratio_min},{self.config.atr_ratio_max})"
            f", session={self.config.session_filter or 'ALL'}"
            f", min_pred_atr={self.config.min_pred_atr}"
        )
        ...
        report_data = {
            ...
            "sl_hits": self._n_sl,
            "to_exits": self._n_to,
            "tp_rate_pct": (self._n_tp / total_trades * 100) if total_trades else 0.0,
            "sl_rate_pct": (self._n_sl / total_trades * 100) if total_trades else 0.0,
            "to_rate_pct": (self._n_to / total_trades * 100) if total_trades else 0.0,
            "confidence_ratio_mean": self._ratio_mean,
            "confidence_ratio_median": self._ratio_med,
        ...
        print(f" Win Rate (PnL>0):     {report_data.get('win_rate_pct', 0.0):.2f}%")
        print(
            f" TP先着 / SL先着 / TO:  {report_data.get('tp_hits', 0)} ({report_data.get('tp_rate_pct', 0.0):.1f}%)"
            f" / {report_data.get('sl_hits', 0)} ({report_data.get('sl_rate_pct', 0.0):.1f}%)"
            f" / {report_data.get('to_exits', 0)} ({report_data.get('to_rate_pct', 0.0):.1f}%)"
        )
        print(
            f" 確信度比 (mean/median): {report_data.get('confidence_ratio_mean', 0.0):.3f}"
            f" / {report_data.get('confidence_ratio_median', 0.0):.3f}"
        )
        ...
        try:
            ...
            if not trade_log.is_empty():
                ...
                # [PF inf 対策] 損失0の群は total_pnl(=gross利益) を全体平均損失で割った
                #   実効PFを併記する。表記: inf(X.XX)。
                _all_loss_abs = [
                    abs(float(p))
                    for p in trade_log["pnl"].to_list()
                    if p is not None and float(p) < 0
                ]
                _global_avg_loss = (
                    (sum(_all_loss_abs) / len(_all_loss_abs)) if _all_loss_abs else 0.0
                )

                def _fmt_pf_inf(pf, total_pnl):
                    """PF が有限ならそのまま。inf(損失0)なら実効PFを併記 inf(X.XX)。"""
                    if pf != float("inf"):
                        return f"{pf:.2f}"
                    if _global_avg_loss > 0 and total_pnl is not None and total_pnl > 0:
                        return f"inf({total_pnl / _global_avg_loss:.2f})"
                    return "inf"
                ...
                def _win_rate_pf(trades):
                    ...
                    return wr, pf, sum(pnls)  # 3つ目=total_pnl(実効PF用)

                wr_l, pf_l, tot_l = _win_rate_pf(l_trades)
                wr_s, pf_s, tot_s = _win_rate_pf(s_trades)
                ...
                # ▼▼▼ 修正後 ▼▼▼
                # [道B] TO は outcome == -1 で直接判定
                to_count = len(trade_log.filter(pl.col("outcome") == -1))
                # [道B] 確率ビンの代わりに確信度比の分布を集計
                ratio_lst = [
                    x
                    for x in trade_log["ratio"].to_list()
                    if x is not None and np.isfinite(x)
                ]
                m2_bins = {
                    "1.0-1.1": sum(1 for x in ratio_lst if 1.0 <= x < 1.1),
                    "1.1-1.2": sum(1 for x in ratio_lst if 1.1 <= x < 1.2),
                    "1.2-1.3": sum(1 for x in ratio_lst if 1.2 <= x < 1.3),
                    "1.3-1.5": sum(1 for x in ratio_lst if 1.3 <= x < 1.5),
                    "1.5-2.0": sum(1 for x in ratio_lst if 1.5 <= x < 2.0),
                    ">= 2.0": sum(1 for x in ratio_lst if x >= 2.0),
                }
                ...
                atr_bins = {
                    ...
                    "1.5-2.0": sum(
                        1 for x in atr_rel_lst if x is not None and 1.5 <= x < 2.0
                    ),
                    "2.0-3.0": sum(
                        1 for x in atr_rel_lst if x is not None and 2.0 <= x < 3.0
                    ),
                    ">= 3.0": sum(1 for x in atr_rel_lst if x is not None and x >= 3.0),
                }

                # [道B] TD(保有時間, 分)の分布。TP/SL到達の早さ・TO張り付きを見る。
                td_lst = [
                    x
                    for x in trade_log["TD"].to_list()
                    if x is not None and np.isfinite(x)
                ]
                td_bins = {
                    "< 1": sum(1 for x in td_lst if x < 1),
                    "1-2": sum(1 for x in td_lst if 1 <= x < 2),
                    "2-3": sum(1 for x in td_lst if 2 <= x < 3),
                    "3-4": sum(1 for x in td_lst if 3 <= x < 4),
                    "4-5": sum(1 for x in td_lst if 4 <= x < 5),
                    "5-10": sum(1 for x in td_lst if 5 <= x < 10),
                    "10-15": sum(1 for x in td_lst if 10 <= x < 15),
                    "15-20": sum(1 for x in td_lst if 15 <= x < 20),
                    "20-30": sum(1 for x in td_lst if 20 <= x < 30),
                    "30-45": sum(1 for x in td_lst if 30 <= x < 45),
                    "45-60": sum(1 for x in td_lst if 45 <= x < 60),
                    "60-120": sum(1 for x in td_lst if 60 <= x < 120),
                }

                # pnl・labelリストを帯別分析で共通利用するため先に取得
                pnl_lst = trade_log["pnl"].to_list()  # raw(控除前)
                label_lst = trade_log["outcome"].to_list()  # 1=TP先着 を win 扱い
            ...
            if not trade_log.is_empty():
                ...
                pnl_lst2 = trade_log["pnl"].to_list()  # raw(控除前)
                lbl_lst2 = trade_log["outcome"].to_list()  # 1=TP先着
                atr_lst2 = (
            ...
            with open(text_report_path, "w", encoding="utf-8") as f:
                ...
                # ▼▼▼ BacktestConfigのパラメーターを出力 (残骸は除外) ▼▼▼
                ...
                for key, value in self.config.__dict__.items():
                    if key in _residual_keys:
                        continue  # 回帰版で未使用の残骸パラメータは表示しない
                ...
                # [道B] Gross(スプレッド控除前)PnL と 支払スプレッド総額 (参考)
                try:
                    _gross = float(
                        np.nansum(
                            [
                    ...
                    f.write(f"Gross PnL (控除前):\t{_gross:,.2f}\n")
                    f.write(f"Total Spread Paid:\t{_spread_tot:,.2f}\n")
                except Exception:
                    pass
                ...
                pf_l_str = _fmt_pf_inf(pf_l, tot_l)
                pf_s_str = _fmt_pf_inf(pf_s, tot_s)
                ...
                f.write(
                    "-" * 23
                    + " TD (保有時間 分) 分布 (約定トレードのみ) "
                    + "-" * 3
                    + "\n"
                )
                _td_total = sum(td_bins.values()) if td_bins else 0
                for k, v in td_bins.items():
                    pct = (v / _td_total * 100) if _td_total > 0 else 0.0
                    f.write(f"{k.ljust(10)}: {str(v).rjust(6)} ({pct:5.1f} %)\n")
                f.write("\n")
                ...
                if "m1_oof" not in str(self.config.oof_long_path):
                    try:
                        ...
                        _m1_bins = _regression_pred_bins(_m1_all)
                        f.write(
                            "-" * 20
                            + " M1 予測値(ATR) Distribution (参考 / OOF) "
                            + "-" * 2
                            + "\n"
                        )
                        f.write("  ※ M1の生の回帰出力分布（M2への入力前）\n")
                ...
                # [回帰版] prediction は mfe_atr/mae_atr の予測値(ATR)。確率でなく回帰値の
                #   ヒストグラムを出す(旧確率ビンは全て >0.95 に潰れて無意味だった)。
                try:
                    ...
                    _all_bins = _regression_pred_bins(_oof_all)
                    _oof_arr = np.array(
                        [x for x in _oof_all if x is not None], dtype=float
                    )
                    f.write(
                        "-" * 20
                        + f" {_proba_label} 予測値(ATR) Distribution (全トリガー / OOF) "
                        + "-" * 2
                        + "\n"
                    )
                    if _oof_arr.size > 0:
                        f.write(
                            f"  mean={_oof_arr.mean():.3f}  median={np.median(_oof_arr):.3f}  "
                            f"p90={np.percentile(_oof_arr, 90):.3f}  max={_oof_arr.max():.3f}\n"
                        )
                ...
                f.write(
                    "-" * 23
                    + " 確信度比 (mfe_pred/mae_pred の大/小) 分布 (約定トレードのみ) "
                    + "-" * 3
                    + "\n"
                )
                f.write("  ※ 確信度ゲート通過後の約定トレードのみ\n")
                for k, v in m2_bins.items():
                    pct = (v / total_trades) * 100 if total_trades > 0 else 0
                    f.write(f"{k.ljust(15)}: {str(v).rjust(6)} ({pct:5.1f} %)\n")
                f.write("\n")

                # [B1] 方向脳 dom_pred の分布 (約定トレードのみ、閾値調整の材料)
                if "dom_pred" in trade_log.columns:
                    _dv = [
                        float(x)
                        for x in trade_log["dom_pred"].to_list()
                    ...
                    if _dv:
                        ...
                        f.write(
                            "-" * 23
                            + " 方向脳 dom_pred 分布 (約定トレードのみ) "
                            + "-" * 3
                            + "\n"
                        )
                        f.write(
                            f"  mean={_da.mean():+.3f}  median={np.median(_da):+.3f}  "
                            f"Long側(>0)={int((_da > 0).sum())}  Short側(<0)={int((_da < 0).sum())}\n"
                        )
                        _dbins = {
                            "<= -1.0": int((_da <= -1.0).sum()),
                            "-1.0--0.5": int(((-1.0 < _da) & (_da <= -0.5)).sum()),
                            "-0.5--0.2": int(((-0.5 < _da) & (_da <= -0.2)).sum()),
                            "-0.2-0.0": int(((-0.2 < _da) & (_da <= 0.0)).sum()),
                            "0.0-0.2": int(((0.0 < _da) & (_da <= 0.2)).sum()),
                            "0.2-0.5": int(((0.2 < _da) & (_da <= 0.5)).sum()),
                            "0.5-1.0": int(((0.5 < _da) & (_da <= 1.0)).sum()),
                            "> 1.0": int((_da > 1.0).sum()),
                        }
                        for k, v in _dbins.items():
                            pct = (v / len(_da)) * 100
                            f.write(
                                f"{k.ljust(15)}: {str(v).rjust(6)} ({pct:5.1f} %)\n"
                            )
                        f.write("\n")
                ...
                for band_name, stats in atr_band_stats.items():
                    ...
                    f.write(
                        ...
                        f"{stats['win_rate']:>7.2f} {_fmt_pf_inf(stats['pf'], stats.get('total_pnl')):>10} {stats['avg_pnl']:>12.2f}\n"
                    )
                f.write("\n")
                ...
                f.write(
                    "  ※ PnL は Gross(スプレッド控除前)。控除後合計は上部 Summary を参照\n"
                )
                f.write(
                ...
                for wd in range(7):
                    ...
                    if st is None:
                        continue
                    pf_str = _fmt_pf_inf(st["pf"], st["total_pnl"])
                    f.write(
                ...
                if valid_hours:
                    ...
                    f.write(
                        f"  Best  Win Rate : {best_wr[0]:02d}:00 JST ({_session(best_wr[0])}) → {best_wr[1]['win_rate']:.2f}%\n"
                    )
                    f.write(
                        f"  Worst Win Rate : {worst_wr[0]:02d}:00 JST ({_session(worst_wr[0])}) → {worst_wr[1]['win_rate']:.2f}%\n"
                    )
                    f.write(
                        f"  Best  PF       : {best_pf[0]:02d}:00 JST ({_session(best_pf[0])}) → PF {best_pf[1]['pf']:.2f}\n\n"
                    )
            ...
            if not trade_log.is_empty():
                ...
                _tl = trade_log.with_columns(
                    [
                        ...
                        .alias("pnl_f"),
                        pl.col("outcome").cast(pl.Int32).alias("label_i"),
...
if __name__ == "__main__":
    ...
    # [道B 段階2] 確信度ゲート系 (前処理を回し直さず振れる)
    parser.add_argument(
        "--entry-ratio",
        type=float,
        default=default_config.entry_ratio_threshold,
        dest="entry_ratio",
        help=f"確信度ゲート(比): 片方の予測がもう片方の何倍超で発注。Default: {default_config.entry_ratio_threshold}",
    )
    parser.add_argument(
        "--atr-ratio-min",
        type=float,
        default=default_config.atr_ratio_min,
        dest="atr_ratio_min",
        help=f"atr_ratio 帯の下限 (min<=x<max)。Default: {default_config.atr_ratio_min}",
    )
    parser.add_argument(
        "--atr-ratio-max",
        type=float,
        default=default_config.atr_ratio_max,
        dest="atr_ratio_max",
        help="atr_ratio 帯の上限 (min<=x<max)。Default: inf (上限なし)",
    )
    parser.add_argument(
        "--session",
        type=str,
        default=None,
        dest="session_filter",
        help="発注セッション(JST)をカンマ区切りで指定。例: London,NY 。未指定=全セッション",
    )
    parser.add_argument(
        "--min-pred-atr",
        type=float,
        default=default_config.min_pred_atr,
        dest="min_pred_atr",
        help=f"勝つ側の予測 ATR 下限。0=無効。Default: {default_config.min_pred_atr}",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        dest="year_month",
        help="[stage3] tick走査/BT する月を YYYY/M で指定 (例 2021/7)。未指定=全期間",
    )
    parser.add_argument(
        "--year",
        type=int,
        default=None,
        dest="year_only",
        help="[stage3] tick走査/BT する年を指定 (例 2021)。--year-month が優先",
    )
    parser.add_argument(
        "--no-exits-cache",
        action="store_true",
        help="[stage3] 動的決済キャッシュを使わず毎回 tick 走査し直す",
    )

    args = parser.parse_args()

    # [stage3] --year-month / --year を target_year/target_month に変換
    import re as _re_ym

    _ty = _tm = None
    if args.year_month:
        _mo = _re_ym.match(r"^(\d{4})/(\d{1,2})$", args.year_month.strip())
        if not _mo:
            raise SystemExit("ERROR: --year-month は YYYY/M 形式 (例 2021/7)")
        _ty, _tm = int(_mo.group(1)), int(_mo.group(2))
    elif args.year_only is not None:
        _ty = args.year_only

    _session_list = (
        [s.strip() for s in args.session_filter.split(",") if s.strip()]
        if args.session_filter
        else default_config.session_filter  # CLI未指定なら config 上部の設定を使う
    )

    config = BacktestConfig(
        target_year=_ty if _ty is not None else default_config.target_year,
        target_month=_tm if _tm is not None else default_config.target_month,
        use_exits_cache=(not args.no_exits_cache),
        initial_capital=args.initial_capital,
        start_date=args.start_date,
        end_date=args.end_date,
        auto_lot_base_capital=args.auto_lot_base_capital,
        auto_lot_size_per_base=args.auto_lot_size_per_base,
        use_fixed_risk=args.use_fixed_risk
        if args.use_fixed_risk
        else default_config.use_fixed_risk,
        ...
        # [道B 段階2] 確信度ゲート系 (CLI で振れる)
        entry_ratio_threshold=args.entry_ratio,
        atr_ratio_min=args.atr_ratio_min,
        atr_ratio_max=args.atr_ratio_max,
        session_filter=_session_list,
        min_pred_atr=args.min_pred_atr,
    ...
    if mode_ans == "2":
        ...
        # [stage3 修正] M2 モードで config.oof_long_path を実際に M2 に設定する。
        #   (旧 stage2 ではここが抜けており、M2 でも M1 予測が使われ M1==M2 になっていた)
        config.oof_long_path = S7_M2_OOF_PREDICTIONS_LONG
        config.oof_short_path = S7_M2_OOF_PREDICTIONS_SHORT
        inference_mode = "M2"
        active_cache_path = S7_BACKTEST_CACHE_M2
        oof_ref_long = S7_M2_OOF_PREDICTIONS_LONG
        oof_ref_short = S7_M2_OOF_PREDICTIONS_SHORT
        logging.info("🧠 [M2モード] Two-Brain OOF (M2) を使用します。")

    # [stage3] BT全体キャッシュを stage3 専用に。TO/K/対象月 が違えば別ファイルにして
    #   古い準備済みデータの誤再利用を防ぐ (exits_cache とは別レイヤ)。
    _tgt_tag = (
        f"{config.target_year}-{config.target_month}"
        if config.target_year is not None
        else "ALL"
    )
    active_cache_path = active_cache_path.parent / (
        f"stage3_{inference_mode}_TO{int(TO_MINUTES)}_KTP{K_TP}_KSL{K_SL}"
        f"_{_tgt_tag}.pkl"
    )
    ...
    def load_or_generate_cache() -> tuple:
        ...
        with open(active_cache_path, "wb") as f:
            pickle.dump(data, f)
        if active_cache_path.exists():
            _sz = active_cache_path.stat().st_size
            logging.info(
                f"[{inference_mode}] キャッシュ保存完了 ({_sz:,} bytes): "
                f"{active_cache_path}"
            )
        else:
            logging.error(
                f"[{inference_mode}] キャッシュ保存失敗(ファイル無し): {active_cache_path}"
            )
        return data
