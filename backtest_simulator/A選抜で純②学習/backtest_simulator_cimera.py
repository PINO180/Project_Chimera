...
class BacktestConfig:
    ...
    # --------------------------------------------------------------------------
    #   True  (既定・従来の挙動):
    #       delta = |p_long − p_short| が m2_delta_threshold 以上のときだけ、
    #       確率の高い【片方だけ】を発注する。long と short が同時に立つことはない。
    #       ※ 以前はこの分岐が if/elif に直書きされており、本フラグは参照されて
    #          いなかった（False にしても --allow-simultaneous を付けても
    #          同時発注は解除されなかった）。本版でフラグを実際に効かせるよう修正。
    #
    #   False (--allow-simultaneous を付けたとき):
    #       long と short を独立に評価し、それぞれ m2_proba_threshold を超えたら
    #       両方発注する（delta は見ない）。
    #       用途: 「大きく振れるが一方向には行かない（レンジ）」バーで
    #       ±PT の両側を取れるかの検証。両側が PT に達すれば +2×PT、
    #       一方向に走れば片方が SL になる。無ドリフトでは期待値ゼロ−スプレッド
    #       2回分なので、「レンジに留まるバーを事前に選べる」ことが前提となる。
    #       ※ ポジション枠を2つ消費する点に注意（max_positions）。
    # ==========================================================================
    ...
    # ------------------------------------------
    # 【入れ子メタラベリング・手動設定】発注幾何は B（本番）幾何に手で合わせる。
    # combo を回すたびに、ラベリング側 RULE_B と同じ値へ下の pt/sl/td を書き換える。
    #   combos 早見表（B側のみ・td はエントリー起点の分＝内部値、BT表示はこれ+3分）:
    #     1: B pt=1.0 sl=5.0 td=30   （電源確認・下の既定値と一致）
    #     2: B pt=2.2 sl=2.2 td=30   （利益の本命）
    #     3: B pt=2.2 sl=2.2 td=17   （出血止め検証）
    #     4: B pt=2.2 sl=2.2 td=30   （定規を磨く）
    #     5: B pt=3.0 sl=3.0 td=42   （長持ちμ伏兵）
    #     6: B pt=1.0 sl=5.0 td=17   （なめらか+出血止め）
    #   ※ pt_multiplier=PT側ATR倍・sl_multiplier=SL側ATR倍。long/short 同値でよい。
    #   ※ 対称(pt=sl)では基底率≈50%になるため、下の m2_proba_threshold / m2_delta_threshold は
    #      旧1:5(基底率83%)前提の値。combo 2以降は要再スイープ。
    #   ※ OOF評価は raw M2 OOF で可。本番(live)は isotonic較正済みM2を通してから閾値。
    # ==========================================
...
class BacktestSimulator:
    ...
    def run(
        ...
        self.cb_simultaneous_taken = 0        # 両建てが実際に成立した回数（診断用）
    ...
    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        ...
        # [入れ子メタラベリング] 発注幾何=B。S6の duration_B_* を duration_* に別名化して読む。
        # （OOF側の true_label=meta_label=label_B と同一幾何＝整合。config pt/sl も RULE_B に手で合わせる。）
        base_cols = [
            "timestamp",
    ...
    def _run_simulation_loop(
        ...
        for i in range(len(df_chunk)):
            ...
            if self.config.prevent_simultaneous_orders:
                # ---- 従来の挙動: delta で勝った【片方だけ】を発注する ----
                # 条件1: 差分(Delta)が閾値以上開いていること
                # 条件2: 勝つ方の絶対確率も m2_proba_threshold を超えていること
                # ※ if/elif なので long と short が同時に True になることはない
                ...
                # 両方Falseのまま（差分不足 or 絶対確率不足）ならブロックカウント
                if not should_trade_long and not should_trade_short:
                    self.cb_simultaneous_prevented += 1
            else:
                # ---- 同時発注を許可: long/short を独立に評価する ----
                # delta は見ない。それぞれが m2_proba_threshold を超えたら発注。
                # 両方超えれば両建てになる（ポジション枠を2つ消費）。
                if p_l > self.config.m2_proba_threshold:
                    should_trade_long = True
                if p_s > self.config.m2_proba_threshold:
                    should_trade_short = True

                # どちらも閾値未満なら「発注なし」としてカウント（意味は従来と同じ）
                if not should_trade_long and not should_trade_short:
                    self.cb_simultaneous_prevented += 1
                # 両建てが実際に発生した回数を別途カウント（診断用）
    ...
    def _analyze_and_report(self, results_df: pl.DataFrame, trade_log: pl.DataFrame):
        ...
        if total_trades > 0:
            ...
            # =================================================================
            # 【修正】実現損益ベースで集計する（Net Profit と基準を揃える）
            # -----------------------------------------------------------------
            #  資金の更新は  next_capital = (current_capital − spread) + pnl
            #  なので、1取引あたりの実現損益は  pnl − spread。
            #  Net Profit = Σ(pnl − spread) となる。
            #
            #  旧実装の問題:
            #   (1) 勝敗を label（＝PT に到達したか）で分けていた。
            #       時間切れで利益が出た玉は「負け」に入り、その正の損益が
            #       total_loss に混ざる。Win Rate も「儲かった割合」ではなく
            #       「PT 到達率」を意味していた。
            #   (2) pnl にスプレッドが入っていないため、PF はコスト前、
            #       Net Profit はコスト後という別基準になり、
            #       PF>1 なのに Net<0 という矛盾が起きていた。
            #
            #  本修正では pnl_net = pnl − spread の符号で勝敗を分ける。
            #  ラベルベースの「PT 到達率」は診断用に別途 pt_hit_rate として残す。
            # =================================================================
            def _to_float_list(col_name: str, frame) -> List[float]:
                out: List[float] = []
                for v in frame[col_name].to_list():
                    try:
            ...
            _pnl = np.array(_to_float_list("pnl", trade_log), dtype=np.float64)
            if "spread" in trade_log.columns:
                _spr = np.array(_to_float_list("spread", trade_log), dtype=np.float64)
                _spr = np.nan_to_num(_spr, nan=0.0)
            else:
                _spr = np.zeros_like(_pnl)
            _pnl_net = _pnl - _spr                     # ★実現損益（スプレッド控除後）

            _valid = np.isfinite(_pnl_net)
            _win_mask = _valid & (_pnl_net > 0)
            _loss_mask = _valid & (_pnl_net <= 0)

            num_winning_trades = int(_win_mask.sum())
            num_losing_trades = int(_loss_mask.sum())
            win_rate = num_winning_trades / total_trades if total_trades > 0 else 0.0

            winning_pnl_series = pl.Series("win_pnl", _pnl_net[_win_mask])
            losing_pnl_series = pl.Series("lose_pnl", _pnl_net[_loss_mask])

            avg_profit = (
                float(winning_pnl_series.mean()) if not winning_pnl_series.is_empty() else 0.0
            )
            avg_loss = (
                float(losing_pnl_series.mean()) if not losing_pnl_series.is_empty() else 0.0
            )
            total_profit = float(winning_pnl_series.sum()) if num_winning_trades else 0.0
            total_loss = float(losing_pnl_series.sum()) if num_losing_trades else 0.0

            # --- 診断用: ラベルベースの PT 到達率（旧 Win Rate の意味）---
            _lab = trade_log["label"].to_numpy() if "label" in trade_log.columns else None
            pt_hit_rate = (
                float((_lab == 1).mean()) if _lab is not None and len(_lab) else 0.0
            )
            total_spread_cost = float(np.nansum(_spr))
            sum_pnl_net = float(np.nansum(_pnl_net))
        ...
        report_data = {
            ...
            "pt_hit_rate": pt_hit_rate,              # ラベル(PT到達)ベースの的中率
            "total_spread_cost": total_spread_cost,  # スプレッド総額
            "sum_pnl_net": sum_pnl_net,              # Σ(pnl−spread)。Net Profit と一致するはず
            ...
            "cb_simultaneous_taken": self.cb_simultaneous_taken,  # 両建て成立回数
        ...
        try:
            ...
            if not trade_log.is_empty():
                ...
                def _win_rate_pf(trades):
                    """方向別の勝率と PF。全体集計と基準を揃えて pnl−spread で判定する。

                    資金更新が next = (current − spread) + pnl なので、
                    実現損益は pnl − spread。スプレッドを引かないと
                    全体の Net Profit と食い違う。
                    """
                    if len(trades) == 0:
                        return 0.0, 0.0
                    _p = trades["pnl"].to_list()
                    _s = (trades["spread"].to_list()
                          if "spread" in trades.columns else [0.0] * len(_p))
                    pnls = []
                    for a, b in zip(_p, _s):
                        if a is None:
                            continue
                        try:
                            v = float(a) - (float(b) if b is not None else 0.0)
                        except (TypeError, ValueError):
                            continue
                        if np.isfinite(v):
                            pnls.append(v)
            ...
            with open(text_report_path, "w", encoding="utf-8") as f:
                ...
                f.write(
                    f"No-Trade (delta/proba unmet):   {report_data.get('cb_simultaneous_prevented', 0)} times\n"
                    f"Simultaneous Orders Taken:       {report_data.get('cb_simultaneous_taken', 0)} times\n"
                )
                f.write(
                ...
                # --- 診断: ラベル(PT到達)ベースの的中率 と スプレッド総額 ---
                # Win Rate は実現損益(pnl−spread)の符号で判定している。
                # PT Hit Rate はラベルが1だった割合（＝M2が予測している対象そのもの）。
                # 両者がずれるのは、時間切れで利益/損失が出た玉があるため。
                _pt = report_data.get("pt_hit_rate", None)
                if _pt is not None:
                    f.write(f"PT Hit Rate (label=1):\t{_pt * 100:.2f} %\n")
                _sp = report_data.get("total_spread_cost", None)
                if _sp is not None:
                    f.write(f"Total Spread Cost:\t{_sp:,.2f}\n")
                # --- 整合チェック: Σ(pnl−spread) と Net Profit が一致するか ---
                _sumnet = report_data.get("sum_pnl_net", None)
                if _sumnet is not None:
                    _diff = _sumnet - float(total_net_profit)
                    f.write(f"Sum(pnl-spread):\t{_sumnet:,.2f}")
                    if abs(_diff) > 0.01:
                        f.write(f"   [!] Net Profit との差 {_diff:+,.2f}")
                    f.write("\n")
                f.write("\n")
