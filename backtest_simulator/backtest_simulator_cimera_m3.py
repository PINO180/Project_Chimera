# /workspace/models/backtest_simulator_cimera_m3.py
# [V5改修版 + M3 Confidence Layer対応]
# M3 Confidence Layerのスコアに応じてロットサイジングを調整する。
# M3スコア < m3_half_threshold  → フルロット（外れにくい・安全）
# M3スコア < m3_full_threshold  → ハーフロット
# M3スコア >= m3_full_threshold → 見送り（外れやすい・危険）
# M3モデル未指定時はV5通常モード（M3スコアによるフィルタなし）で動作する。
...
class BacktestConfig:
    ...
    # ==========================================
    # M3 Confidence Layer設定
    # ==========================================
    m3_model_long_path: Path = M3_MODEL_LONG_PKL
    m3_model_short_path: Path = M3_MODEL_SHORT_PKL
    m3_oof_scores_long_path: Path = M3_OOF_SCORES_LONG
    m3_oof_scores_short_path: Path = M3_OOF_SCORES_SHORT
    use_m3: bool = True  # M3フィルターを有効にするか
    m3_full_threshold: float = 0.90  # これ以上 → 見送り（外れやすい・危険）
    m3_half_threshold: float = 0.40  # 未満 → フルロット / 以上 → ハーフロット
    # Wilder平滑化（error_ema）設定
    m3_error_ema_period: int = 50  # period=50 → alpha=0.02
    m3_error_ema_init: float = 0.05  # 初期値（起動時の外れ率推定）
    # バックテスト開始日（Noneの場合は自動検出）
    start_date: object = None  # dt.date型 or None
...
class BacktestSimulator:
    def __init__(self, config: BacktestConfig):
        ...
        # M3モデルのロード
        self.m3_model_long = None
        self.m3_model_short = None
        if self.config.use_m3:
            try:
                self.m3_model_long = joblib.load(self.config.m3_model_long_path)
                self.m3_model_short = joblib.load(self.config.m3_model_short_path)
                logging.info(
                    f"M3モデル読み込み完了: {self.config.m3_model_long_path.name}"
                )
            except Exception as e:
                logging.warning(f"M3モデル読み込み失敗 → M3フィルターなしで実行: {e}")
                self.m3_model_long = None
                self.m3_model_short = None

        # M3モード時: OOFスコアの最小タイムスタンプを自動検出してstart_dateを補正
        if self.config.use_m3 and self.m3_model_long is not None:
            try:
                m3_min_ts = (
                    pl.read_parquet(self.config.m3_oof_scores_long_path)
                    .select(pl.col("timestamp").min())
                    .item()
                )
                # dt.dateに変換
                if hasattr(m3_min_ts, "date"):
                    m3_min_date = m3_min_ts.date()
                else:
                    m3_min_date = m3_min_ts
                # start_dateがNone、またはm3_min_dateより古い場合は強制上書き
                if (
                    self.config.start_date is None
                    or self.config.start_date < m3_min_date
                ):
                    logging.info(
                        f"【自動補正】M3モード有効: バックテスト開始日を"
                        f"Fold5開始日（{m3_min_date}）に自動設定します。"
                    )
                    self.config.start_date = m3_min_date
                else:
                    logging.info(
                        f"【手動設定優先】バックテスト開始日: {self.config.start_date}"
                        f"（Fold5開始日: {m3_min_date}）"
                    )
            except Exception as e:
                logging.warning(f"M3 OOFスコアの最小タイムスタンプ取得失敗: {e}")
    ...
    def preload_data(self) -> Tuple[Dict[dt.date, pl.DataFrame], pl.DataFrame]:
        ...
        # start_date指定時（M3自動補正含む）: 開始日以降のみ処理
        if self.config.start_date is not None:
            partitions_to_process = partitions_to_process.filter(
                pl.col("date") >= self.config.start_date
            )
            logging.info(
                f"バックテスト期間: {self.config.start_date} 以降 "
                f"({len(partitions_to_process)}パーティション)"
            )
    ...
    def run(
        ...
        if preloaded_data is not None:
            ...
            # キャッシュから読み込んだ場合も start_date で再フィルタリング
            # キャッシュがフル期間で生成されていても M3モードの期間制限を確実に適用する
            if self.config.start_date is not None:
                partitions_to_process = partitions_to_process.filter(
                    pl.col("date") >= self.config.start_date
                )
                logging.info(
                    f"キャッシュ適用後のバックテスト期間: {self.config.start_date} 以降 "
                    f"({len(partitions_to_process)}パーティション)"
                )
        else:
    ...
    def _run_simulation_loop(
        ...
        # --- M3 Confidence Layer: error_ema（Wilder平滑化）---
        m3_alpha = 1.0 / self.config.m3_error_ema_period
        error_ema_long = self.config.m3_error_ema_init
        error_ema_short = self.config.m3_error_ema_init
        ...
        for i in range(len(df_chunk)):
            ...
            for exit_time, direction, is_sl, margin_used, log_entry in sorted(
                ...
                # M3 Confidence Layer: error_emaをWilder平滑化で更新
                # is_sl_hit=True（SL) or pnl<0（タイムアウト負け）を「外れ」とカウント
                if self.config.use_m3 and self.m3_model_long is not None:
                    _pnl_val = log_entry.get("pnl")
                    try:
                        _is_wrong_dec = 1.0 if (is_sl or (float(_pnl_val) < 0)) else 0.0
                    except Exception:
                        _is_wrong_dec = 0.0
                    if direction == 1:
                        error_ema_long = (
                            error_ema_long * (1 - m3_alpha) + _is_wrong_dec * m3_alpha
                        )
                    else:
                        error_ema_short = (
                            error_ema_short * (1 - m3_alpha) + _is_wrong_dec * m3_alpha
                        )
            ...
            # =========================================================
            # M3 Confidence Layer: スコア計算 & ロット調整
            # =========================================================
            m3_score_long = 1.0  # デフォルト: フルロット
            m3_score_short = 1.0
            m3_lot_mult_long = 1.0
            m3_lot_mult_short = 1.0

            if self.config.use_m3 and self.m3_model_long is not None:
                try:
                    # M3特徴量: [logit_m1, logit_m2, atr_ratio, hour_utc, weekday, error_ema]
                    # hour_utcはUTCのまま使用（学習データと一致）
                    _atr_r = (
                        float(atr_ratio_float) if np.isfinite(atr_ratio_float) else 1.0
                    )
                    _ts = current_timestamp_dt
                    _hour_utc = _ts.hour  # UTCのまま
                    _weekday = _ts.weekday()  # UTC基準の曜日

                    # logit_m1（p_lとp_sの平均から計算）
                    _p_avg = (p_l + p_s) / 2.0
                    _p_avg = max(1e-7, min(1 - 1e-7, _p_avg))
                    _logit_m1 = float(
                        np.clip(np.log(_p_avg / (1 - _p_avg)), -10.0, 10.0)
                    )

                    # Long M3スコア
                    _p_l_c = max(1e-7, min(1 - 1e-7, p_l))
                    _logit_m2_l = float(
                        np.clip(np.log(_p_l_c / (1 - _p_l_c)), -10.0, 10.0)
                    )
                    X_m3_l = np.array(
                        [
                            [
                    ...
                    m3_score_long = float(self.m3_model_long.predict(X_m3_l)[0])

                    # Short M3スコア
                    _p_s_c = max(1e-7, min(1 - 1e-7, p_s))
                    _logit_m2_s = float(
                        np.clip(np.log(_p_s_c / (1 - _p_s_c)), -10.0, 10.0)
                    )
                    X_m3_s = np.array(
                        [
                            [
                    ...
                    m3_score_short = float(self.m3_model_short.predict(X_m3_s)[0])

                except Exception:
                    pass  # M3推論失敗時はフルロットで継続

                # スコアに応じたロット倍率
                # スコアが低い＝外れにくい＝安全、スコアが高い＝外れやすい＝危険
                if m3_score_long < self.config.m3_half_threshold:
                    m3_lot_mult_long = 1.0  # フルロット（超安全）
                elif m3_score_long < self.config.m3_full_threshold:
                    m3_lot_mult_long = 0.5  # ハーフロット
                else:
                    should_trade_long = False  # 見送り（危険）

                if m3_score_short < self.config.m3_half_threshold:
                    m3_lot_mult_short = 1.0  # フルロット（超安全）
                elif m3_score_short < self.config.m3_full_threshold:
                    m3_lot_mult_short = 0.5  # ハーフロット
                else:
                    should_trade_short = False  # 見送り（危険）
            ...
            for (
                ...
                if should_trade:
                    ...
                    # M3 Confidence Layer: ロット倍率を適用
                    _m3_mult = (
                        m3_lot_mult_long if direction_int == 1 else m3_lot_mult_short
                    )
                    if _m3_mult < 1.0:
                        raw_lot_size = raw_lot_size * Decimal(str(_m3_mult))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        dest="start_date",
        help="バックテスト開始日（YYYY-MM-DD形式）。M3モード時はOOF最小日で自動補正される。",
    )
    parser.add_argument(
        "--no-m3",
        action="store_true",
        dest="no_m3",
        help="M3フィルターを無効化してV5通常モードで実行する。",
    )

    args = parser.parse_args()

    # start_dateをdt.date型に変換
    _start_date = None
    if args.start_date:
        import datetime as _dt2

        _start_date = _dt2.date.fromisoformat(args.start_date)
    ...
    config = BacktestConfig(
        ...
        m2_delta_threshold=args.m2_delta,
    ...
    # =========================================================
    # 推論モード選択: M2 + M3あり / M2のみ
    # =========================================================
    print("\n" + "=" * 50)
    print("  🧠 推論モードを選択してください:")
    print("    [1] M2 + M3モード (M3 Confidence Layer有効) [デフォルト]")
    print("    [2] M2のみ (M3フィルターなし・V5通常モード)")
    print("=" * 50)
    ...
    if mode_ans == "2":
        config.use_m3 = False
        inference_mode = "M2"
        logging.info("🧠 [M2のみ] M3フィルターなしで実行します。")
    else:
        config.use_m3 = True
        inference_mode = "M2+M3"
        logging.info("🧠🔍 [M2+M3] M3 Confidence Layerを有効化します。")
    ...
    _m3_tag = (
        f"_M3Full{config.m3_full_threshold}_Half{config.m3_half_threshold}"
        if config.use_m3
        else "_noM3"
    )
    _folder_name = (
