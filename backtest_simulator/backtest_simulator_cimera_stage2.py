...
class BacktestConfig:
    ...
    # [要注意] TO窓(分)。前処理の TO_MINUTES と一致させること (現在: 30)。
    #   TO決済価格(close_future)を timestamp+この値 で引くため、前処理の窓とズレると
    #   TO決済価格が誤る。前処理を TO_MINUTES=30 で回したので 30。
    td_minutes_long: float = 10.0
    td_minutes_short: float = 10.0
...
class BacktestSimulator:
    ...
    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        ...
        logging.info(
            f"Preparing data from dynamic-exits pre-pass: {self.config.dynamic_exits_path}"
        )
        exits_lf = pl.scan_parquet(str(self.config.dynamic_exits_path / "**/*.parquet"))
        lf = exits_lf.select(
            [
                "timestamp",
                "timeframe",
