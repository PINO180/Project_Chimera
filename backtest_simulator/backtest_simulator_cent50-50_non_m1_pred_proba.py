...
class BacktestSimulator:
    def __init__(self, config: BacktestConfig):
        ...
        # --- [修正] M1もM2も Top 50 特徴量のみを使用する ---
        # 1. M1用: Top 50 (50-50モードなので全特徴量は使わない)
        self.features_base = self.top_50_features

        # 2. M2用: Top 50 (検証1: m1_pred_proba を除外するため Top 50 そのまま)
        self.features_m2 = self.top_50_features

        logging.info(
            f"features defined: M1({len(self.features_base)}) & M2({len(self.features_m2)}) using Top 50 ONLY (No m1_pred_proba)."
        )
    ...
    # --- [追加] Top 50特徴量ロード用メソッド (これがないと動きません) ---
