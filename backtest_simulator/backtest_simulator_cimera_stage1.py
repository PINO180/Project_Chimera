...
class BacktestConfig:
    ...
    td_minutes_long: float = 6.0
    td_minutes_short: float = 6.0
...
class BacktestSimulator:
    ...
    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        ...
        exits_lf = pl.scan_parquet(
            str(self.config.dynamic_exits_path / "**/*.parquet")
        )
        lf = exits_lf.select(
            [
                "timestamp",
                "timeframe",
                "direction",  # 1=Long / -1=Short / 0=NoTrade
                "outcome",  # 1=TP先着 / 0=SL先着 / -1=TO / -2=NoTrade
                "exit_time",  # 先着/TO の決済時刻(us)。NoTrade は null
                "exit_price",  # TP/SL先着時の動的決済価格。TO/NoTrade は null(→close_futureに委ねる)
