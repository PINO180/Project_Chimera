...
class BacktestSimulator:
    ...
    def run(self):
        ...
        logging.info(
            f"Strategy: Fixed Lot = {self.config.fixed_lot_size}, "
        ...
        for i in range(len(df_chunk)):
            ...
            for (
                ...
                if should_trade:
                    ...
                    # 固定ロットの適用
                    final_lot_size_decimal = DECIMAL_FIXED_LOT
    ...
    def _analyze_and_report(self, results_df: pl.DataFrame, trade_log: pl.DataFrame):
        ...
        report_data = {
            "strategy": f"V5 Two-Brain Fixed Lot (Lot: {self.config.fixed_lot_size}, M2 Thresh: {self.config.m2_proba_threshold}, SL Mult: {self.config.sl_multiplier}, PT Mult: {self.config.pt_multiplier})",
        ...
        if results_df.is_empty() or drawdown.is_empty():
            ...
            try:
                ...
                ax1.set_title(
                    f"V5 Equity Curve (Fixed Lot: {self.config.fixed_lot_size}, M2 Thresh: {self.config.m2_proba_threshold})",
                    fontsize=16,
                )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Project Forge V5 Backtest Simulator (Two-Brain, Fixed Lot + Dynamic SL + Timeouts)"
    )

    parser.add_argument(
        "--fixed-lot",
        type=float,
        default=default_config.fixed_lot_size,
        dest="fixed_lot_size",
        help=f"Fixed lot size per trade. Default: {default_config.fixed_lot_size}",
    )
    parser.add_argument(
        "--base-leverage",
        type=float,
    ...
    config = BacktestConfig(
        fixed_lot_size=args.fixed_lot_size,
