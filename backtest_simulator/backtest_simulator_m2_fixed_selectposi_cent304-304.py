...
class BacktestSimulator:
    def __init__(self, config: BacktestConfig):
        ...
        # --- [修正: 実験パターン② (Full 304) 用] ---
        # M2の入力: M1予測確率 + Base特徴量 (304個)
        # ※ Top 50 JSONの読み込みを廃止し、features_base をそのまま使用します
        self.features_m2 = ["m1_pred_proba"] + self.features_base

        logging.info(
            f"M2 features redefined with {len(self.features_base)} Base features + m1_pred_proba."
        )
    ...
    def _prepare_data(self) -> Tuple[pl.LazyFrame, pl.DataFrame]:
        ...
        if not self.config.oof_mode:  # In-Sample Mode
            ...
            # --- [修正: 実験パターン②] Top 50 の追加ロード処理を削除 ---
            # (features_base に304個すべて含まれているため、追加は不要)
