...
OUTPUT_DIR = Path(
    "/workspace/data/XAUUSD/stratum_7_models/1A_2B/Chimera -4- Reunion first stage 110/backtast data/018"
)
...
class BacktestConfig:
    ...
    fixed_risk_percent: float = (
        0.10  # 口座残高の何%を1トレードのリスクとするか (0.02 = 2%)
    )
    # ▲▲▲ ここまで追加 ▲▲▲
                ...
                if should_trade:
                    ...
                    if self.config.use_fixed_risk:
                        ...
                        spread_cost_per_lot = Decimal(
                            str(self.config.spread_pips)
                        ) * Decimal(str(self.config.value_per_pip))

                        # 1ロットあたりの「トータル最大損失額」（値幅損 ＋ スプレッド損）
                        total_risk_per_lot = sl_loss_per_lot + spread_cost_per_lot

                        if total_risk_per_lot > DECIMAL_ZERO:
                            # 許容損失をトータルリスクで割ることで、スプレッド込みでピッタリ2%に収まるロットを算出
