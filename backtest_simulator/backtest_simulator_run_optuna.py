...
# ランキング出力
# =========================================================
def export_rankings(study: optuna.Study, top_n: int = 20):
    """全Trial終了後・各Trial後にランキングをCSV & コンソール出力する"""
    ...
    rankings = [
        ...
        (
            ...
            "Max_DD_Pct",
            False,
        ),  # DD小さい順（負値のため降順=絶対値小さい順）
    ]

    for label, sort_key, ascending in rankings:
        df_ranked = df.sort_values(by=sort_key, ascending=ascending).head(top_n)
        # CSV保存
        out_path = out_base / f"ranking_top{top_n}_{label}.csv"
        ...
        print(
            ...
            f"{'Sharpe':>7} {'PF':>6} {'DD%':>6} {'Trades':>7} {'Score':>8}"
        )
        print("-" * 70)
        for rank, (_, row) in enumerate(df_ranked.iterrows(), 1):
            print(
                ...
                f"{row['Sharpe_Ratio']:>7.2f} {row['Profit_Factor']:>6.2f} "
                f"{row['Max_DD_Pct']:>6.2f} {int(row['Total_Trades']):>7} "
                f"{row['Score']:>8.4f}"
            )
...
def save_csv_callback(study: optuna.Study, trial: optuna.trial.FrozenTrial):
    ...
    export_rankings(study, top_n=20)
...
if __name__ == "__main__":
    ...
    search_space = {
        "m2_th": [0.30, 0.40, 0.50, 0.55, 0.60],
        "m2_delta": [0.00, 0.20, 0.30, 0.40, 0.50],
        "fixed_risk_pct": [0.02, 0.03, 0.05, 0.07, 0.10],
    }
    study_name = "v5_grid_full_125"
