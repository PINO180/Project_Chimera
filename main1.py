...
def initialize_data_buffer(
    ...
    market_proxy_cache: pd.DataFrame,  # 👈 修正済み
) -> bool:
    """
    [V8.0 修正]
    M1データのみを可能な限り（350万本）取得し、
    エンジンが M3～MN のすべてをリサンプリングする。
    """
    logger.info("ZMQ経由で全時間足の履歴データを取得中 (V8.0: M1 Only)...")
    ...
    # 2. 必要なM1本数を計算
    # D1 (257本 * 1440分 = 37万本) を満たすため、50万本取得する
    lookback = 50000  # 👈 50万本に修正
    ...
    # エンジンに全履歴データを一括で渡す
    engine.fill_all_buffers(history_data_map, market_proxy_cache)  # 👈 修正済み
