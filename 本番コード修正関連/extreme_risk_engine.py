...
class ExtremeRiskEngineV2:
    ...
    def predict_with_confidence(
        ...
        """
        較正済みモデルで予測と確信度を取得 (50-50モデル仕様)

        [修正]
        - M2入力ベクトルを ["m1_pred_proba"] + [Top 50 Features] に統一。
        - 学習時に使用していない文脈特徴量(Context Features)を削除。
        """
        # モデル存在チェック
        ...
        # --- M2入力ベクトルの構築 (50-50モデル仕様) ---
        # 正解: ["m1_pred_proba"] (index 0) + [Top 50 Features]

        # 1. 先頭に M1確率 (較正済み) を配置
        extended_features_list = [float(p_m1)]

        # 2. 続いて Top 50 特徴量 (features[0]) を結合
        extended_features_list.extend(list(features[0]))

        # (注意: V4/ContextPlus由来の "hmm_prob" 等の追加コードは削除しました)
    ...
    def calculate_kelly_fraction(
        ...
        # [修正] 0.0 や 1.0 も許容するように不等号を変更 (<, >)
        # また、どの値が無効か分かるようにログを分離してデバッグしやすくする
        if win_loss_ratio <= 0:
            logger.warning(
                f"ケリー基準計算スキップ: 無効なペイオフレシオ (b={win_loss_ratio})"
            )
            return 0.0

        if p_win < 0 or p_win > 1:
            logger.warning(f"ケリー基準計算スキップ: 無効な確率 (p={p_win})")
            return 0.0
